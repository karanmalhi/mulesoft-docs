...
In addition to the functionality described on this page, you can also use the link:/docs/display/34X/SXC+Module+Reference[SXC Module Reference], which enables efficient XPath XML routing.
...
|link:/docs/display/34X/XmlObject+Transformers[XmlToObject <-> ObjectToXml] |Converts XML to a Java object and back again using http://xstream.codehaus.org/[XStream].
|link:/docs/display/34X/JAXB+Transformers[JAXB XmlToObject <-> JAXB ObjectToXml] |Converts XML to a Java object and back again using the http://java.sun.com/developer/technicalArticles/WebServices/jaxb/[JAXB] binding framework (ships with JDK6)
|link:/docs/display/34X/XSLT+Transformer[XSLT] |Transforms XML payloads using XSLT.
|link:/docs/display/34X/XQuery+Transformer[XQuery] |Transforms XML payloads using http://en.wikipedia.org/wiki/XQuery[XQuery].
|link:/docs/display/34X/DomToXml+Transformer[DomToXml <-> XmlToDom] |Converts DOM objects to XML and back again.
|link:/docs/display/34X/XmlToXMLStreamReader+Transformer[XmlToXMLStreamReader] |Converts XML from a message payload to a StAX XMLStreamReader.
|link:/docs/display/34X/XPath+Extractor+Transformer[XPath Extractor] |Queries and extracts object graphs using XPath expressions using JAXP.
|link:/docs/display/34X/JXPath+Extractor+Transformer[JXPath Extractor] |Queries and extracts object graphs using XPath expressions using JXPath.
|link:/docs/display/34X/XmlPrettyPrinter+Transformer[XmlPrettyPrinter] |Allows you to output the XML with controlled formatting, including trimming white space and specifying the indent.
|===
...
The XML module contains various XPath filters. For general details on how to use filters, see link:/docs/display/34X/Filters[Filters].
