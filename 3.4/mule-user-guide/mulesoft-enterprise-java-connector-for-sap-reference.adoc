...
image:/docs/download/attachments/95393278/sap-transport-image-01.png?version=1&modificationDate=1374598332042[image]
...
This procedure assumes that you already have a Mule instance installed on your host machine. If not, then follow the link:/docs/display/34X/Installing[instructions] for downloading and installing Mule.
...
Port 3300 is predefined by SAP, so if you need to validate other port numbers based on your SAP instance number, you can check the complete list of service-to-port mappings, which can be found link:/docs/display/34X/SAP+JCo+Server+Services+Configuration[here].
...
|jcoClientExtendedProperties-ref |A reference to `java.util.Map` containing additional JCo connection parameters. Additional information and a complete list of parameters can be found link:/docs/display/34X/SAP+JCo+Extended+Properties[here] |  |1.0
...
[NOTE]
You can use link:/docs/display/34X/Mule+Expression+Language+MEL[Mule expressions] inside the address attribute, just as you do for other Mule ESB transports.
...
The complete documentation for the different XML documents can be found link:/docs/display/34X/XML+Definitions[here]:

* link:/docs/display/34X/XML+Definitions[JCo Function]
** link:/docs/display/34X/XML+Definitions[Version 1]
** link:/docs/display/34X/XML+Definitions[Version 2]
* link:/docs/display/34X/XML+Definitions[IDoc]
...
|address |The standard way to provide endpoint properties. For more information check: link:#MuleSoftEnterpriseJavaConnectorforSAPReference-EndpointAddress[Endpoint Address]. |  |1.0
...
|jcoClientExtendedProperties-ref |A Reference to `java.util.Map`, which contains additional JCo connection parameters. Additional information and a complete list of parameters can be found link:/docs/display/34X/SAP+JCo+Extended+Properties[here]. |  |1.0
|===
...
After the flow executes and if the SAP inbound endpoint is request-response, then Mule will return values to the calling SAP instance. Return values can be represented with the XML representation of the link:/docs/display/34X/XML+Definitions[JCo Function]. +
...
|name |The reference name of the endpoint used internally by mule configuration. |  |1.0
|exchange-pattern |The available options are `request-response` and `one-way`. |  |1.0
|address |The standard way to specify endpoint properties. For more information check: link:#MuleSoftEnterpriseJavaConnectorforSAPReference-EndpointAddress[Endpoint Address]. |  |1.0
...
For more information, consult: link:#[Transactions Configuration Reference].
...
* link:/docs/display/34X/Outbound+Endpoint+Transactions[sRFC stateful]
* link:/docs/display/34X/Outbound+Endpoint+Transactions[sRFC stateful BAPI transaction]
* link:/docs/display/34X/Outbound+Endpoint+Transactions[tRFC stateful]
* link:/docs/display/34X/Outbound+Endpoint+Transactions[qRFC stateful]

[WARNING]
If a transaction is not specified, then all calls (execute function or send IDoc) are stateless.

For examples, please take a look at link:/docs/display/34X/Outbound+Endpoint+Transactions[Outbound Endpoint Transactions].
...
* link:/docs/display/34X/Troubleshooting+SAP+Connector[Checking log files]
* link:/docs/display/34X/Troubleshooting+SAP+Connector[Enabling JCo trace]
* link:/docs/display/34X/Troubleshooting+SAP+Connector[Common errors]
