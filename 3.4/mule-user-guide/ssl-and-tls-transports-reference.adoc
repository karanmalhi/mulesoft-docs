...
* The link:/docs/display/34X/TCP+Transport+Reference[TCP Transport], which uses the basic TCP transport.
...
* Communicating with an external service that uses low-level unsecured TCP connections? If so, use the link:/docs/display/34X/TCP+Transport+Reference[TCP] protocol.

* Are you communicating with a flow always located in the same Mule application instance? If so, consider using the link:/docs/display/34X/VM+Transport+Reference[VM transport].
