...
image:/docs/download/attachments/98310299/standard_synchronous.png?version=1&modificationDate=1389628675555[image]

image:/docs/download/attachments/98310299/standard_asynchronous.png?version=1&modificationDate=1389628675546[image]
...
* In clusters of Mule servers, messages can migrate between nodes when sent to an asynchronous flow. This allows for *load balancing between nodes* and higher performance of application. (See our link:/docs/display/34X/Flow+Architecture+Advanced+Use+Case[advanced use case] for an example.)
...
This type of flow processes messages along a single thread, which is ideally suited to link:#FlowsandSubflows-transactional[transactional processing]. 
...
All three types of flows are identified by name and, thus, can be called as often as needed via link:/docs/display/34X/Flow+Ref+Component+Reference[flow reference components] in other flows. For example, if an asynchronous Flow B's job is to update a database with logged data, any number of other flows can feed data to be logged into Flow B to update the same database (see image below). Because it processes messages asynchronously from any flow that triggers its execution, Flow B doesn't hold up the processing of any of its sister flows. Read more about link:#FlowsandSubflows-TriggeringFlows[Triggering Flows] below.

image:/docs/download/attachments/98310299/async_example.png?version=1&modificationDate=1389628675516[image]
...
image:/docs/download/attachments/98310299/subflow_example.png?version=1&modificationDate=1389628675584[image]
...
image:/docs/download/attachments/98310299/sync_example.png?version=1&modificationDate=1389628675603[image]
...
Read more about setting up link:/docs/display/34X/Transactional[Transactional units] in Mule applications.
====
...
image:/docs/download/attachments/98310299/async_flow_example.png?version=1&modificationDate=1389628675528[image]
...
|Asynchronous Flow |Flow Reference wrapped within an link:/docs/display/34X/Async+Scope+Reference[Async Scope] |asynchronous |not inherited
|===

== See Also

* *NEXT STEP:* Learn the various ways you can link:/docs/display/34X/Deploying+Mule+Applications[deploy Mule applications].
...
* Read about some alternative ways to control message processing within a flow using link:/docs/display/34X/Routing+Message+Processors[routing message processors].
* Refer to the link:/docs/display/34X/Flow+Ref+Component+Reference[Flow Reference Component Reference] and link:/docs/display/34X/Async+Scope+Reference[Async Scope].
* Read more about link:/docs/display/34X/Flow+Processing+Strategies[Flow Processing Strategies].
* Read more about setting up link:/docs/display/34X/Transactional[transactional units] in Mule applications.
