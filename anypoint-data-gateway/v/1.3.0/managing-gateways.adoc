...
|6 |Support button. Click to contact MuleSoft support.
...
When copying an object, Data Gateway performs checks to ensure that the new object being created in the target gateway is valid. Data Gateway refuses to perform a copy operation that would result in an invalid object in the target gateway. For example, if the source object belongs to a database table that does not exist in the target gateway, or if there is a mismatch between existing fields, Data Gateway will report the errors, and will not copy the object.
