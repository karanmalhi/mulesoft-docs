...
image:gateway_list_numbered-1.2.png[gateway_list_numbered-1.2]
...
image:modif.object-1.2.png[modif.object-1.2]

[[copy]]
==== Copying an Object Between Gateways

Data Gateway allows you to copy an object from one gateway to another. This feature can be specially useful when working with multiple environments that you wish to maintain in a similar state, such as a development and a production environment.

When copying an object, Data Gateway performs checks to ensure that the object being copied is valid in the target gateway, and reports any errors. Data Gateway refuses to copy an object if is not valid in the target gateway (for example, if it includes database tables that do not exist in the target gateway). In this case, Data Gateway allows you to view and correct the errors (for example, create the relevant table in the target gateway, or assign the object to an existing table).

To copy an object, follow these steps:

. Access the list of objects for the target gateway, i.e. the gateway that you wish to to copy the object(s) to. (For example, in the list of gateways, click the desired gateway, then click *Objects* in the left-hand menu).
. Click the down arrow next to *Create Object*, then select *Copy Objects From...* With this operation, you will select an object from another gateway to copy into the current gateway.

image:copy_objs_menu.png[Copy Objects menu]

. Data Gateway displays the *Copy Objects* screen. From the first drop-down menu, select the source environment, i.e. the environment to which your source gateway belongs. When you select a source environment, the source gateway drop-down menu (the second drop-down menu in the screen) loads a list of gateways valid for that environment, or displays a message if there are no valid gateways.
. From the second drop-down menu, select a source gateway from the list.
. When you select a source gateway, the list of objects on the left becomes populated with objects available on the source gateway. You can hover the mouse over an object to view a list of the fields in that object. Select the object(s) you wish to copy, then click *Copy Objects*.

If the object(s) you wish to copy already exist in the target gateway, Data Gateway prompts you to overwrite them. If the copy operation completes without error, Data Gateway again displays the list of objects.

If the copy operation produces error, Data Gateway does *not* copy the object, and displays an error message:
