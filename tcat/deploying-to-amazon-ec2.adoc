...
* link:#DeployingtoAmazonEC2-InstallingthePlug-in[Installing the Plug-in]
* link:#DeployingtoAmazonEC2-CreatinganAmazonEC2Account[Creating an Amazon EC2 Account]
* link:#DeployingtoAmazonEC2-OpeningPorts[Opening Ports]
* link:#DeployingtoAmazonEC2-AmazonMachineImages[Amazon Machine Images]
* link:#DeployingtoAmazonEC2-UsingthePlugin[Using the Plugin]
* link:#DeployingtoAmazonEC2-CreatingaTcatServerAMI[Creating a Tcat Server AMI]
* link:#DeployingtoAmazonEC2-ConfiguringTcatServerforAutomaticStartuponYourAMI[Configuring Tcat Server for Automatic Startup on Your AMI]
...
You will also need to create a key pair if you do not already have one. To do this, log into the https://console.aws.amazon.com/ec2[EC2 Console], click *Key-Pairs* on the left, click the *Create Key Pair* button, and then enter a key pair name.
...
This will open up ports 8080 and 51433 by default.
...
You will want to link:#DeployingtoAmazonEC2-createAMI[create your own AMI] with a Tcat Server instance on it. For testing purposes, we provide an image with Tcat Server and Ubuntu 9.04 from http://alestic.com[Alestic]. On startup, this image will start Tcat Server and make it available on port 8080. Please note that we provide this image on an *unsupported basis* solely for testing, and the image ID and details of this image may change in the future. The test image ID is: ami-f7d8389e
...
. Enter the ID of the AMI you created, or use the test AMI ID (see link:#DeployingtoAmazonEC2-AMI[Amazon Machine Images] above).
. Specify the instance type (size). This will affect how much your Amazon account is charged.
...
. Enter the URL path where the Tcat Server agent WAR will reside. By default, set this to `/agent`.
. Enter the port to use for the Tcat Server agent.
. Select any server groups you want to add these new servers to.
. Click *Add*.

After the plug-in issues a new instance request, it takes a while for Amazon to provision the image. The instances list will display "pending" until the image is created, at which point it will display "running". Your new Tcat Server instance will then be available on the Servers tab if you opted to automatically register it.
...
. Execute the `update-rc.d tomcat defaults` command. This will configure Ubuntu to run the init script on startup.
