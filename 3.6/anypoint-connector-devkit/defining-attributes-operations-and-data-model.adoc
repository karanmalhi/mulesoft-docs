...
More details can be found in link:/documentation/display/current/Defining+Connector+Attributes[Defining Connector Attributes]. 

== Static Versus Dynamic Data Models 

The underlying data model for the API you connect to often drives design decisions about the best way to represent data going to and from your connector. The goal in general is to choose a representation that fits the underlying data, while still supporting the necessary metadata to work with link:/documentation/display/current/DataSense[DataSense].
...
More details can be found in Static Data Model & Dynamic Data Model Examples in this document about link:/documentation/display/current/Adding+DataSense[Adding DataSense].
...
* *NEXT:* Continue to link:/documentation/display/current/Defining+Connector+Attributes[Defining Connector Attributes].
* You can also jump into your specific use case, checking the link:/documentation/display/current/Anypoint+Connector+Examples[examples].
