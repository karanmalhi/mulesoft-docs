...
[TIP]
The new *Mule Expression Language (MEL)* removes the need to use old expressions in Mule. Refer to link:/documentation/display/current/Mule+Expression+Language+MEL[Mule Expression Language MEL] for details on how to use MEL.
...
Using the custom evaluator allows you to define your custom evaluator via the `custom-evaluator` attribute. Note that some evaluators such as Xpath, Groovy, and Bean are loaded from other Mule modules (link:/documentation/display/current/XML+Module+Reference[XML] and link:/documentation/display/current/Scripting+Module+Reference[Scripting], respectively). These modules must be on your classpath before their evaluators can be used.
...
|ognl |`#[ognl:[MULE:0].equals(42)]` |Set the `evaluator` attribute on the `<expression-filter>` element to `ognl` when specifying an link:#[OGNL filter]. *Note*: OGNL is deprecated in Mule 3.6 and will be removed in Mule 4.0.
