= Tuning Performance
:keywords:tuning, finetune, fine tuning, performance, configuration
...
image:/documentation/download/attachments/122752565/thread_pooling.png?version=1&modificationDate=1419022796475[image]
...
The _threading profile_ specifies how the thread pools behave in Mule. You can specify a separate threading profile for each receiver thread pool, flow thread pool, and dispatcher thread pool. The most important setting of each is `maxThreadsActive`, which specifies how many threads are in the thread pool. The link:#TuningPerformance-calculating[Calculating Threads] section below describes methodologies for determining what this setting should be, whereas the link:#TuningPerformance-threadConfig[Threading Profile Configuration Reference] section describes the options you can set for each threading profile.
...
For more information on configuring the pooling profile, see link:#TuningPerformance-pooling[Pooling Profile Configuration Reference] below.
...
See also the previous link:#TuningPerformance-maxbuffersizenote[maxBufferSize note].
...
If you configure a threading profile with `poolExhaustedAction=WAIT` and a maxBufferSize of a positive value, the thread pool does not grow from `maxThreadsIdle` (corePoolSize) towards `maxThreadsActive` (`maxPoolSize`) _unless_ the queue is completely filled up. See also the previous link:#TuningPerformance-maxbuffersizenote[maxBufferSize note].
