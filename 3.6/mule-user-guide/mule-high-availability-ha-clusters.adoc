...
image:/documentation/download/attachments/122752309/FailoverNode2Fail.png?version=1&modificationDate=1399323534162[image]
...
image:/documentation/download/attachments/122752309/cluster-diagram.png?version=1&modificationDate=1399323534146[image]
...
* Cluster lifecycle management and control. See link:/documentation/display/current/Managing+Mule+High+Availability+%28HA%29+Clusters[Managing Mule High Availability (HA) Clusters].
* Cluster and node performance monitoring. See link:/documentation/display/current/Monitoring+a+Cluster[Monitoring a Cluster].
...
image:/documentation/download/attachments/122752309/topology_4-cluster.png?version=1&modificationDate=1399323534183[image]
...
image:/documentation/download/attachments/122752309/load_balancing.png?version=1&modificationDate=1399323534176[image]
...
[NOTE]
Note that high performance is implemented differently on link:/documentation/display/current/CloudHub[CloudHub], so this section applies only for on-premises deployments.
...
To evaluate Mule's HA clustering capabilities first-hand, download the link:/documentation/display/current/Evaluating+Mule+High+Availability+Clusters+Demo[*Mule HA Demo Bundle*]. Designed to help new users evaluate the capabilities of Mule High Availability Clusters, the Mule HA Demo Bundle teaches you how to use the Mule Management Console to create a cluster of Mule instances, then deploy an application to run on the cluster. Further, this demo simulates two processing scenarios that illustrate the cluster’s ability to automatically balance normal processing load, and its ability to reliably remain active in a failover situation.
...
* If your application processes messages from a non-transactional transport, use a link:/documentation/display/current/Reliability+Patterns[reliability pattern] to move them to a transactional store such as a VM or JMS store.
...
* Implement link:/documentation/display/current/Reliability+Patterns[reliability patterns] to create high reliability applications.
...
* If your link:/documentation/display/current/Endpoint+Configuration+Reference[custom message source] does not use a message receiver to define node http://en.wikipedia.org/wiki/Polling_(computer_science)[polling], then you must configure your message source to implement a ClusterizableMessageSource interface. +
