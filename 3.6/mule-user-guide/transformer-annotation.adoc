...
Transformers in Mule are used to convert messages from one data type to another. As well as the in-built link:/documentation/display/current/Using+Transformers[Mule transformers], it is now possible to create custom transformers using the `@Transformer` annotation.
...
All messages in Mule have headers. If you need to access the headers, add a parameter to the transformer method signature annotated with the link:/documentation/display/current/InboundHeaders+Annotation[@InboundHeaders] or link:/documentation/display/current/OutboundHeaders+Annotation[@OutboundHeaders] annotations. If you need access to the message attachments use the link:/documentation/display/current/InboundAttachments+Annotation[@InboundAttachments] or link:/documentation/display/current/OutboundAttachments+Annotation[@OutboundHAttachments] annotations.
...
Another way would be to add the class to the registry-bootstrap.properties file. Click link:/documentation/display/current/Bootstrapping+the+Registry[here] for more information about along side some example.
