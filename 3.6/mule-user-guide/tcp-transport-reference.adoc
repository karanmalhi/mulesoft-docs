...
* The link:/documentation/display/current/SSL+and+TLS+Transports+Reference[SSL and TLS Transports], which use TCP with socket-level security. 
...
]http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_multicast/namespace-overview.html[SchemaDoc] |image:http://www.mulesoft.org/documentation/images/icons/emoticons/check.gif[image] |image:http://www.mulesoft.org/documentation/images/icons/emoticons/check.gif[image] |image:http://www.mulesoft.org/documentation/images/icons/emoticons/check.gif[image] |image:http://www.mulesoft.org/documentation/images/icons/emoticons/error.gif[image] |image:http://www.mulesoft.org/documentation/images/icons/emoticons/error.gif[image] |image:http://www.mulesoft.org/documentation/images/icons/emoticons/error.gif[image] |one-way, request-response |request-response |org.mule.transport:mule-transport-multicast
...
* Are you communicating with a flow always located in the same Mule application instance? If so, consider use the link:/documentation/display/current/VM+Transport+Reference[VM transport].
...
* Should messages be secured? If so, use the link:/documentation/display/current/SSL+and+TLS+Transports+Reference[SSL transport].
...
* To create a TCP server that accepts incoming connections, declare an inbound tcp endpoint with a http://tcpconnector[tcp:connector]. This creates a TCP server socket that  reads requests from and optionally writes responses to client sockets.
* To poll from a TCP server, declare an inbound tcp endpoint with a http://tcppolling-connector[tcp:polling-connector]. This creates a TCP client socket that  reads requests from and optionally writes responses to the server socket.
* To write to a TCP server, create an outbound endpoint with a http://tcpconnector[tcp:connector]. This creates a TCP client socket that  writes requests to and optionally reads responses from a server socket.
...
]http://www.mulesoft.org/schema/mule/tcp/3.7/mule-tcp.xsd
...
To send messages on a TCP connection, create a simple TCP connector that outbound endpoints can use:
...
Similar to xml-protocol, the xml-eof-protocol element configures the XML protocol, but it  also uses socket closure to terminate a message (even if the XML is not well-formed).
