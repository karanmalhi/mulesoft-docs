...
Port 3300 is predefined by SAP, so if you need to validate other port numbers based on your SAP instance number, you can check the complete list of service-to-port mappings, which can be found link:/documentation/display/current/SAP+JCo+Server+Services+Configuration[here].
...
|*jcoClientExtendedProperties-ref* |A reference to `java.util.Map` containing additional JCo connection parameters. Additional information and a complete list of parameters can be found link:/documentation/display/current/SAP+JCo+Extended+Properties[here] |  |1.0
...
The MuleSoft Enterprise Java Connector for SAP supports both link:#MuleSoftEnterpriseJavaConnectorforSAPReference-InboundEndpoint[inbound] and link:#MuleSoftEnterpriseJavaConnectorforSAPReference-OutboundEndpoint[outbound] endpoints.

* link:#MuleSoftEnterpriseJavaConnectorforSAPReference-InboundEndpoint[Inbound-Endpoint]: Receives IDocs and BAPI calls over RFC.
** link:#MuleSoftEnterpriseJavaConnectorforSAPReference-ReceivingIDocs[Receiving IDocs]
** link:#MuleSoftEnterpriseJavaConnectorforSAPReference-ReceivingBAPIcalls[Receiving BAPI calls]
...
[NOTE]
You can use link:/documentation/display/current/Mule+Expression+Language+MEL[Mule expressions] inside the address attribute, just as you do for other Mule ESB transports.
...
The SAP transport includes link:#MuleSoftEnterpriseJavaConnectorforSAPReference-Transformers[transformers] that will convert the XML documents exchanged between the endpoints and SAP into corresponding SAP objects that the endpoints can handle.

The complete documentation for the different XML documents can be found link:/documentation/display/current/XML+Definitions[here]:

* link:/documentation/display/current/XML+Definitions#XMLDefinitions-JCoFunction[JCo Function] +
** link:/documentation/display/current/XML+Definitions#XMLDefinitions-XMLVersion1[Version 1]
** link:/documentation/display/current/XML+Definitions#XMLDefinitions-v2[Version 2]
* link:/documentation/display/current/XML+Definitions#XMLDefinitions-idoc[IDoc]
...
|*address* |The standard way to provide endpoint properties. For more information see link:#MuleSoftEnterpriseJavaConnectorforSAPReference-EndpointAddress[Endpoint Address]. |  |1.0
...
|*jcoClientExtendedProperties-ref* |A Reference to `java.util.Map`, which contains additional JCo connection parameters. Additional information and a complete list of parameters can be found link:/documentation/display/current/SAP+JCo+Extended+Properties[here]. |  |1.0
...
|*jcoServerExtendedProperties-ref* |A Reference to `java.util.Map`, which contains additional JCo Server connection parameters. Additional information and a complete list of server parameters can be found link:/documentation/display/current/SAP+JCo+Extended+Properties[here]. |  |1.0
...
The *payload* can be transformed into an link:#MuleSoftEnterpriseJavaConnectorforSAPReference-XMLDefinition[XML string] with the `<sap:object-to-xml/>` transformer.
...
After the flow executes and if the SAP inbound endpoint is request-response, then Mule will return values to the calling SAP instance. Return values can be represented with the XML representation of the link:/documentation/display/current/XML+Definitions[JCo Function]. +
...
|*name* |The reference name of the endpoint used internally by Mule configuration. |  |1.0
|*exchange-pattern* |The available options are `request-response` and `one-way`. |  |1.0
|*address* |The standard way to specify endpoint properties. For more information see link:#MuleSoftEnterpriseJavaConnectorforSAPReference-EndpointAddress[Endpoint Address]. |  |1.0
...
|*functionName* |When the type is *function*, this BAPI function is executed. If the type is *function-metadata* then you need to provide the name of the BAPI that you want to retrieve its metadata. This attribute accepts link:/documentation/display/current/Mule+Expression+Language+MEL[Mule Expressions]. |  |1.0
...
[WARNING]
*IMPORTANT*: The key to a fully working application in cluster is the implementation of link:/documentation/display/current/Reliability+Patterns[reliability patterns].
...
For more information, consult the link:/documentation/display/current/Transactions+Configuration+Reference[Transactions Configuration Reference].
...
* link:/documentation/display/current/Outbound+Endpoint+Transactions#OutboundEndpointTransactions-sRFCstateful[sRFC stateful]
* link:/documentation/display/current/Outbound+Endpoint+Transactions#OutboundEndpointTransactions-sRFCStatefulBAPITransaction[sRFC stateful BAPI transaction]
* link:/documentation/display/current/Outbound+Endpoint+Transactions#OutboundEndpointTransactions-tRFCStateful[tRFC stateful]
* link:/documentation/display/current/Outbound+Endpoint+Transactions#OutboundEndpointTransactions-qRFCStateful[qRFC stateful]

For examples, please take a look at link:/documentation/display/current/Outbound+Endpoint+Transactions[Outbound Endpoint Transactions].
...
* Read about using the link:/documentation/display/current/SAP+Connector[SAP connector in Anypoint Studio].
* Find out more about link:/documentation/display/current/SAP+JCo+Extended+Properties[SAP JCo Extended Properties], link:/documentation/display/current/SAP+JCo+Server+Services+Configuration[SAP JCo Server Services Configuration], or link:/documentation/display/current/Outbound+Endpoint+Transactions[Outbound Endpoint Transactions]
* Dig deeper into link:/documentation/display/current/XML+Definitions[XML Definitions].
* Access troubleshooting tips: +
** link:/documentation/display/current/Troubleshooting+SAP+Connector#TroubleshootingSAPConnector-CheckingLogFiles[Checking log files]
** link:/documentation/display/current/Troubleshooting+SAP+Connector#TroubleshootingSAPConnector-EnablingJCoTrace[Enabling JCo trace]
** link:/documentation/display/current/Troubleshooting+SAP+Connector#TroubleshootingSAPConnector-CommonErrors[Common Errors]
