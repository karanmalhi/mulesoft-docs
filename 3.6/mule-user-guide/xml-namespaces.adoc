...
You can specify the namespace on any XML-based functionality in Mule, including the JXPath filter, Jaxen filter, XPath filter, filter-based splitter, expression splitter, round-robin splitter, JXPath extractor transformer, and XPath expression transformer in the link:/documentation/display/current/XML+Module+Reference[XML Module Reference] and link:/documentation/display/current/XPath+Annotation[XPath Annotation].
