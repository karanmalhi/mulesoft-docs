...
In its most basic form, an endpoint acts as a flow-level inbound or outbound implementation of a link:/documentation/display/current/Connecting+Using+Transports[transport] and its underlying data source or message channel. For example:
...
An outbound endpoint can also be dynamic. This means that the endpoint's URI is the value of an link:/documentation/display/current/Mule+Expression+Language+MEL[expression], which is evaluated just before a message is sent to it. This allows the target of a message to be determined by its contents or the value of any message property. Dynamic endpoints can use either of the endpoint formats shown above.
...
link:/documentation/display/current/Using+Transformers[Transformers] can be configured on an endpoint encapsulating transformation logic in an endpoint that can then be reused as required.
...
The following reference tables list the attributes that can be configured for the link:/documentation/display/current/Generic+Connector[generic endpoint] in Mule. 
