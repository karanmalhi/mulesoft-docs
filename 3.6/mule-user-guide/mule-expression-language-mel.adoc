...
This document assumes that you are familiar with Mule ESB and its link:/documentation/display/current/Mule+Concepts[basic concepts]. Consider reading the chapter, link:/documentation/display/current/First+30+Minutes+with+Mule[First 30 Minutes with Mule] to gain familiarity before proceeding. 
...
* *NEXT STEP*: Learn the link:/documentation/display/current/Mule+Expression+Language+Basic+Syntax[basic syntax of MEL], or dive into link:/documentation/display/current/Mule+Application+Architecture[Mule Application Architecture].
* Learn better by example? Access our collection of link:/documentation/display/current/Mule+Expression+Language+Examples[example apps which use MEL].
* Learn better by tutorial? Access the link:/documentation/display/current/Mule+Message+Tutorial[Mule Message Tutorial].
