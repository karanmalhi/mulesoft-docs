...
Scatter-Gather replaces the http://www.mulesoft.org/documentation/display/current/Routing+Message+Processors#RoutingMessageProcessors-All[All] message processor, which is deprecated as of Mule 3.5.0. Note that, unlike All, Scatter-Gather executes routes concurrently instead of sequentially. Parallel execution of routes can greatly increase the efficiency of your application and provides more information than sequential processing. Refer to link:#Scatter-Gather-MigratingtoScatter-GatherfromtheAllMessageRouter[migration details] below for more information about the differences you can expect.

*link:#Scatter-Gather-thecode[Skip to the code]*
...
image:/documentation/download/attachments/122752235/scatter-gather.png?version=1&modificationDate=1414081453454[image]
...
image:/documentation/download/thumbnails/122752235/studio-scattergather.png?version=1&modificationDate=1414081453473[image]


=== General Tab

image:/documentation/download/attachments/122752235/scatterconf.png?version=1&modificationDate=1418756888339[image]
...
image:/documentation/download/attachments/122752235/sg-adv_tab.png?version=1&modificationDate=1418757324771[image]
...
|*Threading Profile* |Optionally, use to customize the threading profile. Refer to link:/documentation/display/current/Tuning+Performance[Tuning Performance] for a description of the configurable attributes. For a brief discussion of threading profiles in Scatter-Gather, see the link:#Scatter-Gather-threadprof[section below]. + |Default threading profile a|
|===
....
[tab,titel="XML Editor or Standalone"]
....
...
|============
|Optional Child Element |Description
|*threading-profile* |Allows you to configure the underlying thread pool. Refer to link:/documentation/display/current/Tuning+Performance[Tuning Performance] for a list of configurable attributes, all of which can be applied here. For a brief discussion of threading profiles in Scatter-Gather, see the link:#Scatter-Gather-threadprof[section below].
|============
....
------
...
For scenarios in which routes execute very quickly (a couple of milliseconds per route) it's probably better to do link:#Scatter-Gather-serial[sequential processing].
...
image:/documentation/download/attachments/122752235/scatter_gather2d.png?version=1&modificationDate=1414081453461[image]
...
An link:#Scatter-Gather-diffs[earlier section] of this page discusses some situations where sequential multicast is desirable over concurrent execution of all message routes. If you need sequential multicast in your application, you can implement it by configuring Scatter-Gather with a custom threading profile of only one thread, as shown below in the *Max Active Threads* field.

image:/documentation/download/attachments/122752235/sg-adv_tab2.png?version=1&modificationDate=1418757495836[image]

In the context of the above link:#Scatter-Gather-thecode[example], the XML would look as follows:
...
* Learn more about link:#[message routing].
