...
This document offers examples of applications which use the Database Connector. For full documentation of the connector, access the link:/documentation/display/current/Database+Connector[Database Connector] documentation and link:/documentation/display/current/Database+Connector+Reference[Database Connector Reference].

== Assumptions

This document presents two usage examples of the database connector. This document assumes you are familiar with the link:/documentation/display/current/Database+Connector[database connector] as well as with link:/documentation/display/current/DataSense[DataSense]; to understand the second example, the reader should also be familiar with the link:/documentation/display/33X/DataMapper+Transformer+Reference[DataMapper transformer] and Mule link:/documentation/display/current/Batch+Processing[batch processing].

=== Database Used in the Examples

The examples in this page were written for MySQL. They use one database with two tables. If you wish to create this database and populate it with the sample data used in these examples, you can download and run this link:#DatabaseConnectorExamples-script[MySQL script]. The script creates the database, database user and tables, and populates them with 102 rows of data.
...
image:/documentation/download/attachments/122752067/DBtestFlow.png?version=1&modificationDate=1421689563246[image]

In the Mule application, an inbound link:/documentation/display/current/HTTP+Connector[HTTP connector] listens for HTTP GET requests with the form: `http://<host>:8081/?lastname=<parameter>`. The HTTP connector passes the value of `<parameter>` as one of the message properties to a database connector. The database connector is configured to extract this value and use it for the SQL query listed below.
...
As you can see, the link:/documentation/display/current/Mule+Expression+Language+MEL[MEL] expression in the SQL query references the value of the parameter passed to the HTTP connector. So if the HTTP connector receives `http://localhost:8081/?lastname=Smith`, the SQL query is `select first_name from employees where last_name = Smith`.
...
|*Display Name* |`Database` .5+|image:/documentation/download/attachments/122752067/db-ex-1.png?version=1&modificationDate=1397160406901[image]
...
In this example, the database connector references the `MySQL_Configuration` link:/documentation/display/current/Global+Elements[global element] to obtain connection parameters. `MySQL_Configuration` is configured with the parameters listed below.
...
image:/documentation/download/attachments/122752067/global_db_connector_example.png?version=1&modificationDate=1399415134043[image]
...
image:/documentation/download/attachments/122752067/global_db_conn_Advanced.png?version=1&modificationDate=1399415216985[image]
...
image:/documentation/download/attachments/122752067/ex.1.v2.browser_results.png?version=1&modificationDate=1397160407105[image]
...
In this example, a database connector retrieves data from two tables. A link:/documentation/display/33X/DataMapper+Transformer+Reference[DataMapper transformer] maps this data to the CSV format. A link:/documentation/display/current/File+Connector[file connector] writes the resulting CSV file to disk, and a link:/documentation/display/33X/Logger+Component+Reference[logger component] logs processing details to the console.

This example uses link:/documentation/display/current/Batch+Processing[batch processing], which means that within this Mule app, messages are divided into records and processed asynchronously. Batch processing, in conjunction with *streaming* enabled on the database connector, can be useful to avoid system overload when dealing with large volumes of data.

image:/documentation/download/attachments/122752067/ex.2-batchjob.flow.png?version=1&modificationDate=1397160407419[image]

The database connector is inside a link:/documentation/display/current/Poll+Reference[poll scope] which, every ten minutes, requests the database connector to perform its configured operation on the database. The database connector performs a database query which returns 100 rows. Mule feeds this data into the DataMapper transformer inside the `Batch Commit` batch scope. DataMapper transforms the input into a CSV file; then, a link:/documentation/display/current/File+Connector[file connector] writes the resulting CSV file to disk. A link:/documentation/display/33X/Logger+Component+Reference[logger component] logs processed records to the console.

[TIP]
====
This example uses the same MySQL database as the previous example. If you want to recreate the database on your MySQL server, you can download or copy-paste the link:#DatabaseConnectorExamples-script[MySQL script] provided in this page. For details, see the see link:#DatabaseConnectorExamples-DatabaseConfiguration[Database Configuration] section above.
====
...
|===
|Parameter |Value |Properties Editor Image
|*Display Name* |`Database` .5+|image:/documentation/download/attachments/122752067/example_2_db_gentab.png?version=1&modificationDate=1397160409604[image]
|*Config Reference* |`MySQL_Configuration`
|*Operation* |`Select`
|*Query Type* |`Parameterized`
...
image:/documentation/download/attachments/122752067/ex2.datamapper.raw.png?version=1&modificationDate=1397160408187[image]
...
image:/documentation/download/attachments/122752067/ex2.datamapper.conn.select.png?version=1&modificationDate=1397160407727[image]
...
image:/documentation/download/attachments/122752067/ex2.datamapper.conn.select2.png?version=1&modificationDate=1397160407872[image]
...
image:/documentation/download/attachments/122752067/ex2.datamapper.all.selected.ops.png?version=1&modificationDate=1397160407576[image]
...
image:/documentation/download/attachments/122752067/ex2.datamapper.CSV.selected.png?version=1&modificationDate=1397160408024[image]
...
image:/documentation/download/attachments/122752067/ex2.Define.the.CSV.png?version=1&modificationDate=1397160408331[image]
...
image:/documentation/download/attachments/122752067/ex2.finished.datamapper.png?version=1&modificationDate=1397160408526[image]
...
* Learn more about link:/documentation/display/current/DataSense[DataSense] and how you can use it to retrieve metadata on remote applications.

* Delve deeper into link:/documentation/display/current/Batch+Processing[batch processing] to gain a deeper understanding of how Mule handles messages in batch jobs.
