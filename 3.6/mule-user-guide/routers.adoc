...
|link:#Routers-All[All] (Deprecated) |Broadcast a message to multiple targets
|link:#Routers-Async[Async] |Run a chain of message processors in a separate thread
|link:#Routers-Choice[Choice] |Evaluates a message against specified criteria, then sends it to the first message processor that matches those criteria.
|link:#Routers-CollectionAggregator[Collection Aggregator] |Checks the group tag (known as a Correlation ID) attached to each message in a group to create a collection of messages which share the same Correlation ID.
|link:#Routers-CollectionSplitter[Collection Splitter] |Accepts a collection of messages (or parts of messages), splits them into individual messages, then sends each new message, in sequence, to the next message processor in a flow.
|link:#Routers-CustomAggregator[Custom Aggregator] |Lets you write you own Java code to determine how messages are constructed and sent.
|link:#Routers-CustomProcessor[Custom Processor] |A custom-written message processor
|link:#Routers-FirstSuccessful[First Successful] |Sends a message to the next message processor within a "circular" list of processor targets.
|link:#Routers-IdempotentMessageFilter[Idempotent Message Filter] |Filter out duplicate message by message ID
|link:#Routers-IdempotentSecureHashMessageFilter[Idempotent Secure Hash Message Filter] |Filter out duplicate message by message content
|link:#Routers-MessageChunkAggregator[Message Chunk Aggregator] |Checks the group tag (Correlation ID) of each message in a collection, selects all the messages whose group tag matches the specified value, then combines those messages into a single message which is then sent to the next message processor in an application flow. This is particularly useful for re-assembling the segments of a long message that has been received as multiple messages, each one consisting of a segment of fixed length created and sent by the Message Chunk Splitter.
|link:#Routers-MessageChunkSplitter[Message Chunk Splitter] |Sections a message into segments of a specified length, then sends each segment, in sequence, to the next message processor in a flow. This is particularly useful when the message recipient cannot accept messages longer than a specified length.
|link:#Routers-MessageFilter[Message Filter] |Filter messages using a filter
|link:#Routers-ProcessorChain[Processor Chain] |Create a message chain from multiple targets
|link:#Routers-RecipientList[Recipient List] |Send a message to multiple connectors
|link:#Routers-RequestReply[Request Reply] |Receive a message for asynchronous processing and accept the asynchronous response on a different channel
|link:#Routers-Resequencer[Resequencer] |Accepts a collection of messages, then uses the Sequence ID of each message to reorder those messages. It then sends the messages (in order of their new sequence), to the next message processor in an application flow.
|link:#Routers-RoundRobin[Round Robin] |Iterates through a list of two or more message processors, sending successive messages to the next message processor on the list. When it reaches the end of the list, it jumps to the start of the list and resumes the iteration.
|link:/documentation/display/current/Until+Successful+Scope[Until Successful] |Repeatedly attempt to process a message until successful
|link:/documentation/display/current/Scatter-Gather[Scatter Gather] |Sends a request message to multiple targets concurrently. It collects the responses from all routes, and aggregates them into a single message.
|link:#Routers-Splitter[Splitter] |Evaluates an expression which determines how it sections a message into two or more parts. The Splitter then sends each of these message parts, in sequence, to the next message processor in an application flow.
|link:#Routers-WireTap[WireTap] |Send a message to an extra message processor as well as to the next message processor in the chain
|===
...
link:/documentation/display/current/Choice+Flow+Control+Reference[Read More]
...
*From 3.1.0* you can further customize the behavior of this router by specifying a _'failureExpression'_ that allows you to use link:/documentation/display/current/Non-MEL+Expressions+Configuration+Reference[Mule Expressions] to define a failure. The _failureExpression_ attribute is configured as follows:
...
After a splitter such as the link:#Routers-MessageChunkSplitter[Message Chunk Splitter] splits a message into parts, the message chunk aggregator router reassembles those parts back into a single message. The aggregator uses the message's correlation ID to identify which parts belong to the same message.
...
The Message Chunk Aggregator also accepts the `timeout` and `failOnTimeout` attributes as described under link:#Routers-CollectionAggregator[Collection Aggregator].
...
To put the chunked items back together again, you can use the link:#Routers-MessageChunkAggregator[Message Chunk Aggregator].
...
The Message Filter is used to control whether a message is processed by using a link:#[filter]. In addition to the filter, you can configure whether to throw an exception if the filter does not accept the message and an optional message processor to send unaccepted messages to.
...
A Processor Chain is a linear chain of message processors which process a message in order. A Processor Chain can be configured wherever a message processor appears in a Mule Schema. For example, to allow a link:#Routers-WireTap[Wire Tap] to transform the current message before sending it off, you can configure the following:
...
The Recipient List message processor allows you to send a message to multiple connectors by specifying an expression that, when evaluated, provides the list of connectors. These messages can optionally be given a correlation ID, as in the link:#Routers-CollectionSplitter[Collection Splitter]. An example is
...
The Resequencer sorts a set of received messages by their correlation sequence property and issues them in the correct order. It uses the `timeout` and `failOnTimeout` attributes described in link:#Routers-CollectionAggregator[Collection Aggregator] to determine when all the messages in the set have been received.
...
link:/documentation/display/current/Splitter+Flow+Control+Reference[Read More]
