...
The *Query Editor* in Studio allows you to easily query and retrieve complex data structures that you can use when designing your application. Using link:/documentation/display/current/DataSense[DataSense] on an Anypoint Connector which supports query functionality, the Query Editor can automatically acquire metadata from remote application, saving you the effort of manually discovering which data types and fields are available to query. The Query Editor displays the available data types and fields in a graphical menu – all you need to do is select the ones you want to query.
...
[TIP]
====
Using the Query Editor in conjunction DataSense's link:/documentation/display/current/Using+Perceptive+Flow+Design[perceptive flow design] greatly facilitates designing and building a SaaS integration flow. Perceptive flow design allows you to create complex mappings by simply placing an link:/documentation/display/current/Datamapper+User+Guide+and+Reference[Anypoint DataMapper transformer] before or after a connector in your flow.
====

== Assumptions

This document assumes that you are familiar with the link:/documentation/display/current/Anypoint+Studio+Essentials[Visual Editor in Anypoint Studio], as well as the http://www.mulesoft.org/documentation/display/current/Anypoint+Connectors[Anypoint Connectors], in particular the Salesforce cloud connector used in the example queries. This document also references link:/documentation/display/current/Anypoint+Connectors[Connectors], link:/documentation/display/current/Global+Elements[Global Elements], and link:/documentation/display/current/DataSense[DataSense]. To learn more about developing with Mule ESB's graphical user interface, review link:/documentation/display/current/Anypoint+Studio+Essentials[Getting Started with Anypoint Studio].

== Using the Query Editor

The table below lists the link:/documentation/display/current/Anypoint+Connectors[Anypoint Connectors] which support Query Editor functionality. (Additionally, you can use the Query Editor with any custom-built connector which supports the query functionality.)
...
Learn how to link:/documentation/display/current/Installing+Extensions[add connectors] to your instance of Anypoint Studio.

=== Accessing the Query Editor

. To access the Query Editor, add a connector to your flow, then access its link:/documentation/display/current/The+Properties+Editor[Properties Editor.]

. Set the value of the *Operations* field to **`Query`** (see image below).
+
image:/documentation/download/attachments/122752113/queryBuilder2.png?version=1&modificationDate=1380134390986[image]

. Use the drop-down menu to select a value for Language (refer to link:#DataSenseQueryEditor-ChoosingaQueryLanguage[Choosing a Query Language] below).

. Enter your query text directly in the *Query Text* field, or click the **Query Builder...** button to build your query using the graphical editor. Refer to the link:#DataSenseQueryEditor-ExploringtheQueryBuilder[Exploring the Query Builder] section for details. 
...
* *link:/documentation/display/current/DataSense+Query+Language[DataSense Query Language]*  allows you to perform queries using http://en.wikipedia.org/en/wiki/SQL[SQL] elements and syntax. Using the DataSense query language, you can use the Query Editor's graphical interface for almost all operations. 
...
|The Query Editor retrieves available object types and fields, then lists them in a menu for you to select and filter. You can build your query using a graphical user interface, shown below. |Within the connector in your flow, you need to reference a link:/documentation/display/current/Global+Elements[Global Element] with DataSense enabled. |You enter your query in the Query Editor's search box, using a simple SQL-like query language. |You must know the language and exact names of the data types and fields you wish to query.
2+|*Image* 2+|*Image*
2+|image:/documentation/download/attachments/122752113/DSLimage.png?version=1&modificationDate=1380322621370[image] 2+|image:/documentation/download/attachments/122752113/NQLimage.png?version=1&modificationDate=1380134627944[image]
|===
...
image:/documentation/download/attachments/122752113/query_builder.png?version=1&modificationDate=1380322647338[image]
...
. Create a Salesforce link:/documentation/display/current/Global+Elements[Global Element].
...
+
image:/documentation/download/attachments/122752113/enable_DS.png?version=1&modificationDate=1379519028147[image]
...
+
image:/documentation/download/attachments/122752113/example_reference.png?version=1&modificationDate=1399316952561[image]

=== Accessing the Query Editor and Building a Query

. In the *Properties Editor* of the Salesforce connector, use the drop-down to select the *Language* you wish to use. In the context of this example, keep the default value, *DataSense Query Language*. (The link:#DataSenseQueryEditor-UsingtheNativeQueryLanguage[section below] describes an example of a query built using Salesforce's native query language.)
...
. Click *Add Filter* to create a query filter. You can create multiple filters to operate on available fields, and configure sorting, offset and limits for query results. Refer to link:#DataSenseQueryEditor-ExploringtheQueryBuilder[Exploring the Query Builder] section above for more detail.
...
image:/documentation/download/attachments/122752113/query_language.png?version=1&modificationDate=1379522259384[image]
...
+
image:/documentation/download/attachments/122752113/basic_advanced.png?version=1&modificationDate=1380323126939[image]
====
...
* Learn more about the Query Editor's link:/documentation/display/current/DataSense+Query+Language[DataSense Query Language]. 
