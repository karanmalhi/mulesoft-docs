...
:keywords: mmc, sql, server, microsfot
...
This will uncompress the file, whose filename extension  changes from `.tar.gz` to `.tar`. Unpack the file with the `tar` command, as shown below.
