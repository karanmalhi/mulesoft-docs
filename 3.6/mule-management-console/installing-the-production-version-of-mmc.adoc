...
Note that if you have downloaded *MuleSoft's bundled runtime download* – ESB with Mule Management Console (MMC) – it is only for use as an evaluation tool and not for production. For installation instructions for the trial version, See link:/docs/display/current/Installing+the+Trial+Version+of+MMC[Installing the Trial Version of MMC].
====

== Assumptions

This document assumes that you are have the current version of Mule Enterprise already installed. Download it from the http://www.mulesoft.com/support-login[customer portal] if you need to upgrade to this version. If you are using an older version of Mule Enterprise,  link:/docs/display/current/Installing+the+Management+Console[switch] to that version of this documentation.

This document assumes that your enterprise license is current. Please see link:/docs/display/current/Installing+an+Enterprise+License[Installing an Enterprise License] for information on obtaining and installing an enterprise license. 

This document also assumes that you have a Web application server such as Tcat, Tomcat, or JBoss installed. Refer to the link:/docs/display/current/Hardware+and+Software+Requirements[Hardware and Software Requirements] for the latest information about hardware requirements and supported application servers, Java Runtime Environments, and browsers.
...
. Rename your .war file to `mmc.war`. The name of the WAR file varies depending on the version that you download, following the format `<console_name>-<version>.war`. After you download this file from the customer portal, change the name to `mmc.war.`
+
...
+
. If your link:/docs/display/current/Deployment+Scenarios[Mule ESB instance is packaged as a Web app] (not recommended), please contact support for special instructions. 
...
The detailed, step-by-step instructions for installing and deploying MMC to WebSphere are provided on link:/docs/display/current/Installing+and+Deploying+MMC+to+WebSphere[Installing and Deploying MMC to WebSphere]. 
...
* MMC version 3.4.2 and newer: See "Persisting MMC Data On External Databases in MMC 3.4.2".
...
For an example, see link:/docs/display/current/Installing+and+Deploying+MMC+to+WebSphere[Installing and Deploying MMC to WebSphere]. 
...
image:/docs/download/attachments/122750977/MMC_login.png?version=1&modificationDate=1421450369449[image]

== See Also

* Get familiar with the link:/docs/display/current/Orientation+to+the+Console[MMC console].
* Learn the basics of using MMC with the link:/docs/display/current/MMC+Walkthrough[MMC Walkthrough].
* Learn more about how to link:/docs/display/current/Setting+Up+MMC[set up MMC] to meet your needs.
* Access the link:/docs/display/current/Troubleshooting+with+MMC[troubleshooting] guide.
