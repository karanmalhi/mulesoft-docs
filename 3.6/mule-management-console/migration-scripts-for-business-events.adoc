...
+
This fires up your OS's default Web browser with the Web client for H2, which displays the below configuration screen.
+
...
. For JDBC URL, enter `jdbc:h2:file:///<path_to_mmc-data>/tracking/persistency`, replacing `<path_to_mmc-data>` with the actual path to the directory containing your MMC tracking database. For example:
+
[source,bash]
----
jdbc:h2:file:////var/lib/tomcat6/webapps/persistency
----
+
*Note*: The path ends in `persistency`, with no filename extension.
+
. Leave *User Name* and *Password* blank.
. Click *Connect*. The H2 Web client should display this window:
+
...
+
. H2 runs the SQL statement, then prints a success message: +
+
