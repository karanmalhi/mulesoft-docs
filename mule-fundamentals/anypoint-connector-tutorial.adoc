...
image:anypoint-connector-tutorial-1.png[image]

After learning some of the basic functionality in the link:/mule-fundamentals/basic-studio-tutorial[Basic] and link:/mule-fundamentals/content-based-routing-tutorial[Content-Based Routing Tutorials], it's time to try your hand at developing a more complex application which makes use of **Anypoint Connectors**.
...
+
 +
 image:anypoint-connector-tutorial-2.png[image]
...
However, if you wanted to expose such an application publicly, you would need to configure your application to link:/mule-user-guide/using-a-connector-to-access-an-oauth-api[authenticate via OAuth].
====
+
 +
...
. Access token secret +
 +
 image:anypoint-connector-tutorial-5.png[image]

 +

== Modeling a Flow with a Connector

Launch Anypoint Studio and create a new Mule Project named `Connector Tutorial`. If you need more directions on launching Studio and creating a project, please refer to the link:/mule-fundamentals/basic-studio-tutorial[Basic Studio Tutorial]. 
...
|*link:/mule-user-guide/foreach[Foreach Scope]* |Processes each item in a POJO iteratively. Find this in the Scopes section of the palette.
...
image:anypoint-connector-tutorial-6.png[image]
...
Once you configure the individual elements within it, which is in the next section, this flow accomplishes the goals that you set out to achieve with this application. Each building block that you select and place on the canvas performs part of the functionality of your application, as shown in the image below.
 ...
 image:anypoint-connector-tutorial-7.png[image]
...
. Click the HTTP building block in your flow to open its link:/mule-user-guide/the-properties-editor[Properties Editor].  For this element to work, you must reference an abstract element called a *Connector Configuration*, which contains several of the high level necessary configuration properties. One single Connector Configuration element can be referenced by as many HTTP connectors as you like. Create a new Connector Configuration element by clicking the green plus sign next to the Connector Configuration field. +
 +
 image:anypoint-connector-tutorial-8.png[image]
...
. A new window opens up. Fill in the two required fields: *Host* and *Port*. Set `localhost` as the host and leave the default value `8081` as the port. +
 image:anypoint-connector-tutorial-9.png[image]  +
...
image:anypoint-connector-tutorial-10.png[image]
[cols=",",options="header",]
|===
|Field |Value
|*Path* |`gettweets`
...
image:anypoint-connector-tutorial-11.png[image]
...
Read more about link:/mule-fundamentals/global-elements[Global Elements in Mule].

. Click the plus sign next to the *Connector Configuration* field. +
  +
 image:anypoint-connector-tutorial-12.png[image] +

  +
. Enter values for the fields according to the table below. +
 +
 image:anypoint-connector-tutorial-13.png[image] +
...
image:anypoint-connector-tutorial-14.png[image]
...
image:anypoint-connector-tutorial-15.png[image]
...
. Mule immediately kicks into gear, starting your application and letting it run. When the startup process is complete, Studio displays a message in the console that reads,  `Started app 'connector_tutorial'` . +
 +
 image:anypoint-connector-tutorial-16.png[image] +
...
image:anypoint-connector-tutorial-17.png[image] +
 +
...
image:anypoint-connector-tutorial-18.png[image]
...
==== ~image:anypoint-connector-tutorial-19.png[image] ~ Hints
...
There are a few ways to accomplish this task, one of which is to add the extra text to the tweet inside a new variable. However, you can take a shortcut using the link:/mule-user-guide/set-payload-transformer-reference[set payload] transformer. Replace the content of the entire payload using an expression composed of multiple parts, such as the following the following:

image:anypoint-connector-tutorial-20.png[image]
...
image:anypoint-connector-tutorial-21.png[image]

==== image:anypoint-connector-tutorial-22.png[image] Answer
...
image:anypoint-connector-tutorial-23.png[image]
...
* *NEXT STEP:* Try the link:/mule-fundamentals/mule-message-tutorial[Mule Message Tutorial].
* See http://www.mulesoft.org/connectors[specific documentation for each connector].
* Import link:/mule-user-guide/installing-connectors[additional connectors] into your instance of Anypoint Studio.
* Learn how to create your own Anypoint Connectors using the link:/anypoint-connector-devkit/[Anypoint Connector DevKit].
