...
image:flows-and-subflows-1.png[image]

image:flows-and-subflows-2.png[image]
...
* In clusters of Mule servers, messages can migrate between nodes when sent to an asynchronous flow. This allows for *load balancing between nodes* and higher performance of application. (See our link:/mule-fundamentals/flow-architecture-advanced-use-case[advanced use case] for an example.)
...
All three types of flows are identified by name and, thus, can be called as often as needed via link:/mule-user-guide/flow-reference-component-reference[flow reference components] in other flows. For example, if an asynchronous Flow B's job is to update a database with logged data, any number of other flows can feed data to be logged into Flow B to update the same database (see imageflows-and-subflows-3.png[Triggering Flows] below.

image:flows-and-subflows-3.png[image]
...
image:flows-and-subflows-4.png[image]
...
image:flows-and-subflows-5.png[image]
...
image:flows-and-subflows-6.png[image]
...
Read more about setting up link:/mule-user-guide/transactional[Transactional units] in Mule applications.
====
...
image:flows-and-subflows-7.png[image]
...
|Asynchronous Flow |Flow Reference wrapped within an link:/mule-user-guide/async-scope-reference[Async Scope] |asynchronous |not inherited
|===

== See Also

* *NEXT STEP:* Learn the various ways you can link:/mule-fundamentals/deploying-mule-applications[deploy Mule applications].
* Examine an link:/mule-fundamentals/flow-architecture-advanced-use-case[advanced use case] showing a more complex flow architecture that uses several child flows.
* Read about some alternative ways to control message processing within a flow using link:/mule-user-guide/routers[routing message processors].
* Refer to the link:/mule-user-guide/flow-reference-component-reference[Flow Reference Component Reference] and link:/mule-user-guide/async-scope-reference[Async Scope].
* Read more about link:/mule-user-guide/flow-processing-strategies[Flow Processing Strategies].
* Read more about setting up link:/mule-user-guide/transactional[transactional units] in Mule applications.
