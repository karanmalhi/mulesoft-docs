...
* In clusters of Mule servers, messages can migrate between nodes when sent to an asynchronous flow. This allows for *load balancing between nodes* and higher performance of application. (See our link:/mule-fundamentals/v/3.8-m1/flow-architecture-advanced-use-case[advanced use case] for an example.)
...
|Asynchronous Flow |Flow Reference wrapped within an link:/mule-user-guide/v/3.8-m1/async-scope-reference[Async Scope] |asynchronous |not inherited
|===
...
* Examine an link:/mule-fundamentals/v/3.8-m1/flow-architecture-advanced-use-case[advanced use case] showing a more complex flow architecture that uses several child flows.
