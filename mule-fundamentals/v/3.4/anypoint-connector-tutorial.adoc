...
This tutorial assumes that you have link:/mule-fundamentals/v/3.4/download-and-launch-mule-studio[downloaded and installed Mule Studio]. If you do not have any previous experience with Eclipse or an Eclipse-based IDE, please review the brief introduction to the link:/docs/display/34X/Mule+Studio+Essentials[Mule Studio interface] and the document describing link:/mule-fundamentals/v/3.4/global-elements[Global Elements in Mule]. Further, consider following the link:/mule-fundamentals/v/3.4/basic-studio-tutorial[Basic] and link:/mule-fundamentals/v/3.4/content-based-routing-tutorial[Content-based Routing] tutorials.
