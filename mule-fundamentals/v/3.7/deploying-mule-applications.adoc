...
image:/docs/download/attachments/122751613/deploy_embedded.png?version=1&modificationDate=1386791243110[image]
...
image:/docs/download/attachments/122751613/terminate_button2.png?version=2&modificationDate=1418418294409[image]

== Deploy on an Enterprise Server

Export the Studio application to a zip file, then deploy it on an** Enterprise Server**. This approach is ideal for Mule servers in production environments. The zip file contains all the resources (such as code libraries and application metadata) necessary to run the application, and import it into Studio again for further development. You can use the Mule ESB as an enterprise server as described in link:/docs/display/current/Downloading+and+Starting+Mule+ESB[Downloading and Starting Mule ESB].

image:/docs/download/attachments/122751613/deploy_server.png?version=1&modificationDate=1386791274297[image]
...
image:/docs/download/attachments/122751613/StudioExport-1.png?version=1&modificationDate=1399054586232[image]
...
image:/docs/download/attachments/122751613/deploy_mmc.png?version=1&modificationDate=1386791392947[image]

Once in the repository, the application can be deployed from the Repository to multiple Mule server instances and even to multi-node high-availability clusters. To publish an application to the management console’s app repo, you must first download, then install and set-up Mule ESB Standalone with Mule Management Console. Access the management console’s link:/docs/display/current/MMC+Walkthrough[Walkthrough] to set up a console environment, then complete the following steps to publish your Studio application to its application repository.
...
. Follow the directions in the management console link:/docs/display/current/MMC+Walkthrough[Walkthrough] to deploy your application to the Mule server (or cluster of servers).
...
image:/docs/download/attachments/122751613/deploy_cloudhub.png?version=2&modificationDate=1420666789517[image]
...
image:/docs/download/attachments/122751613/deploytoch.png?version=1&modificationDate=1399060724339[image]
...
image:/docs/download/attachments/122751613/Chconsoledeploying.png?version=2&modificationDate=1420667355892[image]

== Sharing Resources

If you're deploying multiple applications to the same place, in whichever of the ways explained above, and those applications could share the same resources, then you can create a common *domain* where you can define common configurations that can then be referenced by multiple projects. This allows you to, for example, expose different services in different projects through the same HTTP host and port and be able to deploy everything without any conflicts. link:/docs/display/current/Shared+Resources[Read More].
...
* Learn more about link:/docs/display/current/CloudHub[CloudHub].
* Learn how to deploy multiple applications that link:/docs/display/current/Shared+Resources[Shared Resources] through any of these methods
