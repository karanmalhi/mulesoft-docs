...
. Leave checkbox, â€˜*Enable server validation field*â€™ unchecked.
...
+
. Enter _/demo/supplier-inbox/_ as *Path* and _*.dat_ in *Filename*. Note that weâ€™re leaving the B2B options unset. The polling setting can be safely ignored by now.
+
