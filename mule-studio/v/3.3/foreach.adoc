...
Typically, a flow that processes messages containing a collection of elements must use one flow control to split the collection into individual elements, which it processes iteratively, then use another flow control to re-aggregate the elements into a new collection so they can be passed out of the flow. Several of Mule ESB’s link:/docs/display/33X/Studio+Flow+Controls[flow controls] support such splitting-and-re-aggregation, but even when carefully chosen for a specific task, they cannot match the convenience of the *Foreach Scope*.
