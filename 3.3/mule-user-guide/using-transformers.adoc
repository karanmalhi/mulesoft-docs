...
The link:#UsingTransformers-ref[Available Transformers] section of this page describes all the standard transformers provided with Mule. Additionally, many link:/docs/display/33X/Transports+Reference[transports] and link:/docs/display/33X/Modules+Reference[modules] have their own transformers, such as the ObjectToJMSMessage transformer for the JMS transport.
...
package. They do not require any special configuration. For details on these transformers, see link:/docs/display/33X/Transformers+Configuration+Reference[Transformers Configuration Reference].
...
|ExpressionTransformer |Evaluates one or more expressions on the current message and return the results as an Array. For details, see link:/docs/display/33X/Mule+Expression+Language+MEL[Mule Expression Language MEL].
...
package. They provide the ability to transform between different XML formats, use XSLT, and convert to POJOs from XML. For information, see link:/docs/display/33X/XML+Module+Reference[XML Module Reference].

[width="100%",cols="50%,50%",options="header",]
|===
|Transformer |Description
|link:/docs/display/33X/XmlObject+Transformers[XmlToObject <-> ObjectToXml] |Converts XML to a Java object and back again using http://xstream.codehaus.org[XStream].
|link:/docs/display/33X/JAXB+Transformers[JAXB XmlToObject <-> JAXB ObjectToXml] |Converts XML to a Java object and back again using the http://java.sun.com/developer/technicalArticles/WebServices/jaxb/[JAXB] binding framework (ships with JDK6)
|link:/docs/display/33X/XSLT+Transformer[XSLT] |Transforms XML payloads using XSLT.
|link:/docs/display/33X/XQuery+Transformer[XQuery] |Transforms XML payloads using http://en.wikipedia.org/wiki/XQuery[XQuery].
|link:/docs/display/33X/DomToXml+Transformer[DomToXml <-> XmlToDom] |Converts DOM objects to XML and back again.
|link:/docs/display/33X/XmlToXMLStreamReader+Transformer[XmlToXMLStreamReader] |Converts XML from a message payload to a StAX XMLStreamReader.
|link:/docs/display/33X/XPath+Extractor+Transformer[XPath Extractor] |Queries and extracts object graphs using XPath expressions using JAXP.
|link:/docs/display/33X/JXPath+Extractor+Transformer[JXPath Extractor] |Queries and extracts object graphs using XPath expressions using JXPath.
|link:/docs/display/33X/XmlPrettyPrinter+Transformer[XmlPrettyPrinter] |Allows you to output the XML with controlled formatting, including trimming white space and specifying the indent.
|===
...
package. They provide the ability to work with JSON documents and bind them automatically to Java objects. For information, see link:/docs/display/33X/Native+Support+for+JSON[Native Support for JSON].

=== Scripting

The link:/docs/display/33X/Scripting+Module+Reference[Scripting] transformer transforms objects using scripting, such as JavaScript or http://groovy.codehaus.org[Groovy]scripts. This transformer is in the
...
|===
|Transformer |Description
|link:/docs/display/33X/Transformers+Configuration+Reference#TransformersConfigurationReference-encrypt[Encryption <-> Decryption] |A pair of transformers that use a configured EncryptionStrategy implementation to encrypt and decrypt data.
|===
...
|===
|Transformer |Description
|link:/docs/display/33X/Transformers+Configuration+Reference#TransformersConfigurationReference-gzip[GZipCompressTransformer <-> GZipUncompressTransformer] |A pair of transformers that compress and uncompress data.
|===
...
|===
|Transformer |Description
|link:/docs/display/33X/Transformers+Configuration+Reference#TransformersConfigurationReference-base64[Base64Encoder <-> Base64Decoder] |A pair of transformers that convert to and from Base 64 encoding.
|link:/docs/display/33X/Transformers+Configuration+Reference#TransformersConfigurationReference-xml[XMLEntityEncoder <-> XMLEntityDecoder] |A pair of transformers that convert to and from XML entity encoding.
|===

=== Email

The Email transport provides several transformers for converting from email to string, object to MIME, and more. For details, see link:/docs/display/33X/Email+Transport+Reference[Email Transport Reference].

=== File

The File transport provides transformers for converting from a file to a byte array (byte[]) or a string. For details, see link:/docs/display/33X/File+Transport+Reference[File Transport Reference].

=== HTTP

The HTTP transport provides several transformers for converting an HTTP response to a Mule message or string, and for converting a message to an HTTP request or response. For details, see link:/docs/display/33X/HTTP+Transport+Reference[HTTP Transport Reference]. Additionally, the Servlet transport provides transformers that convert from HTTP requests to parameter maps, input streams, and byte arrays. For details, see link:/docs/display/33X/Servlet+Transport+Reference[Servlet Transport Reference].

=== JDBC

*_Enterprise Edition_* +
The Mule Enterprise version of the JDBC transport provides transformers for moving CSV and XML data from files to databases and back. For details, see link:/docs/display/33X/JDBC+Transport+Reference[JDBC Transport Reference].

=== JMS

The link:/docs/display/33X/JMS+Transport+Reference[JMS Transport Reference] and link:/docs/display/33X/Mule+WMQ+Transport+Reference[Mule WMQ Transport Reference] (enterprise only) both provide transformers for converting between JMS messages and several different data types.

=== Strings and Byte Arrays

The link:/docs/display/33X/Multicast+Transport+Reference[Multicast Transport Reference] and link:/docs/display/33X/TCP+Transport+Reference[TCP Transport Reference] both provide transformers that convert between byte arrays and strings.

=== XMPP

The XMPP transport provides transformers for converting between XMPP packets and strings. For details, see link:/docs/display/33X/XMPP+Transport+Reference[XMPP Transport Reference].

=== Custom

Mule supports the ability to build link:/docs/display/33X/Creating+Custom+Transformers[Custom Transformer]. Build custom transformers to meet specific data conversion needs in your application.
