...
. Review link:#[Contributing to Mule Source Code Using an IDE] and link:#[Contributing to Mule Source Code Using Maven] to learn more about how to work in your newly set up development environment.
. Review the link:#[Mule Coding Conventions] documentation to ensure you adhere to source code standards, thus increasing the likelihood that your changes will be merged with the mule-3.x (i.e. master) source code.
...
* Rather than adjusting source code, you may wish to extend Mule by creating custom components. Refer to link:/docs/display/current/Extending[Extending] for more details.
* Beyond creating custom components, you can extend Mule even further using our link:/docs/display/33X/Mule+DevKit[Mule DevKit].
