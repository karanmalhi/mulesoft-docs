...
MMC version 3.4.1 for MS-SQL Server allows you to link:/mule-management-console/persisting-mmc-data-to-ms-sql-server[persist MMC data to MS SQL Server] instead of on MMC's default internal databases. Users of this version of MMC should upgrade to MMC 3.6.0, which includes support for MS-SQL Server.
...
[NOTE]
Before running MMC 3.6.0, you will also need to perform the steps described in link:/mule-management-console/upgrading-to-mmc-3.6.0-with-an-external-database[Upgrading to MMC 3.6.0 with an External Database]. Additionally, if your current MMC installation uses LDAP for user authentication, you must complete the steps described in link:/mule-management-console/upgrading-to-mmc-3.6.0-with-ldap[Upgrading to MMC 3.6.0 with LDAP].
...
* Read the configuration details for link:/mule-management-console/persisting-mmc-data-to-ms-sql-server[Persisting MMC Data to MS SQL Server].
