...
. At this point, Mule ESB and MMC are up and running. To log in to MMC, open a Web browser and go to the URL indicated in the last line of the startup messages. In the example shown above, this would be `http://localhost:8585/mmc-3.7.0`.
...
[CAUTION]
Do not move the startup, shutdown, and status scripts from their original location. They contain references to relative paths and won't work if you move them.
