...
[NOTE]
If your current installation of MMC link:/documentation/display/current/Setting+Up+and+Managing+Users+via+LDAP[uses LDAP for managing users], you will also need to migrate your LDAP configuration. See link:/documentation/display/current/Upgrading+to+MMC+3.6.0+with+LDAP[Upgrading to MMC 3.6.0 with LDAP] for details.
...
* Read an overview of link:/documentation/display/current/Configuring+MMC+for+External+Databases+-+Quick+Reference[configuring MMC for external databases], which includes links to detailed instructions for each supported database server.
* Learn how to link:/documentation/display/current/Upgrading+to+MMC+3.6.0+with+LDAP[upgrade to MMC 3.5.0 with LDAP].
