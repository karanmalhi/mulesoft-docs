...
If you are programming in Java, it is recommended that you use the link:https://wikis.oracle.com/display/Jersey/Main[Jersey client]. Or, for JSON support, use the link:http://hc.apache.org/httpclient-3.x/index.html[HttpClient] with link:http://jackson.codehaus.org/[Jackson], the high-performance JSON processor.
