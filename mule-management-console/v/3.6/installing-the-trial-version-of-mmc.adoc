...
MuleSoft offers the link:https://www.mulesoft.com/dl/mmc[trial version of the Mule Management Console] for evaluation. The trial version is limited in some functionality and is not appropriate for production deployments. For information about installing the production version of MMC, refer instead to link:/mule-management-console/v/3.6/installing-the-production-version-of-mmc[Installing the Production Version of MMC].

== Prerequisites

This document assumes that you have downloaded the current version of MuleSoft's link:https://www.mulesoft.com/dl/mmc[MMC download] – ESB with Mule Management Console (MMC). (For information about installing the production version of MMC, refer instead to link:/mule-management-console/v/3.6/installing-the-production-version-of-mmc[Installing the Production Version of MMC].)

== Downloading the Trial Version of MMC

. Go to link:https://www.mulesoft.com/dl/mmc[MMC Download], and click *Download*.
...
+
[WARNING]
Always run the scripts from the directory they reside on. The scripts contain relative paths and do not work properly if you run them from another directory.
+
[TIP]
...
+
. If on Unix, the startup script prompts you for the listening port for Mule ESB. Enter the desired port, or press Enter to accept the default, 7777. On Windows, the startup script does not prompt you for the Mule port, since it always uses the default, 7777.
. The startup script launches an embedded Tomcat server (listening on port 8585 by default) and deploy MMC to it. Then, it will start Mule ESB. During startup, your terminal console fills with startup messages. After startup is complete, the script finishes and you  regain your terminal prompt. The last few messages indicate successful startup.
+
[source, code, linenums]
----
$ ./startup.sh
...
----
+
. At this point, Mule ESB and MMC are up and running. To log in to MMC, open a Web browser and go to the URL indicated in the last line of the startup messages. In the example above, this would be `http://localhost:8585/mmc-3.5.0`.
...
+
[CAUTION]
Do not move the startup, shutdown, and status scripts from their original location. They contain references to relative paths and do not work if you move them.
