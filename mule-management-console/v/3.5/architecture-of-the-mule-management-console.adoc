...
|*6* |MMC's REST interface, which exposes MMC functionality through a REST API. MMC users can invoke this API to programmatically Mule instances paired with MMC. For more details about MMC's REST API, see *this link is missing link:/mule-management-console/v/3.7/setting-up-mmc[MMC REST API].
