...
This document assumes that you are familiar with the basic link:/docs/display/34X/Architecture+of+the+Mule+Management+Console[Architecture of the Mule Management Console]. It also assumes that you have installed MMC version 3.4.2. (Download it from the http://www.mulesoft.com/support-login[customer portal] if you need to upgrade to this version.)
...
For more information on starting Mule with custom parameters, see link:/docs/display/34X/Starting+and+Stopping+Mule+ESB[Starting and Stopping Mule ESB].
====
...
The `databases` directory contains one configuration file per supported database engine (see link:#EncryptingLDAPandDatabasePasswordsinMMC3.4.2-supported_dbs[note] below). There are two types of configuration files:
...
* Learn how to link:/docs/display/34X/Encrypting+MMC+User+Passwords[encrypt MMC User Passwords]
* Learn about link:/docs/display/34X/Persisting+MMC+Data+On+External+Databases[persisting MMC Data On External Databases]
* Learn how to link:/docs/display/34X/Setting+Up+and+Managing+Users+via+LDAP[enable LDAP Authentication] for MMC
