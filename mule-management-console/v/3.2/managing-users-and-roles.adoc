...
You can also use LDAP to enable authentication (see link:/mule-management-console/v/3.2/enabling-authentication-through-ldap[Enabling Authentication Through LDAP]). Note that after enabling authentication through LDAP, the Users link is no longer available.

See link:/mule-management-console/v/3.2/automating-tasks-using-scripts[Automating Tasks Using Scripts] for an explanation of the Utility options (Admin Shell and Scheduler).
...
link:/mule-management-console/v/3.2/setting-up-alert-destinations-and-notifications[<< Previous: *Setting Up Alert Destinations and Notifications*]
