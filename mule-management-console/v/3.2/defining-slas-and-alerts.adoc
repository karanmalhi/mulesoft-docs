...
A field for which data is required is marked with an exclamation point image:warning.png[warning] and highlighted in red if left blank. (The figure below has a field marked as required.) For more information on these fields, see link:#DefiningSLAsandAlerts-fields[mmc:below].
