= Installing Anypoint Runtime Manager On Premise
...
This section details connection parameters and recommended settings for connecting to your ARM on-premise installation. You can complete the steps in this section before or after installing ARM, or you can use your own custom network configuration.
...
* Obtain the the installation ZIP file for Anypoint Docker Setup from MuleSoft
...
== Adding a Server to Your ARM On-Prem

[[download_agent]]
=== Downloading Mule Agent for ARM On-Prem
...
In your ARM on-premise installation, click *Applications* in the navigation bar and select your environment. Then, select *Servers* in the left-hand menu.
...
For example, the command listed below adds server `testsrv1` to the ARM on-premise residing on `anypoint.mulesoft.local`:
...
=== Starting and Stopping ARM On-Prem
...
The output below shows the listing for a full running ARM on-premise installation (only the first five columns are displayed).
