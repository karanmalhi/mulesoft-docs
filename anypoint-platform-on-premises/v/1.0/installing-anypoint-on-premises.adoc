...
*OS Requirements*: We tested with RedHat, CentOS and Ubuntu, but practically any OS capable of running docker should be suficient.

*Server vs Agent Hosts*: You can run the Rancher Server, Rancher Agent, and all MuleSoft images on the same machine.  We recommend installing the Rancher Server on one host and then the Rancher Agent and MuleSoft images on other hosts.
