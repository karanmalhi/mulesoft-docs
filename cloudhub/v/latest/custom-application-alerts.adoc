...
+
[IMPORTANT]
The *Inherited Token Authentication* option only works when your CloudHub connector's application is run inside a CloudHub instance. With this approach the connector retrieves the API Token from the CloudHub instance where the app is deployed.
+
