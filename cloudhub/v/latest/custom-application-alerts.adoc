...
+
. Set the properties for "Domain", "Message" and "Priority" making sure that you have selected the *Create notification* operation:
+
image:cloudhub-connector-settings.png[cloudhub-connector-settings]
+
. Additional information can be included with the notification by configuring the "Custom Properties" section. The expected format is `Map<String, String>`.
//todo: what can you do with custom properties? How should the "From Expression" be expressed Map<customKey,customMessage>?

When your domain root is visited, the *Create notification* operation in the CloudHub connector creates a "Hello World!" notification message for the domain that the application is currently running on.
