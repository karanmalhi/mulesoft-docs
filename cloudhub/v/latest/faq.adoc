...
Yes! You can assign a static IP to your application in the *Static IPs* tab on the application settings. For details, see the *Static IPs Tab* section in link:/cloudhub/deploying-a-cloudhub-application[Deploying a CloudHub Application]. By default, you are allocated a number of static IPs equal to 2x the number of Production vCores in your subscription. To raise this limit, please contact MuleSoft Support. 
