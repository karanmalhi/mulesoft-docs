...
CloudHub provides an implementation of the user object store. It places limits on the usage of this to avoid abuse. These are detailed on the link:http://www.mulesoft.org/documentation/display/current/Managing+Application+Data+with+Object+Stores[Object Store] page.
