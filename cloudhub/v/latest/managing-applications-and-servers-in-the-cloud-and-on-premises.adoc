...
The `amc_setup` script actually invokes the link:/mule-agent/v/1.2.0/[Mule agent] installation script, which has several useful parameters for configuring security and proxies. For details on the options, see *Installation Options* in link:/mule-agent/v/1.2.0/installing-mule-agent[Installing Mule Agent].
