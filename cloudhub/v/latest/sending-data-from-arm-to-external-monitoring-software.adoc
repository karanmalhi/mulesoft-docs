...
In between ARM and the external services is the link:/mule-agent/v/1.3.0/[Mule Agent], a module of Mule ESB that enables easy integration and sends event notifications to third party monitoring tools.
...
When using ARM as an on premises application, launch the agent with the following arguments:
