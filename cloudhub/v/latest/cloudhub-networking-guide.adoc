...
*Important*: The CloudHub load balancer proxies port :80 to :8081 on the Mule worker. The `http.port` and `https.port` values must be set to port 8081 for HTTP and port 8082 for HTTPS, and cannot be set to any other port number.
