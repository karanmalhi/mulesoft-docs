...
|`flushFrequency` |Frequency at which the flush method  runs, in milliseconds. | 
|`filePath` |Path to the buffer file. Only valid for buffers of type `DISK`. |`PATH/bufferFile`
|`whenExhausted` |Strategy to follow when the buffer is exhausted. If not specified, the buffer  grows automatically. Valid values are `FAIL` and `FLUSH.` |N/A
...
|`discardMessagesOnFlushFailure` |If true, messages are not returned to the buffer if the flush is not successful. |`false`
|===
