...
You can use an integrated development environment (IDE) such as Anypoint Studio (which is based on Eclipse), Eclipse, and IntelliJ to rapidly develop Mule applications. For more information on Anypoint Studio, see link:/mule-fundamentals/anypoint-studio-essentials[Anypoint Studio Essentials].
...
Want more options for testing your Mule applications? Try link:/mule-user-guide/munit[*MUnit*], a Beta-version Mule testing framework. Although MUnit is not yet supported by MuleSoft, you can access developer documentation and community FAQs on the https://github.com/mulesoft/munit/wiki[MUnit github wiki page]. 
