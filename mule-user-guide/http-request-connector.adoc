...
In Studio, to instance this connector as an HTTP Request Connector (and not an link:/mule-user-guide/http-listener-connector[HTTP Listener Connector]), you must place it into the *Process* section of a flow (ie: anywhere except the beginning).

image:http-request-connector-1.png[image]
...
image:http-request-connector-2.png[image]
...
image:http-request-connector-5.png[image]
...
image:http-request-connector-6.png[image]
...
|Enable Datasense |The *Enable DataSense* checkbox allows Studio to use link:/mule-user-guide/datasense[DataSense] to expose metadata from the RAML file and interact with it on other elements in Studio.
|===
...
image:http-request-connector-7.png[image]
...
image:http-request-connector-8.png[image]

For this example, you must first link:/mule-user-guide/variable-transformer-reference[generate a variable] named `customMap`. If you assign that variable a value through the following MEL expression:
...
image:http-request-connector-9.png[image]
...
image:http-request-connector-10.png[image]
...
For this example you need to link:/mule-user-guide/variable-transformer-reference[create a variable] named `customMap`. If you set that variable to the MEL expression `#[{'p1':'v1', 'p2':'v2'}]`, it will generate a GET request to http://www.example.com/test/v1/v2
...
 image:http-request-connector-11.png[image] +
...
image:http-request-connector-12.jpeg[image]
...
image:http-request-connector-13.png[image]
...
In order to send parameters in a POST request, the payload of the Mule message should be a Map with the names and the values of the parameters to send. Hence, one way of sending form parameters in your request is adding a Set Payload element before the HTTP Request Connector to make the payload of your message equal to the form parameters you must send:
 ...
 image:http-request-connector-14.jpeg[image]
...
An HTTP response is mapped to the Mule Message in exactly the same way that the HTTP request is mapped to a Mule Message in the link:/mule-user-guide/http-listener-connector[HTTP Listener Connector], except that the following elements don't apply to HTTP responses:
...
image:http-request-connector-15.jpeg[image]
...
* link:/mule-user-guide/http-listener-connector[HTTP Listener Connector]
* Consult a reference to the deprecated predecessor of this element, the HTTP endpoint link:/mule-user-guide/http-request-connector[HTTP Transport Reference]
