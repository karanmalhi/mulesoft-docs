...
|link:/docs/display/current/Until+Successful+Scope[Until Successful] |Repeatedly attempt to process a message until successful
|link:/docs/display/current/Scatter-Gather[Scatter Gather] |Sends a request message to multiple targets concurrently. It collects the responses from all routes, and aggregates them into a single message.
...
link:/docs/display/current/Choice+Flow+Control+Reference[Read More]
...
*From 3.1.0* you can further customize the behavior of this router by specifying a _'failureExpression'_ that allows you to use link:/docs/display/current/Non-MEL+Expressions+Configuration+Reference[Mule Expressions] to define a failure. The _failureExpression_ attribute is configured as follows:
...
link:/docs/display/current/Splitter+Flow+Control+Reference[Read More]
