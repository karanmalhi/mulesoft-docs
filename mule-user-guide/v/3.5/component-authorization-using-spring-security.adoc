...
In addition to the `daoAuthenticationProvider` and `inMemoryDaoImpl` beans (see link:/documentation/display/current/Configuring+Security[Configuring Security]), the following beans must be configured:
