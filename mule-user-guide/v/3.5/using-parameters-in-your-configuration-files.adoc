...
Use unique file names for your properties files to ease the burden on sysadmins. A good strategy is to use the application name or ID in the default and override properties file names. +
 Should you need to encrypt passwords in your properties file, consider using the link:/mule-user-guide/v/3.5/mule-credentials-vault[Mule Credentials Vault]. Refer to the documentation for link:/mule-user-guide/v/3.5/anypoint-enterprise-security[Anypoint™ Enterprise Security] for more information on securing applications in Mule.
