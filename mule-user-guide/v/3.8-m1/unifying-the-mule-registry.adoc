...
Mule 3.7 and newer versions provides upgrades to the lifecycle and link:/mule-user-guide/v/3.8-m1/dependency-injection[dependency injection mechanisms], and unifies the Mule Registry. These changes maintain backwards compatibility.
...
When Mule starts, it searches the classpath for `registry-bootstrap.properties` files, which contain keys and class names for objects to automatically add to the registry. For example, the new link:/mule-user-guide/v/3.8-m1/http-connector[HTTP Connector] uses a properties file to register a connection manager. The Batch module uses it to register the batch engine, etc.
