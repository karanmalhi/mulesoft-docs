...
In addition to the `daoAuthenticationProvider` and `inMemoryDaoImpl` beans (see link:/mule-user-guide/v/3.8-m1/configuring-security[Configuring Security]), the following beans must be configured:
