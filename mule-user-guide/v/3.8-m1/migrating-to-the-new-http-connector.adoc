...
If you are upgrading your projects to Mule 3.6 Runtime from an older version, one of the main adjustments you should consider doing is to replace any instances of the old link:/mule-user-guide/v/3.8-m1/deprecated-http-transport-reference[HTTP endpoint-based Connector] for the new high-performance link:/mule-user-guide/v/3.8-m1/http-connector[HTTP operation-based Connector]. The old HTTP endpoint-based Connector still does work with Mule 3.6 Runtime, but it's deprecated and will be eventually removed in future versions.
