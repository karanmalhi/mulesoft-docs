...
Mule uses link:http://logging.apache.org/log4j/1.2/index.html[Log4j]. WebLogic can be configured so that WebLogic's and Mule's logging both use log4j. For more information on using Commons logging and WebLogic, see link:http://e-docs.bea.com/wls/docs103/logging/config_logs.html#using_log4j.
