...
//2x and 3.2x
_*Enterprise*_ ]
* link:#UsingTransformers-JMS[JMS]
* link:#UsingTransformers-StringsandByteArrays[Strings and Byte Arrays]
* link:#UsingTransformers-XMPP[XMPP]
* link:#UsingTransformers-Custom[Custom]

link:#UsingTransformers-CommonAttributes[Common Attributes]

* link:#UsingTransformers-returnClass[returnClass]
* link:#UsingTransformers-ignoreBadInput[ignoreBadInput]
* link:#UsingTransformers-mimeType[mimeType]
* link:#UsingTransformers-encoding[encoding]
...
The link:#UsingTransformers-ref[Available Transformers] section of this page describes all the standard transformers provided with Mule. Additionally, many link:/docs/display/current/Transports+Reference[transports] and link:/docs/display/current/Modules+Reference[modules] have their own transformers, such as the ObjectToJMSMessage transformer for the JMS transport.
...
package. They do not require any special configuration. For details on these transformers, see link:/docs/display/current/Transformers+Configuration+Reference[Transformers Configuration Reference].
...
|ExpressionTransformer |Evaluates one or more expressions on the current message and return the results as an Array. For details, see link:/docs/display/current/Mule+Expression+Language+MEL[Mule Expression Language MEL].
...
package. They provide the ability to transform between different XML formats, use XSLT, and convert to POJOs from XML. For information, see link:/docs/display/current/XML+Module+Reference[XML Module Reference].
...
|link:/docs/display/current/JXPath+Extractor+Transformer[JXPath Extractor] |Queries and extracts object graphs using XPath expressions using JXPath.
...
package. They provide the ability to work with JSON documents and bind them automatically to Java objects. For information, see link:/docs/display/current/Native+Support+for+JSON[Native Support for JSON].

=== Scripting

The link:/docs/display/current/Scripting+Module+Reference[Scripting] transformer transforms objects using scripting, such as JavaScript or http://groovy.codehaus.org[Groovy] scripts. This transformer is in the
...
|===
|Transformer |Description
|link:/docs/display/current/Transformers+Configuration+Reference#TransformersConfigurationReference-encrypt[Encryption <-> Decryption] |A pair of transformers that use a configured EncryptionStrategy implementation to encrypt and decrypt data.
|===
...
|===
|Transformer |Description
|link:/docs/display/current/Transformers+Configuration+Reference#TransformersConfigurationReference-gzip[GZipCompressTransformer <-> GZipUncompressTransformer] |A pair of transformers that compress and uncompress data.
|===
...
|===
|Transformer |Description
|link:/docs/display/current/Transformers+Configuration+Reference#TransformersConfigurationReference-base64[Base64Encoder <-> Base64Decoder] |A pair of transformers that convert to and from Base 64 encoding.
|link:/docs/display/current/Transformers+Configuration+Reference#TransformersConfigurationReference-xml[XMLEntityEncoder <-> XMLEntityDecoder] |A pair of transformers that convert to and from XML entity encoding.
|===

=== Email

The Email transport provides several transformers for converting from email to string, object to MIME, and more. For details, see link:/docs/display/current/Email+Transport+Reference[Email Transport Reference].

=== File

The File transport provides transformers for converting from a file to a byte array (byte[]) or a string. For details, see link:/docs/display/current/File+Transport+Reference[File Transport Reference].

=== HTTP

The HTTP connector provides several transformers for converting an HTTP response to a Mule message, map or string, and for converting a message to an HTTP request or response. For details, see link:/docs/display/current/HTTP+Connector[HTTP Connector].

=== JDBC +
_*Enterprise*_

The Mule Enterprise version of the JDBC transport provides transformers for moving CSV and XML data from files to databases and back. For details, see link:/docs/display/current/JDBC+Transport+Reference[JDBC Transport Reference].

=== JMS

The link:/docs/display/current/JMS+Transport+Reference[JMS Transport Reference] and link:/docs/display/current/Mule+WMQ+Transport+Reference[Mule WMQ Transport Reference] (enterprise only) both provide transformers for converting between JMS messages and several different data types.

=== Strings and Byte Arrays

The link:/docs/display/current/Multicast+Transport+Reference[Multicast Transport Reference] and link:/docs/display/current/TCP+Transport+Reference[TCP Transport Reference] both provide transformers that convert between byte arrays and strings.

=== XMPP

The XMPP transport provides transformers for converting between XMPP packets and strings. For details, see link:/docs/display/current/XMPP+Transport+Reference[XMPP Transport Reference].

=== Custom

Mule supports the ability to build link:/docs/display/current/Creating+Custom+Transformers[Custom Transformer]. Build custom transformers to meet specific data conversion needs in your application.
