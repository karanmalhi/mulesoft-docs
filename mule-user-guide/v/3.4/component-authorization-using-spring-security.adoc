...
* link:/docs/display/34X/Component+Authorization+Using+Spring+Security#ComponentAuthorizationUsingSpringSecurity-SecuringFlowComponents[Securing Flow Components]
* link:/docs/display/34X/Component+Authorization+Using+Spring+Security#ComponentAuthorizationUsingSpringSecurity-SettingSecurityPropertiesontheSecurityProvider[Setting Security Properties on the Security Provider]
...
In addition to the `daoAuthenticationProvider` and `inMemoryDaoImpl` beans (see link:/docs/display/34X/Configuring+Security[Configuring Security]), the following beans must be configured:
