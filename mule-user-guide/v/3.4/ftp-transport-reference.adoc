...
Mule also supports the SFTP protocol for secure file transfer. As of Mule 3, the link:/docs/display/34X/SFTP+Transport+Reference[SFTP Transport] is included in the Mule distribution.
...
* Support for link:/docs/display/34X/Configuring+Reconnection+Strategies[reconnection strategies]
...
If link:/docs/display/34X/Configuring+Reconnection+Strategies[reconnection strategies] are configured, the FTP connection can be re-established automatically by Mule based on the policy you have configured.

The FTP transport does not support transactions as the File Transfer Protocol itself is not transactional. Instead you should design compensating transactions into your architecture using link:/docs/display/34X/Error+Handling[exception strategies] in Mule.
...
|file:abstract-filenameParser |0..1 |The filenameParser is used when writing files to an FTP server. The parser will convert the outputPattern attribute to a string using the parser and the current message. To add a parser to your configuration, you import the "file" namespace into your XML configuration. For more information about filenameParsers, see the link:/docs/display/34X/File+Transport+Reference[File Transport Reference].
|===
...
This lets you use link:/docs/display/34X/Non-MEL+Expressions+Configuration+Reference[arbitrary expressions] to compose the filename that is used when storing files on the FTP server.
