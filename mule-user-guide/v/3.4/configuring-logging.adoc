...
For logging, Mule ESB uses link:http://www.slf4j.org/[slf4j], which is a logging facade that discovers and uses a logging strategy from the classpath, such as link:http://log4j.apache.org[Log4J] or the JDK Logger. By default, Mule includes link:http://logging.apache.org/log4j/1.2/[Log4J], which is configured with a file called `log4j.properties`.
