...
You can configure the Mule JMX subsystem for remote management with third-party tools like link:http://mc4j.org/[MC4J]. Mule provides an RMI registry agent, which binds to an existing RMI registry or creates a new one on a defined URI.
