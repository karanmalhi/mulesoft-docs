...
If you are not familiar with XML schema, you may want to take an introductory course link:http://www.w3schools.com/schema/schema_intro.asp[here]. However, Mule defines most of what you need out of the box, so it's fairly straightforward to jump in and write your own. Following are a few key concepts:
