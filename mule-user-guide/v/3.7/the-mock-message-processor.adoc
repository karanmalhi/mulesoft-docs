...
* link:#TheMockMessageProcessor-to_mock[Defining the Message Processor to Mock]
* link:#TheMockMessageProcessor-processor_attributes[Defining Mocked Behavior Using Message Processor Attributes]
* link:#TheMockMessageProcessor-mock_response[Defining the Payload of the Mock Response]
...
[TIP]
If the attribute you wish to use when mocking is similar to `config-ref` and resolves to an actual bean, you can use the MUnit MEL function `getBeanFromMuleContext('bean_name')`. This function inspects the Mule registry and returns the bean with the matching name if present. See link:#[Assertion] for details.
