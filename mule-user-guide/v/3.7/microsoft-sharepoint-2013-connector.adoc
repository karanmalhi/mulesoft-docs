...
Since version 2.1.10 you can use as well the _internal_ or _title_ field names in DSQL queries.
...
If their _title_ are the following _Author_, _Date created_ and _Details_ respectively, then you can write the query the using field 'names':
...
> Note: In order to filter by a datetime field type you need to write the value using ISO8601 format using DSQL (eg. Created > 2000-01-01T00:00:00-03:00).
