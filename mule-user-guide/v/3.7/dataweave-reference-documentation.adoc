...
=== Reference Mule Properties

In Mule, you can define properties at a project level, these are then assigned when deploying. The recomended way to do this is using a properties file `mule-app.properties` in the 'src/main/app' folder, for more details and other ways to assign these, see link:/mule-user-guide/v/3.7/configuring-properties[Configuring Properties]. These properties can then be referenced by your DataWeave code.

To create a placeholder for one of these properties in your DataWeave code, simply write p('propertyName'). In the example below, keep in mind that a property named 'language' must first be defined.
...
<flow name="mySecondFlow">
    <set-payload doc:name="Set Payload" value="#[payload.get("b") + ' world!' ]"/>
</flow>
----
