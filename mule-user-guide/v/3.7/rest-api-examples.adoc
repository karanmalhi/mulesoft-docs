...
To split the incoming XML, the splitter uses the link:/mule-user-guide/v/3.7/mule-expression-language-mel[Mule Expression Language] expression `#[xpath('//results')]`, which as you can see contains the link:http://en.wikipedia.org/wiki/XPath[XPath] expression that actually retrieves the XML element `<results>`.
