...
The authorization token should be configured (ie. match) on the connector and the Gateway configuration file. The following configuration setting shows how the token is set within the Gateway configuration file  `Mule.SelfHost.exe.config` :
...
When you configure the connectors running in the Mule ESB, the authorization token value should be set at the `Gateway Access Token` (`accessToken` attribute at the connector's XML configuration).
