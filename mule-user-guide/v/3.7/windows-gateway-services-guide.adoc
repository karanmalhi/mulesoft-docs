...
. Copy the Authentication Token that displays for use when you configure the connection to the Gateway Service from Anypoint Studio.. For more information, see the link:/docs/display/current/Windows+PowerShell+Connector+Guide[Windows PowerShell Connector Guide.] After you copy the token, click *Install*.
