...
Use the link:http://mojo.codehaus.org/exec-maven-plugin/[exec-maven-plugin]'s _java_ goal to run the class `org.mule.MuleServer` and pass it a `'-config configname.xml'` parameter. Maven will work out the classpath for you.
