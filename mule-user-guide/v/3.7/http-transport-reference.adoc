...
The HTTP transport provides support for exposing applications over HTTP and making HTTP client requests from Mule flows to external services. Mule supports *listener connectors*, that receive HTTP requests, and *request connectors* that send requests and receive a response.
...
* Provides security includes TLS, certificates, and authentication
...
[source,xml, linenums]
----
<http:request address="http://localhost:8080#[message.inboundProperties['http.request']" />
----
