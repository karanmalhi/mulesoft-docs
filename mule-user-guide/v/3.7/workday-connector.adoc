...
You can install a connector in Anypoint Studio using the instructions in link:/mule-fundamentals/v/3.7/anypoint-exchange[Installing a Connector from Anypoint Exchange] . However, to use the Workday Connector in a production environment, you must have either:
...
image:wd_palette+%281%29.png[wd_palette+%281%29]
...
image:global_elements_1.jpeg[global_elements_1]
...
image:global_elements_2.jpeg[global_elements_2]
...
+
image:cash+management.png[cash+management]
+
...
[TIP]
====
To set the host for the HTTP connector, you need to add a connector configuration. Click the plus image:plus.png[plus] icon next to *Connector Configuration* (outlined above) to display the global element properties for the HTTP connector. Then, set the value of the *Host* field to `localhost`.
