...
This document assumes that you are familiar with the link:/documentation/display/current/Mule+Studio+Essentials[Visual Editor in Anypoint Studio], link:/documentation/display/current/Anypoint+Connectors[Anypoint Connectors] and link:/documentation/display/current/Understand+Global+Mule+Elements[Global Elements] . Review the link:/documentation/display/current/Getting+Started+with+Mule+Studio[Getting Started with Anypoint Studio] chapter to learn more about developing with Mule ESB's graphical user interface.

== Configuring Auto-Paging

The table below lists the link:/documentation/display/current/Anypoint+Connectors[Anypoint Connectors] which support auto-paging functionality. (Additionally, you can use auto-paging with any custom-built connector which supports auto-paging functionality.)
