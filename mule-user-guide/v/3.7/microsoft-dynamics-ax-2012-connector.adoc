...
|*AOS Services WSDL Port |The port number that the WSDL Services runs on. The default port number is `8101`.
...
|*AOS Services Endpoint Port (Optional) |Use this configuration to override the service endpoint that is specified in the Services' WSDL document.
|===
+
