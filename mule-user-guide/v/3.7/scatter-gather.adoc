...
|*Threading Profile* |Optionally, use to customize the threading profile. Refer to link:/docs/display/current/Tuning+Performance[Tuning Performance] for a description of the configurable attributes. For a brief discussion of threading profiles in Scatter-Gather, see the link:#Scatter-Gather-threadprof[section below]. + |Default threading profile |
