...
*The above configuration will listen for HTTP requests in the address link:http://www.example.com/test[http://localhost:8081]*
....
...
[NOTE]
If using wildcards leads to a situation where a request's address happens to match the path of multiple listeners, then the listener with the *morst specific path* is ALWAYS be given priority, regardless of the order in which the connectors are defined. Only one connector handles each request. +
...
[TIP]
====
To send an HTTP request that contains form parameters, the easiest way is to use a browser extension such as https://chrome.google.com/webstore/detail/postman-rest-client/fdmmgilgnpjigdojojpjoooidkmcomcm[Postman] (Google Chrome), or the link:http://curl.haxx.se/[curl] command line utility. +
...
In the example above, two headers will be added to every response to a request to the app. Both headers take their values from link:/mule-user-guide/v/3.7/mule-expression-language-date-and-time-functions[dateTime MEL expressions.]
...
Note that in either case, the request response will not be displayed as the response body, so it won't be visible if you make your request throug a browser window. +
 +
To be able to view the status code and explanation of a an HTTP request, you can use a browser extension such as https://chrome.google.com/webstore/detail/postman-rest-client/fdmmgilgnpjigdojojpjoooidkmcomcm[Postman] (Google Chrome), or the link:http://curl.haxx.se/[curl] command line utility.
...
* To update allplications that use the old HTTP endpoint based connector, see link:/mule-user-guide/v/3.7/migrating-to-the-new-http-connector[Migrating to the New HTTP Connector]
* Consult a reference to the deprecated predecessor of this element,  link:/mule-user-guide/v/3.7/http-transport-reference[HTTP Transport Reference]
