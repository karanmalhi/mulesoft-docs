...
Some properties are include with every script component (on any language) to make the execution context accessible. Note that the access to these variables depends on the language select.
Next table describe them:
...
|`registry` | Adds lookup/register/unregister methods for Mule-specific entities.
