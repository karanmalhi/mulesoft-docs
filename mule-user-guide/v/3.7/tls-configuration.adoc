...
[TIP]
The standard JDK distribution includes a *default trust store* with certificates for several major certificate authorities (CA's) which is used by default in the 'tls:trust-store' element, but you can generate your own if you wish to have greater security or when using self-signed certificates.
...
The client will trust the server if a chain of trust can be established, either directly to the server because it's certificate is in the trust store or to a signing CA whose certificate is present, failing otherwise. This means that a trust store must be defined when using self-signed certificates.
