...
You can install a connector in Anypoint Studio using the instructions in link:/documentation/display/current/Anypoint+Exchange#AnypointExchange-InstallingaConnectorfromAnypointExchange[Installing a Connector from Anypoint Exchange].
...
To use a Web Service Consumer in your Mule application:

. Add the *WSDL file* of the service you want to consume to your Mule project
. Embed a *Web Service Consumer element* in your Mule flow and configure it to define the operation it  performs, and identify the Global Web Service Consumer element it references
...
To configure a web service consumer:

. Acquire the *WSDL file* for the SOAP Web service you intend to consume, or identify the *URL of the WSDL*.
...
. If the SOAP service returns a response with headers, you notice that the tree in the input section of the DataWeave transformer has multiple inbound properties in it. Each header enters the Mule flow as an inbound property. You can double click it to open this property in a different tab and provide test data.
...
<ws:consumer-config name="OrderWS" wsdlLocation="enterprise.wsdl.xml" service="OrderService"
  port="Soap" serviceAddress="https://login.orderservice.com/services/Soap/c/22.0"
  doc:name="Web Service Consumer">
  <ws:security>
    <ws:wss-username-token username="test" password="test" passwordType="DIGEST"
...
 
<ws:consumer-config wsdlLocation="Test.wsdl" service="TestService"
    port="TestPort" serviceAddress="http://localhost:${port}/services/Test"
    connectorConfig="customConfig" name="globalConfig"/>
----
...
+
image:wsc_global_elements.jpg[wsc_global_elements]
...
. In your instance of the Web Service Consumer in your flow, click the green plus sign next to *Connector Configuration*:
+
...
+
image:adding_https_ref2.jpg[image,title="adding_https_ref2.jpg"]
...
image:wsc_studio_flow.png[wsc_studio_flow]
