...
The module archetype can be used for updating existing modules and transports. It allows developers to add template code for schema configurations and link:/documentation/display/current/Bootstrapping+the+Registry[bootstrap the registry]. It will leave your existing code untouched.
