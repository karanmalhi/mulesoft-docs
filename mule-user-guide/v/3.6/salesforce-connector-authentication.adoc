...
This document gives examples of OAuth and basic configurations, but focuses on basic authentication. For an in-depth explanation of how to connect to Salesforce through OAuth, see link:/documentation/display/current/Using+a+Connector+to+Access+an+OAuth+API[Using a Connector to Access an OAuth API].

== Assumptions

This document assumes that you have a Salesforce account with access to data. It also assumes you are familiar with the link:/documentation/display/current/Salesforce+Connector[Salesforce Connector] in Anypoint Studio.
...
. Enter the message processor's properties editor, then click the green plus sign to create a new link:/documentation/display/current/Global+Elements[Global Element] to set up connection parameters.
 +
image:/documentation/download/attachments/122752071/SelectConnectorConfig.png?version=1&modificationDate=1398119582782[image]
...
+
image:/documentation/download/attachments/122752071/GE.png?version=1&modificationDate=1395430888353[image]
+
[TIP]
====
In the image above, the fields hold link:/documentation/display/current/Configuring+Properties[parametrized references] to a configuration file placed in the `src` folder of your project. You can also directly provide the actual values into the fields if you don't plan to change these.
====
...
. Create a new Mule Project and add an `sfdc:global-element` at the start of your file. link:/documentation/display/current/Global+Elements[Global Elements] set up reusable connection parameters.
+
[source, xml]
----
...
----
+
You must provide four required credentials: *username*, *password*, *securityToken*. The *url* parameter is optional. You can either directly provide these values in the fields or add link:/documentation/display/current/Configuring+Properties[parameterized references] to a configuration file.
...
[WARNING]
====
Currently, when authenticating via OAuth, DataSense is not able to access Salesforce metadata, meaning that you miss out in some very practical design-time Studio features like autocompletion of returned fields, link:/documentation/display/current/DataMapper+Concepts[DataMapper] auto detection of data structure, and exploring payload data via the metadata visualizer.
====
...
For a detailed explanation of how perform these steps with Salesforce, read link:/documentation/display/current/Using+a+Connector+to+Access+an+OAuth+API[Using a Connector to Access an OAuth API.]
...
* link:/documentation/display/current/Salesforce+Connector+Reference[Salesforce Connector Reference]

* Read about link:/documentation/display/current/Using+a+Connector+to+Access+an+OAuth+API[connecting to OAuth APIs] with Mule
