...
You can find examples in link:/mule-fundamentals/v/3.6/anypoint-exchange[the Library]. The following example is for a custom transformer called `StringToNameString`, which wraps Java string in a custom class called `NameString`:
