...
Access link:/documentation/display/current/Transformer+Annotation[Transformer Annotation] if you are looking for information about creating a transformer with the Transformer Annotation.
...
When Mule starts, it discovers this bootstrap file before loading any configuration and installs any objects listed in the file into the local registry. For more information, see link:/documentation/display/current/Bootstrapping+the+Registry[Bootstrapping the Registry].
...
You can find examples in link:/documentation/display/current/Anypoint+Exchange[the Library]. The following example is for a custom transformer called `StringToNameString`, which wraps Java string in a custom class called `NameString`:
