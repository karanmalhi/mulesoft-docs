...
This page provides details on the global settings you configure at the root level of a Mule configuration. For more information on configuration, see link:/documentation/display/current/About+Mule+Configuration[About Mule Configuration]. For information on threading profiles, see link:/documentation/display/current/Tuning+Performance[Tuning Performance].
