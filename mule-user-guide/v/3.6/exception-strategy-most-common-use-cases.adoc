...
. The catch exception strategy replaces the message payload with new content. Mule uses the link:/documentation/display/current/Mule+Expression+Language+MEL[Mule Expression Language MEL] `exception` variable to include an error number in the message payload.
