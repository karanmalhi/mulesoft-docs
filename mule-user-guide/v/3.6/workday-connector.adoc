...
* A global Workday Human Resource connector configuration (read more about link:/mule-fundamentals/v/3.7/global-elements[Global Elements in Mule] )
...
sign (*+*) next to *Connector Configuration* (outlined above) to display the global element properties for the HTTP connector. Then, set the value of the *Host* field to `localhost`.
+
...
+
image:global_elements_1.png[global_elements_1]
+
[cols=",",options="header"]
|===
