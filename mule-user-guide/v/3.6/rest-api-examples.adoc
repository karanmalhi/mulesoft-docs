...
This page provides three examples of consuming REST APIs: a link:#RESTAPIExamples-e1[simple example] that simply details how to retrieve data from a REST API, and two examples – one for link:#RESTAPIExamples-restxml[XML] and one for link:#RESTAPIExamples-restjson[JSON] – showing how to consume a REST API and insert selected data into an external database.

== Simple Example: Consuming a REST API

*link:#RESTAPIExamples-code[Skip to the code]*
...
image:/documentation/download/attachments/122752040/rest+api+1.png?version=1&modificationDate=1419860187322[image]
...
The first element, an link:/documentation/display/current/HTTP+Connector[HTTP listener], listens on localhost port 8081 (the default) for incoming GET requests. Hitting the listener triggers the flow. Requests to the HTTP listener must take the form:
...
image:/documentation/download/attachments/122752040/http+listener+basic.png?version=1&modificationDate=1419867081961[image]
...
image:/documentation/download/attachments/122752040/http+config+1.png?version=1&modificationDate=1418766173402[image]
...
+
image:/documentation/download/attachments/122752040/http+global+element+baconipsum.png?version=1&modificationDate=1419867308706[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
+
image:/documentation/download/attachments/122752040/http+connector+basic.png?version=1&modificationDate=1419868383126[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
+
image:/documentation/download/attachments/122752040/request+connector+w+params.png?version=1&modificationDate=1419868243527[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
|*Display Name* |`File` .3+|image:/documentation/download/attachments/122752040/f1.elem3.png?version=1&modificationDate=1402421099460[image]
|*Path* |`/tmp/`
...
image:/documentation/download/attachments/122752040/browser1.png?version=1&modificationDate=1402421099428[image]
...
image:/documentation/download/attachments/122752040/yahoo+xml.png?version=1&modificationDate=1419877957205[image]
...
The HTTP request connector passes the XML it received from the API to a byte-array-to-string transformer, which converts the message payload to a string. Next, a link:/documentation/display/current/Splitter+Flow+Control+Reference[splitter] splits the message into parts. The splitter is configured to split the incoming message using the XML element `<results`> as delimiter. The reason for this is that the REST API uses `<results>` to delimit the information for each stock symbol retrieved, as shown below.
...
To split the incoming XML, the splitter uses the link:/documentation/display/current/Mule+Expression+Language+MEL[Mule Expression Language] expression `#[xpath('//results')]`, which as you can see contains the http://en.wikipedia.org/wiki/XPath[XPath] expression that actually retrieves the XML element `<results>`.

The last element in the flow, a link:/documentation/display/current/JDBC+Connector[Database (JDBC)] connector, receives each XML chunk corresponding to each queried symbol. The JDBC connector is configured to run the following SQL query:
...
The full configuration for this example is detailed below. To see the application in action, skip to link:#RESTAPIExamples-run2[Running This Example].
...
. Create a new Connector Configuration Element and set up the *Host* and *Base Path*:
+
image:/documentation/download/attachments/122752040/yahoo+global+element.png?version=1&modificationDate=1419878220587[image]
+
[width="20%,cols=",",options="header"]
|===
|Parameter |Value
...
+
image:/documentation/download/attachments/122752040/yahoo+http+w+params.png?version=1&modificationDate=1419878751368[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Byte Array to String` .5+|image:/documentation/download/thumbnails/122752040/byte_array_to_str.png?version=1&modificationDate=1402421099442[image]
|*Return Class* |`-`
|*Ignore Bad Input* |no
|*Encoding* |`-`
|*MIME Type* |`-`
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Byte Array to String` .4+|image:/documentation/download/attachments/122752040/splitter-gentab.png?version=1&modificationDate=1402421099566[image]
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Database` .5+|image:/documentation/download/attachments/122752040/db_conn_general_tab.png?version=1&modificationDate=1402496843506[image]
|*Connector configuration*  |`Postgres` +
(This is the DB global element referenced by this connector; for configuration details see link:#RESTAPIExamples-db_global[Database Global Connector].)
|*Operation* |`Insert`
|*Query type* |`Dynamic`
|*Dynamic query a|
...
image:/documentation/download/attachments/122752040/db_conn_adv_tab.png?version=1&modificationDate=1402496961619[image]
....
[tab,title="Standalone XML"]
....
[source, xml]
...
The database global connector is a link:/documentation/display/34X/Global+Elements[global element] that contains the connection configuration for the desired database. It is referenced by the database connector in the application flow.

Note that you must install the appropriate driver for your database in your Studio application. For details, see the Adding the Database Driver section in link:/documentation/display/current/Database+Connector[Database Connector].
...
|===
|Parameter |Value |Configuration window image
|*Name* |`Postgres` .5+|image:/documentation/download/attachments/122752040/db_global_elem.png?version=1&modificationDate=1402497637228[image]
...
image:/documentation/download/attachments/122752040/browser2.png?version=1&modificationDate=1402421099436[image]
...
image:/documentation/download/thumbnails/122752040/pgadmin.png?version=1&modificationDate=1402421099541[image]
...
image:/documentation/download/attachments/122752040/yahoo+flow+2.png?version=1&modificationDate=1419881239673[image]
...
The HTTP Request Connector passes the JSON it received from the API to a link:/documentation/display/current/JSON+Module+Reference[JSON-to-object transformer] configured to output a java.util.Map object. The transformer sends this object, which contains the JSON data as key=value pairs, to the last element in the flow, a link:/documentation/display/current/Database+Connector[database connector]. This connector uses an SQL query with embedded link:/documentation/display/current/Mule+Expression+Language+MEL[Mule Expression Language] expressions to extract specific values from the JSON and insert them into the external database. The full SQL query is shown below.
...
The full configuration for this example is detailed below. To see the application in action, skip to link:#RESTAPIExamples-runjson[Running This Example].
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`JSON to Object` .2+|image:/documentation/download/thumbnails/122752040/json.to.object.general.png?version=1&modificationDate=1402421099520[image]
|*Mapper Reference* |`-`
|===
...
|===
|Parameter |Value |Configuration window image
|*Return Class* |`java.util.Map` .3+|image:/documentation/download/thumbnails/122752040/json.to.object-adv_tab.png?version=1&modificationDate=1402421099514[image]
|*Ignore Bad Input* |no
|*Encoding* |`-`
|*MIME Type* |`-`
|===
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Database` .4+|image:/documentation/download/attachments/122752040/db_conn_json_gral_tab.png?version=1&modificationDate=1402501478553[image]
|*Connector configuration* |`Postgres`
|*Operation* |`insert`
|*Query Type* |`Dynamic`
|*Dynamic query* a|
...
image:/documentation/download/thumbnails/122752040/pgadmin-json.png?version=1&modificationDate=1402421099535[image]

== See Also

* Explore how to create a REST API with link:/documentation/display/current/Building+Your+API[APIkit], Mule's dedicated toolkit for developing and exposing REST APIs.
