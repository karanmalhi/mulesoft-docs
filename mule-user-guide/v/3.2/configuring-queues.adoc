...
[NOTE]
This page describes how Mule uses queues and how you configure them. Most of the information on this page applies to Mule 3.1.x and Mule 3.2. However, a number of changes have been made in Mule 3.2 that change the way queues are configured. These changes are described in link:#ConfiguringQueues-changem32[Changes in Mule 3.2] – the material in that section applies only to Mule 3.2.
...
A number of changes have been made in Mule 3.2 that that change the way queues are configured. The changes include the use of link:#ConfiguringQueues-pstrategy[MULE3USER:processing strategies for flows], and link:#ConfiguringQueues-objstore[MULE3USER:object stores for queues].
