...
EJB endpoints are configured the same way as link:/documentation-3.2/display/32X/RMI+Transport+Reference[RMI endpoints]. Note that only outbound endpoints can use the EJB transport. For a given endpoint, you must provide the following information:
