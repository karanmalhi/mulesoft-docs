...
The `@Schedule` annotation also supports link:#ScheduleAnnotation-cron[MULE3USER:cron] expressions. These provide a powerful way to express time triggers.
...
|cron |A link:#ScheduleAnnotation-cron[MULE3USER:cron command] that specifies when to call the method. |You must set either `cron` or `interval`.
