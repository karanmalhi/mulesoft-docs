...
Other namespaces can be introduced via `<spring:beans>` or by adding a dedicated element in a module (see the link:/documentation-3.2/display/32X/Scripting+Module+Reference[Scripting module's] `<lang>` element).
