...
|link:#RoutingMessageProcessors-All[MULE3USER:All] |Broadcast a message to multiple targets
|link:#RoutingMessageProcessors-Async[MULE3USER:Async] |Run a chain of message processors in a separate thread
|link:#RoutingMessageProcessors-Choice[MULE3USER:Choice] |Send a message to the first matching message processor
|link:#RoutingMessageProcessors-CollectionAggregator[MULE3USER:Collection Aggregator] |Aggregate messages into a message collection
|link:#RoutingMessageProcessors-CollectionSplitter[MULE3USER:Collection Splitter] |Split a message that is a collection
|link:#RoutingMessageProcessors-CombineCollections[MULE3USER:Combine Collections] |Converts a payload consisting of a collection of collections into a single list.
|link:#RoutingMessageProcessors-CustomAggregator[MULE3USER:Custom Aggregator] |A custom-written class that aggregates messages
|link:#RoutingMessageProcessors-CustomProcessor[MULE3USER:Custom Processor] |A custom-written message processor
|link:#RoutingMessageProcessors-FirstSuccessful[MULE3USER:First Successful] |Iterate through message processors until one succeeds (added in 3.0.1) +
|link:#RoutingMessageProcessors-IdempotentMessageFilter[MULE3USER:Idempotent Message Filter] |Filter out duplicate message by message ID
|link:#RoutingMessageProcessors-IdempotentSecureHashMessageFilter[MULE3USER:Idempotent Secure Hash Message Filter] |Filter out duplicate message by message content
|link:#RoutingMessageProcessors-MessageChunkAggregator[MULE3USER:Message Chunk Aggregator] |Aggregate messages into a single message
|link:#RoutingMessageProcessors-MessageChunkSplitter[MULE3USER:Message Chunk Splitter] |Split a message into fixed-size chunks
|link:#RoutingMessageProcessors-MessageFilter[MULE3USER:Message Filter] |Filter messages using a filter
|link:#RoutingMessageProcessors-ProcessorChain[MULE3USER:Processor Chain] |Create a message chain from multiple targets
|link:#RoutingMessageProcessors-RecipientList[MULE3USER:Recipient List] |Send a message to multiple endpoints
|*[MULE3USER:Mule 3.2]* link:#RoutingMessageProcessors-RequestReply[MULE3USER:Request Reply] |Receive a message for asynchronous processing and accept the asynchronous response on a different channel
|link:#RoutingMessageProcessors-Resequencer[MULE3USER:Resequencer] |Reorder a list of messages
|link:#RoutingMessageProcessors-RoundRobin[MULE3USER:Round Robin] |Round-robin among a list of message processors (added in 3.0.1)
|*[MULE3USER:Mule 3.2]* link:#RoutingMessageProcessors-UntilSuccessful[MULE3USER:Until Successful] |Repeatedly attempt to process a message until successful +
|link:#RoutingMessageProcessors-Splitter[MULE3USER:Splitter] |Split a message using an expression
|link:#RoutingMessageProcessors-WireTap[MULE3USER:WireTap] |Send a message to an extra message processor as well as to the next message processor in the chain
|===
...
The Message Chunk Aggregator also accepts the `timeout` and `failOnTimeout` attributes as described under link:#RoutingMessageProcessors-CollectionAggregator[MULE3USER:Collection Aggregator].
...
To put the chunked items back together again, you can use the link:#RoutingMessageProcessors-MessageChunkAggregator[MULE3USER:Message Chunk Aggregator].
...
A Processor Chain is a linear chain of message processors which process a message in order. A Processor Chain can be configured wherever a message processor appears in a Mule Schema. For example, to allow a link:#RoutingMessageProcessors-WireTap[MULE3USER:Wire Tap] to transform the current message before sending it off, you can configure the following:
...
The Resequencer sorts a set of received messages by their correlation sequence property and issues them in the correct order. It uses the `timeout` and `fileOnTimeout` attributes described in link:#RoutingMessageProcessors-CollectionAggregator[MULE3USER:Collection Aggregator] to determine when all the messages in the set have been received.
