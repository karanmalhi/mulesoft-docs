...
You can specify which source types a transformer can accept and which type it will return. This allows Mule to validate the incoming message before invoking the transformer and to validate the output before sending the message on. If you create a MULE3USER:discoverable transformer, you must set the source and return types.
