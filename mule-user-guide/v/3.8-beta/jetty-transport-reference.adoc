...
The Jetty transport provides support for exposing applications over HTTP by embedding a light-weight Jetty server. The link:/mule-user-guide/v/3.8-beta/jetty-ssl-transport[Jetty SSL Transport] works the same way but over SSL. You can only define inbound endpoints with this transport.
...
Jetty endpoints are configured the same way as link:/mule-user-guide/v/3.8-beta/file-transport-reference[FTP endpoints]. Note that the Jetty transport can only be used for inbound endpoints.
