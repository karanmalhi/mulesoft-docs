...
Access link:/mule-user-guide/v/3.8-beta/transformer-annotation[Transformer Annotation] if you are looking for information about creating a transformer with the Transformer Annotation.
...
When Mule starts, it discovers this bootstrap file before loading any configuration and installs any objects listed in the file into the local registry. For more information, see link:/mule-user-guide/v/3.8-beta/bootstrapping-the-registry[Bootstrapping the Registry].
...
You can find examples in link:/mule-fundamentals/v/3.7/anypoint-exchange[the Library]. The following example is for a custom transformer called `StringToNameString`, which wraps Java string in a custom class called `NameString`:
