...
For information on MSMQ, see the link:/mule-user-guide/v/3.8-beta/msmq-connector-user-guide[MSMQ Connector User Guide].
...
. Copy the Authentication Token that displays for use when you configure the connection to the Gateway Service from Anypoint Studio.. For more information, see the link:/mule-user-guide/v/3.8-beta/windows-powershell-connector-guide[Windows PowerShell Connector Guide.] After you copy the token, click *Install*.
