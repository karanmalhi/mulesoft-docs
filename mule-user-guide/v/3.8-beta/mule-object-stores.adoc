...
* When configuring an link:/mule-user-guide/v/3.8-beta/idempotent-filter[idempotent message filter] or until a successful scope occurs.
...
* *In-memory store* – Prior to Mule 3.5.0, in-memory store was the default. As of Mule 3.5.0, persistent store is the default. For more information, see "Object Stores and Clustering" in the link:/mule-user-guide/v/3.8-beta/cache-scope[Cache Scope] document.
...
. link:/mule-user-guide/v/3.8-beta/idempotent-filter[idempotent filter] with an in-memory object store
