...
[TIP]
The standard JDK distribution includes a *default truststore* which is used by default in the 'tls:trust-store' element, but you can generate your own if you wish to have greater security.
...
Without a trust store that contains the server's certificate, the client will not trust the server and will raise a `javax.net.ssl.SSLHandshakeException`.
