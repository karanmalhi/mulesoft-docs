...
For more about Mule transformers, see link:/mule-user-guide/v/3.8-beta/using-transformers[Using Transformers].
...
For a current reference to using expressions in Mule, see link:/mule-user-guide/v/3.8-beta/mule-expression-language-mel[Mule Expression Language MEL]. 
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/mule-user-guide/v/3.8-beta/logger-component-reference[Logger Component Reference]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/mule-user-guide/v/3.8-beta/routers[Routing Message Processors].
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/mule-user-guide/v/3.8-beta/entry-point-resolver-configuration-reference[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

❾ Calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about connectors, see link:/mule-user-guide/v/3.8-beta/anypoint-connectors[Anypoint Connectors].
...
❶❺ Writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see link:/mule-user-guide/v/3.8-beta/error-handling[Error Handling].
...
For more about configuration patterns, see link:/mule-user-guide/v/3.8-beta/using-mule-configuration-patterns[Using Mule Configuration Patterns].
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/mule-user-guide/v/3.8-beta/transaction-management[Transaction Management].
...
For more about Mule security, see link:/mule-user-guide/v/3.8-beta/configuring-security[Configuring Security].
