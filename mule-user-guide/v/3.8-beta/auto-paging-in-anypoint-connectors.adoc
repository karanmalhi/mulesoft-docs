...
This document assumes that you are familiar with the link:/mule-fundamentals/v/3.7/anypoint-studio-essentials[Anypoint Studio Essentials], link:/mule-user-guide/v/3.8-beta/anypoint-connectors[Anypoint Connectors] and link:/mule-fundamentals/v/3.7/global-elements[Global Elements]. Review the link:/mule-fundamentals/v/3.7/basic-studio-tutorial[Getting Started with Anypoint Studio] chapter to learn more about developing with Mule ESB's graphical user interface.

== Configuring Auto-Paging

The table below lists the link:/mule-user-guide/v/3.8-beta/anypoint-connectors[Anypoint Connectors] which support auto-paging functionality. (Additionally, you can use auto-paging with any custom-built connector which supports auto-paging functionality.)
...
When you use a connector that employs paging, Mule creates an iterator object that splits the payload into manageable pages. Regardless of the page-size, the iterator pushes out objects one at a time, then fetches the next pages on demand. To process the paginated output, you must use elements in your flow that can handle collections, such as a link:/mule-user-guide/v/3.8-beta/foreach[Foreach scope], a collection splitter, a link:/mule-user-guide/v/3.8-beta/dataweave[Transform Message component], etc. Because Mule processes the set of pages one page at a time, it prevents memory usage from exceeding its limits.
...
* Learn more about link:/mule-user-guide/v/3.8-beta/scopes[Scopes].

* Learn more about the link:/mule-user-guide/v/3.8-beta/foreach[Foreach] scope.

* Need to handle really large payloads? Learn about link:/mule-user-guide/v/3.8-beta/mule-high-availability-ha-clusters[Mule High Availability HA Clusters].
