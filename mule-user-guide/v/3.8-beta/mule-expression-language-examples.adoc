...
This document assumes you are familiar with Mule Expression Language's link:/mule-user-guide/v/3.8-beta/mule-expression-language-basic-syntax[basic syntax] and are comfortable building and running Mule applications using link:/mule-fundamentals/v/3.7/anypoint-studio-essentials[Mule Studio] or XML.
...
In this example, like in the previous one, the Mule application saves a CSV file with user data and returns a greeting. The call to the service includes two parameters, `username` and `age`. The service stores these two parameters and adds a third boolean parameter that evaluates if the user is above a certain age (if `age ``> 18`). In this case, however, the mapping between input and output fields and the generation of the third field is performed by a link:/mule-user-guide/v/3.8-beta/datamapper-user-guide-and-reference[DataMapper] component.
...
* Access the Mule Expression Language link:/mule-user-guide/v/3.8-beta/mule-expression-language-reference[Reference] and link:/mule-user-guide/v/3.8-beta/mule-expression-language-tips[Tips].
