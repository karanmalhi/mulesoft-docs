...
Depending on the needs of your application, you can use link:/mule-user-guide/v/3.8-beta/dataweave[Transform Message component] or a JSON-to-Object transformer to transform this response into another format from which you can extract information, route messages, etc.
...
. You can also use a link:/mule-user-guide/v/3.8-beta/configuring-properties[property placeholder], then define the value in your *mule-app.properties* file
+
...
. Note that you can also replace these with link:/mule-user-guide/v/3.8-beta/configuring-properties[property placeholders] and define the properties in your `mule-app.properties` file.
+
...
Rather than hardcode the values of your credentials, you can define them as link:/mule-user-guide/v/3.8-beta/configuring-properties[property placeholders].
...
[NOTE]
Although the HTTP connector has built-in polling capabilities (described below), you can, instead, configure a *link:/mule-user-guide/v/3.8-beta/poll-reference[Poll]* scope, then place your HTTP endpoint within it. Using a Poll scope allows you to take advantage of more robust polling functionality such as watermarking and scheduling with cron expressions.
...
* *POST requests and the API schema:* If you are calling a REST API with a POST request, you need to obtain the API schema for the POST and match that format in the payload of the Mule message that you send to the API with your request. A good way to do this is to insert a link:/mule-user-guide/v/3.8-beta/dataweave[Transform Message component] before the HTTP outbound endpoint in your flow, then use the API's schema in to define the output format.
+
image:consume-api-dw.png[consume-api-dw]

== See Also

* See some link:/mule-user-guide/v/3.8-beta/rest-api-examples[example applications] that consume REST APIs.
