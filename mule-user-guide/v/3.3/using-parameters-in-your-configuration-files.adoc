...
Should you need to encrypt passwords in your properties file, consider using the link:/docs/display/33X/Mule+Credentials+Vault[Mule Credentials Vault]. Refer to the documentation for link:/docs/display/33X/Anypoint+Enterprise+Security[Anypoint Enterprise Security] for more information on securing applications in Mule.
