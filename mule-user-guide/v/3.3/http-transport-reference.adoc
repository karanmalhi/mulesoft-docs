...
You can use the link:/docs/display/33X/HTTPS+Transport+Reference[HTTPS Transport Reference] to create secure connections over HTTP. If you want to secure requests to your HTTP endpoint, the HTTP connector supports HTTP Basic/Digest authentication methods (as well as the Mule generic header authentication). To configure HTTP Basic, you configure a link:/docs/display/33X/Configuring+Security[Security Endpoint Filter] on an HTTP endpoint.
...
This connector also accepts all the attributes from the link:/docs/display/33X/TCP+Transport+Reference[TCP connector].
...
For more information, see link:#HTTPTransportReference-polling[Polling HTTP Services] below.
...
The HTTP inbound endpoint attributes override those specified for the link:/docs/display/33X/Endpoint+Configuration+Reference[default inbound endpoint attributes].
...
The HTTP outbound endpoint attributes override those specified for the link:/docs/display/33X/Endpoint+Configuration+Reference[default outbound endpoint attributes].
...
The HTTP endpoint attributes override those specified for the link:/docs/display/33X/Endpoint+Configuration+Reference[default global endpoint attributes].
