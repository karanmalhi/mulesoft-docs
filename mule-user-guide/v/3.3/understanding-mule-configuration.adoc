...
For more about Mule transformers, see link:/docs/display/33X/Using+Transformers[Using Transformers].
...
Filters once again can be configured as global elements and referred to where they are used, or configured at their point of use.  For more about Mule filters see link:/docs/display/33X/Using+Filters[Using Filters]

==== Expressions

[WARNING]
The following description of expressions refers to the pre-Mule 3.3 expression evaluators. For a current reference to using expressions in Mule, see link:/docs/display/33X/Mule+Expression+Language+MEL[Mule Expression Language MEL].
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/docs/display/33X/Logger+Component+Reference[Logger Component Reference].
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/docs/display/33X/Routing+Message+Processors[Routing Message Processors].
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/docs/display/33X/Entry+Point+Resolver+Configuration+Reference[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

❾ calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about connectors, see link:/docs/display/33X/Anypoint+Connectors[Anypoint Connectors].
...
❶❺ writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see link:#[Exception Handling].
...
For more about configuration patterns, see link:/docs/display/33X/Using+Mule+Configuration+Patterns[Using Mule Configuration Patterns] .
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/docs/display/33X/Transaction+Management[Transaction Management].
...
For more about Mule security, see link:/docs/display/33X/Configuring+Security[Configuring Security].
