...
Each transport has a service descriptor that describes what classes are used to construct the transport. For complete information, see link:/documentation/display/current/Transport+Service+Descriptors[Transport Service Descriptors].
...
Any exceptions messages used in your transport implementation should be stored in a resource bundle so that they can be link:/documentation/display/current/Internationalizing+Strings[internationalized] . The message bundle is a standard Java properties file and must be located at:
