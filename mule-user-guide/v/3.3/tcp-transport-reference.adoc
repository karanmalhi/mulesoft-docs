...
* The link:/docs/display/33X/SSL+and+TLS+Transports+Reference[SSL and TLS Transports], which use TCP with socket-level security. 
...
* Are you communicating with a flow always located in the same Mule application instance? If so, consider use the link:/docs/display/33X/VM+Transport+Reference[VM transport].
...
* Should messages be secured? If so, use the link:/docs/display/33X/SSL+and+TLS+Transports+Reference[SSL transport].
