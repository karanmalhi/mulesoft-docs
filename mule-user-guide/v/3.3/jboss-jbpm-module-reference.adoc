...
[WARNING]
This module provides a "Plug-in" for JBoss jBPM to be used with Mule's BPM support. If you have not yet read the general documentation for Mule's link:/docs/display/33X/BPM+Module+Reference[BPM Support], please read that first and then come back to this page.
...
* link:#JBossjBPMModuleReference-jbpm-config[Simple declaration of jBPM] as the BPMS in your Mule configuration using sensible defaults.
* link:#JBossjBPMModuleReference-jpdl-elements[Custom elements] for jBPM's process definition language (jPDL) which allow you to easily integrate Mule into your business processes.

Refer to link:/docs/display/33X/BPM+Module+Reference[BPM Module Reference] for a list of general features offered by Mule's BPM support.
...
This module uses the schema from the link:/docs/display/33X/BPM+Module+Reference[BPM Module]; it does not have its own schema.
...
Refer to link:/docs/display/33X/BPM+Module+Reference[BPM Module Reference] for detailed information on the elements of the BPM schema.
