...
The SMTP connector supports all the link:/docs/display/33X/Configuring+a+Transport[common connector attributes and properties] and the following optional elements and attributes:
...
SMTP endpoints describe details about the SMTP server and the recipients of messages sent from the SMTP endpoint. You link:/docs/display/33X/Configuring+Endpoints[configure the endpoints] just as you would with any other transport, with the following additional attributes:
...
See the link:/docs/display/33X/Transports+Reference#TransportsReference-TransportMatrix[transport matrix].
