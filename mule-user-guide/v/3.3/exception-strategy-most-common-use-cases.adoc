...
* link:#ExceptionStrategyMostCommonUseCases-RoutetheBefore-ErrorMessagetotheDeadLetterQueue[Route the Before-Error Message to the Dead Letter Queue]
* link:#ExceptionStrategyMostCommonUseCases-RoutetheInitial-StateMessagetotheDeadLetterQueue[Route the Initial-State Message to the Dead Letter Queue]
* link:#ExceptionStrategyMostCommonUseCases-RoutetheMessageBasedonExceptionType[Route the Message Based on Exception Type]
* link:#ExceptionStrategyMostCommonUseCases-ReturninganErrorMessage[Returning an Error Message]
* link:#ExceptionStrategyMostCommonUseCases-RollbacktheTransactionandSendaNotification[Rollback the Transaction and Send a Notification]
* link:#ExceptionStrategyMostCommonUseCases-StopProcessingWhenanExceptionOccurs[Stop Processing When an Exception Occurs]
...
. The catch exception strategy replaces the message payload with new content. Mule uses the link:/docs/display/34X/Mule+Expression+Language+MEL[Mule Expression Language MEL] `exception` variable to include an error number in the message payload. The error number helps the caller understand the exception.
