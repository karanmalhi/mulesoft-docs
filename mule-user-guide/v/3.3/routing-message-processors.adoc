...
[ link:#RoutingMessageProcessors-QuickReference[Quick Reference] ] [ link:#RoutingMessageProcessors-All[All] ] [ link:#RoutingMessageProcessors-Async[Async] ] [ link:#RoutingMessageProcessors-Choice[Choice] ] [ link:#RoutingMessageProcessors-CollectionAggregator[Collection Aggregator] ] [ link:#RoutingMessageProcessors-CollectionSplitter[Collection Splitter] ] [ link:#RoutingMessageProcessors-CombineCollections[Combine Collections] ] [ link:#RoutingMessageProcessors-CustomAggregator[Custom Aggregator] ] [ link:#RoutingMessageProcessors-CustomProcessor[Custom Processor] ] [ link:#RoutingMessageProcessors-FirstSuccessful[First Successful] ] [ link:#RoutingMessageProcessors-IdempotentMessageFilter[Idempotent Message Filter] ] [ link:#RoutingMessageProcessors-IdempotentSecureHashMessageFilter[Idempotent Secure Hash Message Filter] ] [ link:#RoutingMessageProcessors-MessageChunkAggregator[Message Chunk Aggregator] ] [ link:#RoutingMessageProcessors-MessageChunkSplitter[Message Chunk Splitter] ] [ link:#RoutingMessageProcessors-MessageFilter[Message Filter] ] [ link:#RoutingMessageProcessors-ProcessorChain[Processor Chain] ] [ link:#RoutingMessageProcessors-RecipientList[Recipient List] ] [ link:#RoutingMessageProcessors-RequestReply[Request Reply] ] [ link:#RoutingMessageProcessors-Resequencer[Resequencer] ] [ link:#RoutingMessageProcessors-RoundRobin[Round Robin] ] [ link:#RoutingMessageProcessors-Splitter[Splitter] ] [ link:#RoutingMessageProcessors-UntilSuccessful[Until Successful] ] [ link:#RoutingMessageProcessors-WireTap[WireTap] ]

Message Processors are used within flows to control how messages are sent and received within that flow. This is further described in link:/docs/display/33X/Using+Flows+for+Service+Orchestration[Using Flows for Service Orchestration].
...
|link:#RoutingMessageProcessors-All[All] |Broadcast a message to multiple targets
|link:#RoutingMessageProcessors-Async[Async] |Run a chain of message processors in a separate thread
|link:#RoutingMessageProcessors-Choice[Choice] |Send a message to the first matching message processor
|link:#RoutingMessageProcessors-CollectionAggregator[Collection Aggregator] |Aggregate messages into a message collection
|link:#RoutingMessageProcessors-CollectionSplitter[Collection Splitter] |Split a message that is a collection
|link:#RoutingMessageProcessors-CombineCollections[Combine Collections] |Converts a payload consisting of a collection of collections into a single list.
|link:#RoutingMessageProcessors-CustomAggregator[Custom Aggregator] |A custom-written class that aggregates messages
|link:#RoutingMessageProcessors-CustomProcessor[Custom Processor] |A custom-written message processor
|link:#RoutingMessageProcessors-FirstSuccessful[First Successful] |Iterate through message processors until one succeeds (added in 3.0.1)
|link:#RoutingMessageProcessors-IdempotentMessageFilter[Idempotent Message Filter] |Filter out duplicate message by message ID
|link:#RoutingMessageProcessors-IdempotentSecureHashMessageFilter[Idempotent Secure Hash Message Filter] |Filter out duplicate message by message content
|link:#RoutingMessageProcessors-MessageChunkAggregator[Message Chunk Aggregator] |Aggregate messages into a single message
|link:#RoutingMessageProcessors-MessageChunkSplitter[Message Chunk Splitter] |Split a message into fixed-size chunks
|link:#RoutingMessageProcessors-MessageFilter[Message Filter] |Filter messages using a filter
|link:#RoutingMessageProcessors-ProcessorChain[Processor Chain] |Create a message chain from multiple targets
|link:#RoutingMessageProcessors-RecipientList[Recipient List] |Send a message to multiple endpoints
|link:#RoutingMessageProcessors-RequestReply[Request Reply] |Receive a message for asynchronous processing and accept the asynchronous response on a different channel
|link:#RoutingMessageProcessors-Resequencer[Resequencer] |Reorder a list of messages
|link:#RoutingMessageProcessors-RoundRobin[Round Robin] |Round-robin among a list of message processors (added in 3.0.1)
|link:#RoutingMessageProcessors-UntilSuccessful[Until Successful] |Repeatedly attempt to process a message until successful
|link:#RoutingMessageProcessors-Splitter[Splitter] |Split a message using an expression
|link:#RoutingMessageProcessors-WireTap[WireTap] |Send a message to an extra message processor as well as to the next message processor in the chain
|===
...
*From 3.1.0* you can further customize the behavior of this router by specifying a _'failureExpression'_ that allows you to use link:/docs/display/33X/Non-MEL+Expressions+Configuration+Reference[Mule Expressions] to define a failure. The _failureExpression_ attribute is configured as follows:
...
After a splitter such as the link:#RoutingMessageProcessors-MessageChunkSplitter[Message Chunk Splitter] splits a message into parts, the message chunk aggregator router reassembles those parts back into a single message. The aggregator uses the message's correlation ID to identify which parts belong to the same message.
...
The Message Chunk Aggregator also accepts the `timeout` and `failOnTimeout` attributes as described under link:#RoutingMessageProcessors-CollectionAggregator[Collection Aggregator].
...
To put the chunked items back together again, you can use the link:#RoutingMessageProcessors-MessageChunkAggregator[Message Chunk Aggregator].
...
The Message Filter is used to control whether a message is processed by using a link:/docs/display/33X/Using+Filters[filter]. In addition to the filter, you can configure whether to throw an exception if the filter does not accept the message and an optional message processor to send unaccepted messages to.
...
A Processor Chain is a linear chain of message processors which process a message in order. A Processor Chain can be configured wherever a message processor appears in a Mule Schema. For example, to allow a link:#RoutingMessageProcessors-WireTap[Wire Tap] to transform the current message before sending it off, you can configure the following:
...
The Recipient List message processor allows you to send a message to multiple endpoints by specifying an expression that, when evaluated, provides the list of endpoints. These messages can optionally be given a correlation ID, as in the link:#RoutingMessageProcessors-CollectionSplitter[Collection Splitter]. An example is
...
The Resequencer sorts a set of received messages by their correlation sequence property and issues them in the correct order. It uses the `timeout` and `failOnTimeout` attributes described in link:#RoutingMessageProcessors-CollectionAggregator[Collection Aggregator] to determine when all the messages in the set have been received.
...
See link:/docs/display/33X/Mule+Object+Stores[Mule Object Stores] for further information about object stores in Mule.
