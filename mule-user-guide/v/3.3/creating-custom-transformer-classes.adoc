...
Access link:/docs/display/current/Transformer+Annotation[Transformer Annotation] if you are looking for information about creating a transformer with the Transformer Annotation.
...
When Mule starts, it will discover this bootstrap file before loading any configuration and will install any objects listed in the file into the local registry. For more information, see link:/docs/display/current/Bootstrapping+the+Registry[Bootstrapping the Registry].
...
The link:/docs/display/current/Hello+World+Example[Hello World example] defines a custom transformer called `StringToNameString`, which wraps Java string in a custom class called `NameString`:
