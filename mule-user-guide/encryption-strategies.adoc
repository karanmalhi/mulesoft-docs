...
The Security Manager can be configured with one or more encryption strategies that can then be used by encryption transformers, security filters, or secure transports such as link:/mule-user-guide/ssl-and-tls-transports-reference[SSL] or link:/mule-user-guide/http-connector[HTTPS]. These encryption strategies can greatly simplify configuration for secure messaging as they can be shared across components.
