...
This document offers examples of applications which use the Database Connector. For full documentation of the connector, access the link:/mule-user-guide/database-connector[Database Connector] documentation and link:/mule-user-guide/database-connector-reference[Database Connector Reference].

== Assumptions

This document presents two usage examples of the database connector. This document assumes you are familiar with the link:/mule-user-guide/database-connector[database connector] as well as with link:/mule-user-guide/datasense[DataSense]; to understand the second example, the reader should also be familiar with the link:/documentation/display/33X/DataMapper+Transformer+Reference[DataMapper transformer] and Mule link:/mule-user-guide/batch-processing[batch processing].
...
image:database-connector-examples-1.png[image]

In the Mule application, an inbound link:/mule-user-guide/http-connector[HTTP connector] listens for HTTP GET requests with the form: `http://<host>:8081/?lastname=<parameter>`. The HTTP connector passes the value of `<parameter>` as one of the message properties to a database connector. The database connector is configured to extract this value and use it for the SQL query listed below.
...
As you can see, the link:/mule-user-guide/mule-expression-language-mel[MEL] expression in the SQL query references the value of the parameter passed to the HTTP connector. So if the HTTP connector receives `http://localhost:8081/?lastname=Smith`, the SQL query is `select first_name from employees where last_name = Smith`.
...
|*Display Name* |`Database` .5+|image:database-connector-examples-2.png[image]
...
In this example, the database connector references the `MySQL_Configuration` link:/mule-fundamentals/global-elements[global element] to obtain connection parameters. `MySQL_Configuration` is configured with the parameters listed below.
...
image:database-connector-examples-3.png[image]
...
image:database-connector-examples-4.png[image]
...
image:database-connector-examples-5.png[image]
...
In this example, a database connector retrieves data from two tables. A link:/documentation/display/33X/DataMapper+Transformer+Reference[DataMapper transformer] maps this data to the CSV format. A link:/mule-user-guide/file-connector[file connector] writes the resulting CSV file to disk, and a link:/documentation/display/33X/Logger+Component+Reference[logger component] logs processing details to the console.

This example uses link:/mule-user-guide/batch-processing[batch processing], which means that within this Mule app, messages are divided into records and processed asynchronously. Batch processing, in conjunction with *streaming* enabled on the database connector, can be useful to avoid system overload when dealing with large volumes of data.

image:database-connector-examples-6.png[image]

The database connector is inside a link:/mule-user-guide/poll-reference[poll scope] which, every ten minutes, requests the database connector to perform its configured operation on the database. The database connector performs a database query which returns 100 rows. Mule feeds this data into the DataMapper transformer inside the `Batch Commit` batch scope. DataMapper transforms the input into a CSV file; then, a link:/mule-user-guide/file-connector[file connector] writes the resulting CSV file to disk. A link:/documentation/display/33X/Logger+Component+Reference[logger component] logs processed records to the console.
...
|===
|Parameter |Value |Properties Editor Image
|*Display Name* |`Database` .5+|image:database-connector-examples-7.png[image]
|*Config Reference* |`MySQL_Configuration`
|*Operation* |`Select`
|*Query Type* |`Parameterized`
...
image:database-connector-examples-8.png[image]
...
image:database-connector-examples-9.png[image]
...
image:database-connector-examples-10.png[image]
...
image:database-connector-examples-11.png[image]
...
image:database-connector-examples-12.png[image]
...
image:database-connector-examples-13.png[image]
...
image:database-connector-examples-14.png[image]
...
image:database-connector-examples-15.png[image]
...
image:database-connector-examples-16.png[image]
...
* Learn more about link:/mule-user-guide/datasense[DataSense] and how you can use it to retrieve metadata on remote applications.

* Delve deeper into link:/mule-user-guide/batch-processing[batch processing] to gain a deeper understanding of how Mule handles messages in batch jobs.
