...
+
image:fips-140-2-compliance-support-1.png[image]
+
...
+
image:fips-140-2-compliance-support-4.png[image]
+
. Save.

When Mule next launches, the startup readout indicates that FIPS security mode is enabled.

image:fips-140-2-compliance-support-5.png[image]
...
link:/mule-user-guide/anypoint-enterprise-security[Anypoint Enterprise Security] version 1.3 supports FIPS security mode. When in FIPS security mode, Mule will not register a security provider, allowing the module to rely on the platform security provider.
