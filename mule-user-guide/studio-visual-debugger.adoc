...
To do this, you set a *breakpoint* at any building block in your flow that you wish to check or test (see imagestudio-visual-debugger-1.png[Mule Expressions] against the message.
...
image:studio-visual-debugger-1.jpeg[image]
...
+
image:studio-visual-debugger-2.png[image]
+

. Studio applies a red dot to the building block's icon on the canvas. +

+
image:studio-visual-debugger-3.png[image]
+
...
. In the *Package Explorer* pane, right-click your application, then select *Debug As* > *Mule Application*. Studio begins running the application in Debug mode, and displays the *Confirm Perspective Switch* window. +
 image:studio-visual-debugger-4.png[image] +
...
image:studio-visual-debugger-5.png[image]
...
image:studio-visual-debugger-6.png[image]

The left pane displays the message payload (in this case, an HTTP GET request received via an link:/mule-user-guide/http-connector[HTTP Connector]). The right pane displays various metadata associated with the message:
...
image:studio-visual-debugger-7.png[image]
...
image:studio-visual-debugger-8.png[image]
...
image:studio-visual-debugger-9.png[image]
...
image:studio-visual-debugger-10.png[image]
...
|10 |image:studio-visual-debugger-11.png[image] |*Terminate –* Click to stop the currently running application. | 

|11
|image:studio-visual-debugger-12.png[image] |*Evaluate Mule Expression –* Click to evaluate a Mule expression. a|
...
|12 |image:studio-visual-debugger-13.png[image] |*Next Processor –* Click to run the application and stop at the next message processor in the flow, even if there is no breakpoint at the next processor. |`F6`

|13
|image:studio-visual-debugger-14.png[image] |*Run to Processor –* Click to run the application and stop at the currently selected processor, even if there is no breakpoint at the currently selected processor. Emulates the behavior of the "Run to cursor" option in Java debuggers. (To select a processor, single-click its building block in the canvas.) |`F7`

|14
|image:studio-visual-debugger-15.png[image] |*resume -* Click to run the application until the next breakpoint, if any. | `F8`
|===
...
. Click the *Evaluate Mule Expression* icon  above the right-hand pane in the Mule Debugger View. Studio displays the expression evaluation window (with yellow background in the imagestudio-visual-debugger-16.png[image]
...
image:studio-visual-debugger-16.png[image]
...
image:studio-visual-debugger-17.png[image]

In the imagestudio-visual-debugger-18.png[<message processor number>] - <flow name>`.
...
image:studio-visual-debugger-18.png[image]

image:studio-visual-debugger-19.png[image]

==== Configuring Conditional Breakpoints

A conditional breakpoint is activated only if a user-defined condition evaluates to _true_ when Studio executes the building block with the breakpoint. You define the condition using a link:/mule-user-guide/mule-expression-language-mel[Mule Expression].
...
image:studio-visual-debugger-20.png[image]
...
image:studio-visual-debugger-21.png[image]
...
* Remote debugging does not work with link:/mule-user-guide/mule-high-availability-ha-clusters[clusters].
====
...
Before accessing and debugging your application on a remote server, you must first link:/mule-management-console/deploying-applications[export and deploy] your application to the desired Mule ESB server.
...
* Explore link:/mule-user-guide/datasense[DataSense], another tool to facilitate development of applications with fewer bugs at runtime
* Use Studio, including Studio Visual Debugger, as an link:/mule-user-guide/studio-in-eclipse[Eclipse plugin].
* Learn more about setting link:/mule-user-guide/business-events[Business Events] in your Enterprise Edition Studio application.
