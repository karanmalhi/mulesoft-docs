...
] http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_sftp/namespace-overview.html[SchemaDoc] |image:sftp-transport-reference-1.png[image] |one-way, request-response |one-way |org.mule.transport:mule-transport-sftp
|===
...
. Create the **sftp.properties** properties file in your Classpath or set your PATH variable to the file's location. For information on specifying SFTP server access information for a username, password, host, and port, using Anypoint Studio, see link:/mule-user-guide/sftp-connector[SFTP Connector]. 
...
  See link:/mule-user-guide/transports-reference[transport matrix].
