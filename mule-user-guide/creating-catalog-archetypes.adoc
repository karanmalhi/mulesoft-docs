...
link:/mule-user-guide/pattern-based-configuration[Configuration patterns] are elements that simplify configuring specific and recurring activities. Mule comes with some link:/mule-user-guide/using-mule-configuration-patterns[ready made patterns] that cover generic scenarios.
...
* *si*: The pattern receives messages from a single inbound endpoint. It can optionally be configured with inbound transformers. The link:/mule-user-guide/simple-service-pattern[Simple Service] pattern is of this kind.

* *siso*: The pattern receives messages from a single inbound endpoint and dispatches to a single outbound endpoint. The link:/mule-user-guide/bridge-pattern[Bridge], link:/mule-user-guide/validator-pattern[Validator], and link:/mule-user-guide/proxying-web-services[Web Service Proxy] patterns are of this kind.
