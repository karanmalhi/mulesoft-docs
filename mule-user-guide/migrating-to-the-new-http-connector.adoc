...
If you are upgrading your projects to Mule 3.6 Runtime from an older version, one of the main adjustments you should consider doing is to replace any instances of the old link:/mule-user-guide/http-transport-reference[HTTP endpoint-based Connector] for the new high-performance link:/mule-user-guide/http-connector[HTTP operation-based Connector]. The old HTTP endpoint-based Connector still does work with Mule 3.6 Runtime, but it's deprecated and will be eventually removed in future versions.
...
The new HTTP Connector has different syntax from the old one. Besides a different name and different attributes, the new connector must always reference a link:/mule-fundamentals/global-elements[global element], this is consistent with how most of the other connectors in Mule work. Multiple connectors can reference one single global element that encapsulates many of the common configuration settings.
...
image:migrating-to-the-new-http-connector-1.png[image]
...
In both these cases, Mule parses the request to generate a Mule Message that is much simpler to consume. https://www.mulesoft.org/documentation/display/current/HTTP+Listener+Connector#HTTPListenerConnector-TheRequestBody[Read More].
...
In both cases, Mule adds the corresponding headers to the HTTP request and builds the message body accordingly. https://www.mulesoft.org/documentation/display/current/HTTP+Request+Connector#HTTPRequestConnector-HTTPRequestBody[Read More].
...
* link:/mule-user-guide/http-request-connector[HTTP Request Connector]
====
