...
image:rest-api-examples-1.png[image]
...
The first element, an link:/mule-user-guide/http-connector[HTTP listener], listens on localhost port 8081 (the default) for incoming GET requests. Hitting the listener triggers the flow. Requests to the HTTP listener must take the form:
...
image:rest-api-examples-2.png[image]
...
image:rest-api-examples-3.png[image]
...
+
image:rest-api-examples-4.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
+
image:rest-api-examples-5.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
+
image:rest-api-examples-6.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
|*Display Name* |`File` .3+|image:rest-api-examples-7.png[image]
|*Path* |`/tmp/`
...
image:rest-api-examples-8.png[image]
...
image:rest-api-examples-9.png[image]
...
The `<query>` part of the request consists of the parameters accepted by the REST API. When the HTTP listener receives the HTTP request, the `<query>` part of the URL becomes a set of inbound properties. The HTTP lsitener passes the message to the next element in the flow, the HTTP request connector. The HTTP request connector uses a set of simple link:/mule-user-guide/mule-expression-language-mel[MEL expressions] to extract the query parameters from the message, and to construct the full URL for the remote API, including the query parameters.
...
The HTTP request connector passes the XML it received from the API to a byte-array-to-string transformer, which converts the message payload to a string. Next, a link:/mule-user-guide/splitter-flow-control-reference[splitter] splits the message into parts. The splitter is configured to split the incoming message using the XML element `<results`> as delimiter. The reason for this is that the REST API uses `<results>` to delimit the information for each stock symbol retrieved, as shown below.
...
To split the incoming XML, the splitter uses the link:/mule-user-guide/mule-expression-language-mel[Mule Expression Language] expression `#[xpath('//results')]`, which as you can see contains the http://en.wikipedia.org/wiki/XPath[XPath] expression that actually retrieves the XML element `<results>`.

The last element in the flow, a link:/mule-user-guide/jdbc-connector[Database (JDBC)] connector, receives each XML chunk corresponding to each queried symbol. The JDBC connector is configured to run the following SQL query:
...
. Set up the Properties editor of the Connector:
+
image:rest-api-examples-10.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
. Create a new Connector Configuration Element and set up the port and host:
+
image:rest-api-examples-11.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
. Create a new Connector Configuration Element and set up the *Host* and *Base Path*:
+
image:rest-api-examples-12.png[image]
+
[width="20%,cols=",",options="header"]
|===
|Parameter |Value
...
. Save changes and return to the connector's properties editor by clicking *OK*. Then set up the path and method:
+
image:rest-api-examples-13.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
+
image:rest-api-examples-14.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Byte Array to String` .5+|image:rest-api-examples-15.png[image]
|*Return Class* |`-`
|*Ignore Bad Input* |no
|*Encoding* |`-`
|*MIME Type* |`-`
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Byte Array to String` .4+|image:rest-api-examples-16.png[image]
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Database` .5+|image:rest-api-examples-17.png[image]
...
image:rest-api-examples-18.png[image]
....
[tab,title="Standalone XML"]
....
[source]
...
Note that you must install the appropriate driver for your database in your Studio application. For details, see the Adding the Database Driver section in link:/mule-user-guide/database-connector[Database Connector].
...
|===
|Parameter |Value |Configuration window image
|*Name* |`Postgres` .5+|image:rest-api-examples-19.png[image]
...
image:rest-api-examples-20.png[image]
...
image:rest-api-examples-21.png[image]
...
image:rest-api-examples-22.png[image]
...
The HTTP Request Connector passes the JSON it received from the API to a link:/mule-user-guide/json-module-reference[JSON-to-object transformer] configured to output a java.util.Map object. The transformer sends this object, which contains the JSON data as key=value pairs, to the last element in the flow, a link:/mule-user-guide/database-connector[database connector]. This connector uses an SQL query with embedded link:/mule-user-guide/mule-expression-language-mel[Mule Expression Language] expressions to extract specific values from the JSON and insert them into the external database. The full SQL query is shown below.
...
image:rest-api-examples-23.png[image]
...
image:rest-api-examples-24.png[image]
...
. Create a new Connector Configuration Element and set up the *Host* and *Base Path*:
+
image:rest-api-examples-25.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
. Save changes and return to the connector's properties editor by clicking ok. Then set up the path and method:
+
image:rest-api-examples-26.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
. Click the *Add Parameter* button three times to add three parameters. Leave all as the default type **query-param**. For the `value` field of each, write a mule expression that takes the equivalent value from the query parameters of the request that first reached the HTTP Listener, these are transformed into inbound properties by the time they reach the HTTP Request Connector. 
+
image:rest-api-examples-27.png[image]
+
[width="20%",cols=",",options="header"]
|===
|Parameter |Value
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`JSON to Object` .2+|image:rest-api-examples-28.png[image]
|*Mapper Reference* |`-`
|===
...
|===
|Parameter |Value |Configuration window image
|*Return Class* |`java.util.Map` .3+|image:rest-api-examples-29.png[image]
|*Ignore Bad Input* |no
|*Encoding* |`-`
|*MIME Type* |`-`
|===
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Database` .4+|image:rest-api-examples-30.png[image]
|*Connector configuration* |`Postgres`
|*Operation* |`insert`
|*Query Type* |`Dynamic`
|*Dynamic query* a|
...
image:rest-api-examples-31.png[image]
....
[tab,title="XML Standalone"]
....
[source]
...
|===
|Parameter |Value |Configuration window image
|*Name* |`Postgres` .5+|image:rest-api-examples-32.png[image]
|*Database URL* |`jdbc:postgresql://dbserver/stock`
|*Driver Class Name* |`org.postgresql.Driver`
|*Use XA Transactions* |No (default)
|*Enable DataSense* |Yes (default)
...
image:rest-api-examples-33.png[image]
...
image:rest-api-examples-34.png[image]
