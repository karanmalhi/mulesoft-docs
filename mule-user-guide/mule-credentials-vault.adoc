...
[NOTE]
This topic introduces the idea of Global Elements; if you are unfamiliar with this functionality, access link:/mule-fundamentals/global-elements[Understand Global Mule Elements] to learn more before proceeding.
...
image:mule-credentials-vault-1.png[image]
...
image:mule-credentials-vault-2.png[image]
...
image:mule-credentials-vault-3.png[image]
...
+
image:mule-credentials-vault-4.png[image]
. Click the image:mule-credentials-vault-5.png[(error)] in the `.properties` tab to close the Properties File Editor window.

. In the Package Explorer, right-click the `.properties` file, then select *Open With* > *Mule Properties Editor*.
. Click the green image:mule-credentials-vault-6.png[(plus)] icon in the Studio toolbar (see image below) to open the *Add a new property* dialog.
+
image:mule-credentials-vault-7.png[image]

. Add the key-value pair (property) you wish to record in the properties file. 
+
image:mule-credentials-vault-8.png[image]
...
====
+
image:mule-credentials-vault-9.png[image]
...
+
image:mule-credentials-vault-10.png[image]
...
+
image:mule-credentials-vault-11.png[image]
...
* Access the link:/mule-user-guide/anypoint-enterprise-security-example-application[example application] which demonstrate Anypoint Enterprise Security in action.
