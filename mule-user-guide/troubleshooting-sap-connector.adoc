...
This page contains some common troubleshooting suggestions for the link:/mule-user-guide/mulesoft-enterprise-java-connector-for-sap-reference[SAP Connector].
...
In this example, JCo libraries are informing that the parameter with name _type_ is not valid. The complete list of valid property names can be found link:/mule-user-guide/sap-jco-extended-properties[here].
