...
For an overview of the graphical mapping editor, see link:/mule-user-guide/datamapper-visual-reference[DataMapper Visual Reference].

To see a working example, see the link:#DataMapperConcepts-basic_ex[basic example] in this page. The DataMapper documentation also contains working examples detailing how to perform specific tasks:

[width="100%",cols=",",options="header"]
|===
|Example |Description
|link:/mule-user-guide/datamapper-flat-to-structured-and-structured-to-flat-mapping[DataMapper Flat-to-Structured and Structured-to-Flat Mapping] |Shows how to build maps that extract from and populate complex, multiple-level nested structures.
|link:/mule-user-guide/datamapper-fixed-width-input-format[DataMapper Fixed Width Input Format] |Two scenarios using DataMapper to extract data from fixed-width input files.
...
|link:/mule-user-guide/mapping-elements-inside-lists[Mapping Elements] |Describes how to map nested listed to flat and structured  data.
|===
...
+
image:datamapper-concepts-1.jpeg[image] +
+
...
+
image:datamapper-concepts-2.jpeg[image]

[TIP]
====
For a detailed explanation about what each part of the GUI is for, see link:/mule-user-guide/datamapper-visual-reference[DataMapper Visual Reference]
====
...
image:datamapper-concepts-3.png[image]
...
+
image:datamapper-concepts-4.png[image]
...
[TIP]
====
For instructions on how to do this see the link:/mule-fundamentals/anypoint-connector-tutorial[Anypoint Connector Tutorial], under the section *Obtaining Access to the Twitter API*.
====
+
...
+
image:datamapper-concepts-5.jpeg[image]
...
+
image:datamapper-concepts-6.jpeg[image]
+
...
+
image:datamapper-concepts-7.jpeg[image]
...
+
image:datamapper-concepts-8.jpeg[image]
...
+
image:datamapper-concepts-9.jpeg[image]
+
...
Once two elements are mapped, by default the input will be simply replicated into the output, but if you want you can define transformations that populate the output elements. If you want to define a transformation, you must write a link:/mule-user-guide/mule-expression-language-mel[MEL expression] that computes the output value based on the values of input elements.
...
For an illustration of this technique, see link:/mule-user-guide/mapping-elements-inside-lists[Mapping Elements Inside Lists]. 
...
[NOTE]
====
The specifics of using the DataMapper UI to create a mapping flow like this one are covered in link:/mule-user-guide/datamapper-visual-reference[DataMapper Visual Reference].
====

image:datamapper-concepts-10.png[image]
...
image:datamapper-concepts-11.png[image]
...
image:datamapper-concepts-12.png[image]

image:datamapper-concepts-13.png[image]
...
image:datamapper-concepts-14.jpeg[image]
...
For more information on Rules, see "Using Rules to Extract Fields from Complex Structured Data" in link:/mule-user-guide/building-a-mapping-flow-in-the-graphical-mapping-editor[Building a Mapping Flow in the Graphical Mapping Editor] . 
...
For more details on Filters, see  "Setting an Element Mapping XPath Filter" in link:/mule-user-guide/building-a-mapping-flow-in-the-graphical-mapping-editor[Building a Mapping Flow in the Graphical Mapping Editor]. 
...
For more on input and output arguments, see the discussion of "Using Data Mapping Input and Output Arguments" in link:/mule-user-guide/building-a-mapping-flow-in-the-graphical-mapping-editor[Building a Mapping Flow]. For more on lookup tables, see "Using Lookup Tables" in link:/mule-user-guide/building-a-mapping-flow-in-the-graphical-mapping-editor[Building a Mapping Flow].
...
* For a more detailed explanation about what each part of the GUI does, see link:/mule-user-guide/datamapper-visual-reference[DataMapper Visual Reference]
