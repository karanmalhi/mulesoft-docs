...
image:mule-deployment-model-1.png[image]

If you're deploying multiple applications to the same place, and those applications could share the same resources, then you can create a common *domain* where you can define common configurations that can then be referenced by multiple projects. This allows you to, for example, expose different services in different projects through the same HTTP host and port and be able to deploy everything without any conflicts. link:/mule-user-guide/shared-resources[Read More].

Understanding the deployment model can be divided into several topics:

* link:/mule-user-guide/hot-deployment[Hot Deployment]
* link:/mule-user-guide/application-deployment[Application Deployment]
* link:/mule-user-guide/application-format[Application Format]
