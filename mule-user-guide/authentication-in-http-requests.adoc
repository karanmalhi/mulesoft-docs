...
The link:/mule-user-guide/http-request-connector[HTTP Request Connector] supports four types of authentication:
...
+
image:authentication-in-http-requests-1.png[image]
....
...
image:authentication-in-http-requests-2.png[image]
...
When this is the case, the parameters will be automatically extracted and you can easily use link:/mule-user-guide/mule-expression-language-mel[MEL expressions] later in the flow to reference these values in the Mule Message that was generated from the response to the request to the token URL.
...
It’s possible to customize when Mule will perform one of these requests to obtain a new access token. This is configured through a link:/mule-user-guide/mule-expression-language-mel[MEL Expression] that is evaluated against the Mule Message that is generated from the response of the HTTP Request Connector call.
...
image:authentication-in-http-requests-3.png[image]
...
By default, the token manager uses an in-memory object store to store the credentials. You can customize the token manager object store by using the objectStore-ref attribute. link:/mule-user-guide/mule-object-stores[See how to configure a custom object store].
...
*  link:/mule-user-guide/migrating-to-the-new-http-connector[Migrating to the New HTTP Connector]
