...
[TIP]
Our default and recommended XA Transaction Manager is *Bitronix*, read link:/mule-user-guide/using-bitronix-to-manage-transactions[Using Bitronix to Manage Transactions] for details on how to implement it in Mule.
...
This property is only applicable if you are using the the default transport, `XaTransactedJmsMessageReceiver` , on JMS inbound connectors configured to use XA transactions. If you are using JBoss transactions, please read link:/mule-user-guide/jboss-transaction-manager-reference[JBoss Transaction Manager Reference] to learn how to configure the `timeout` value.
...
|JBoss |  |image:xa-transactions-1.png[java:/TransactionManager] a|

----
org.mule.transaction.lookup.JBossTransactionManagerLookupFactory
----

|JRun |  |image:xa-transactions-1.png[java:/TransactionManager] a|
...
|Other |  |image:xa-transactions-1.png[(tick)] |Specified via a `jndiName` property a|
...
|Resin |  |image:xa-transactions-2.png[java:comp/TransactionManager] a|
...
|WebLogic |image:xa-transactions-2.png[(tick)] |javax.transaction.TransactionManager a|
...
|WebSphere |  |image:xa-transactions-2.png[(tick)] |proprietary API call a|
...
* Refer to link:/mule-user-guide/using-bitronix-to-manage-transactions[Using Bitronix to Manage Transactions] to see how to implement our default and recommended XA Transaction Manager +
