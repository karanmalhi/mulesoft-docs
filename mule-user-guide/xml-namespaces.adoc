...
You can specify the namespace on any XML-based functionality in Mule, including the JXPath filter, Jaxen filter, XPath filter, filter-based splitter, expression splitter, round-robin splitter, JXPath extractor transformer, and XPath expression transformer in the link:/mule-user-guide/xml-module-reference[XML Module Reference] and link:/mule-user-guide/xpath-annotation[XPath Annotation].
