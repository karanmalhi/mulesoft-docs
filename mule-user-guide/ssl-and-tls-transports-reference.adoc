...
* The link:/mule-user-guide/tcp-transport-reference[TCP Transport], which uses the basic TCP transport.
...
* Communicating with an external service that uses low-level unsecured TCP connections? If so, use the link:/mule-user-guide/tcp-transport-reference[TCP] protocol.

* Are you communicating with a flow always located in the same Mule application instance? If so, consider using the link:/mule-user-guide/vm-transport-reference[VM transport].
