...
|Reference to a strategy |Select to configure the cache scope to follow a link:#CacheScope-CreatingaGlobalCachingStrategy[global caching strategy] that you have created; select the global caching strategy from the drop-down menu or create one by clicking the image:cache-scope-3.png[(plus)]. |`cachingStrategy-ref="Caching_Strategy"`
...
. In the Cache properties editor, click radio button next to the *Reference to a strategy field*, then click **image:cache-scope-4.png[(plus)]**.
...
|Key Generator |(Optional) Identify a custom-built Spring bean that generates a key. Use when request classes do not return the same SHA256 hashcode for objects that represent the same value. If you have not created any custom key generators, the *Key Generator* drop-down box will be empty. Click image:cache-scope-6.png[(plus)] next to the field to create one. |`keyGenerator-ref="Bean"`
|===
+
...
+
image:cache-scope-7.png[image]
+
[width="100%",cols=",",options="header"]
|===
|Field| Value |Description |XML
|*Response Generator* | |Specify the name of a Response Generator that will direct the cache strategy to use a custom-built Spring bean to generate a response that combines data from both the new request and the cached response. Click image:cache-scope-8.png[(plus)] next to the field to create a new Spring bean for your caching strategy to reference. |`responseGenerator-ref="Bean1"`
|*Consumable Message Filter* | |Specify the name of a Consumable Message Filter to direct the cache strategy to use a custom-built Spring bean to detect whether a message contains a consumable payload. Click image:cache-scope-9.png[(plus)] next to the field to create a new Spring bean for your caching strategy to reference. |`consumableFilter-ref="Bean2"`
...
|*custom-object-store*
.4+|See link:#CacheScope-ConfiguringAnObjectStoreforCache[Configuring An Object Store for Cache], below
|*in-memory-store*
|*managed-store*
|*simple-text-file-store*
|===
...
By default, Mule stores all cached responses in an InMemoryObjectStore. link:#CacheScope-CreatingaCachingStrategy[Create a caching strategy] and define a new link:/mule-user-guide/mule-object-stores[object store] if you want to customize the way Mule stores cached responses.
...
+
image:cache-scope-10.png[image]
...
image:cache-scope-11.png[image]

image:cache-scope-12.png[image]

image:cache-scope-13.png[image]

image:cache-scope-14.png[image]
...
* Read more about link:/mule-user-guide/scopes[Scopes] in Mule.
