...
In Studio, to instance the connector as an HTTP Listener Connector (and not an link:/mule-user-guide/http-request-connector[HTTP Request Connector]), you must place it into the *Source* section of a flow (ie: as the first element in the flow):

image:http-listener-connector-1.png[image]
...
image:http-listener-connector-2.png[image]
...
image:http-listener-connector-3.png[image]
...
image:http-listener-connector-4.png[image]
...
+
image:http-listener-connector-5.png[image]
...
+
image:http-listener-connector-6.png[image]
...
+
image:http-listener-connector-7.png[image]
...
image:http-listener-connector-8.png[image]
...
----
+
image:http-listener-connector-9.png[image]
...
image:http-listener-connector-10.png[image]
...
image:http-listener-connector-11.png[image]
...
image:http-listener-connector-12.png[image]
...
When using an HTTP Listener Connector, whenever there are outbound properties included in the Mule Message that arrives to the end of the flow, each outbound property is transformed into an HTTP response header. It's easy to add properies to the Mule Message by using the link:/mule-user-guide/property-transformer-reference[Property Transformer].
...
In the example above, two headers will be added to every response to a request to the app. Both headers take their values from link:/mule-user-guide/mule-expression-language-date-and-time-functions[dateTime MEL expressions].
...
. Set the variable's name to `headersOut` and its value to a map through the following link:/mule-user-guide/mule-expression-language-mel[MEL expression]:  `#[['date': server.dateTime, 'expires' : server.dateTime.plusHours(1)]]`
...
+
image:http-listener-connector-13.png[image]
...
See link:/mule-user-guide/tls-configuration[TLS Configuration] for more details.
...
See link:/mule-user-guide/configuring-the-spring-security-manager[Configuring the Spring Security Manager] for more details.
...
* To send HTTP requests, see  link:/mule-user-guide/http-request-connector[HTTP Request Connector]
*  link:/mule-user-guide/consuming-a-rest-api[Consuming a REST API]  
*  link:/mule-user-guide/rest-api-examples[REST API Examples]
* link:/mule-user-guide/authentication-in-http-requests[Authentication in HTTP Requests] +
* To update allplications that use the old HTTP endpoint based connector, see link:/mule-user-guide/migrating-to-the-new-http-connector[Migrating to the New HTTP Connector]
* Consult a reference to the deprecated predecessor of this element,  link:/mule-user-guide/http-transport-reference[HTTP Transport Reference]
