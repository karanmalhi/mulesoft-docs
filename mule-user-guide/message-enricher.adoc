...
You can make the source a namespace by simply populating the field with a link:/mule-user-guide/mule-expression-language-mel[Mule Expression Language MEL] expression, for example with an xpath expression that takes arguments from the Mule message.
