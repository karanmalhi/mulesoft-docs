...
Other namespaces can be introduced via `<spring:beans>` or by adding a dedicated element in a module (see the link:/mule-user-guide/scripting-module-reference[Scripting module's] `<lang>` element).
