...
. The Mule URL endpoint: any valid link:/mule-user-guide/mule-endpoint-uris[Mule Endpoint] used to determine the transport, endpoint, and other information about delivery of the message. This can also be an endpoint name stored in configuration.
...
For SOAP support, you can use the Mule link:/mule-user-guide/cxf-module-reference[CXF Module Reference].
...
If you do not set the wire format, the serialization format is used. For more information on transformers, see link:/mule-user-guide/using-transformers[Using Transformers].
