...
[TIP]
====
Should you need to encrypt passwords in your properties file, consider using the link:/mule-user-guide/mule-credentials-vault[Mule Credentials Vault]. Refer to the documentation for link:/mule-user-guide/anypoint-enterprise-security[Anypoint Enterprise Security] for more information on securing applications in Mule.
====

== Assumptions

This document assumes that you are familiar with the exercise of building and deploying Mule applications in Mule Studio. Consider reading the link:/mule-fundamentals/[Mule Fundamentals] and link:/mule-fundamentals/first-30-minutes-with-mule[First 30 Minutes with Mule] for more information.
...
+
image:deploying-to-multiple-environments-1.png[image]
...
+
image:deploying-to-multiple-environments-2.png[image]
+
[TIP]
====
Learn how to encrypt a properties file using the link:/mule-user-guide/mule-credentials-vault[Mule Credentials Vault].
====
...
[TIP]
====
Learn more about link:/mule-user-guide/configuring-properties[configuring variable properties].
====

. To configure Studio to use a specific environment when  *deploying on-premise* , *debugging*  or  *using link:/mule-user-guide/datasense[DataSense]*  to retrieve metadata from a SaaS provider, double-click to open your application's  `mule-project.xml` file, located in the root directory of your project.
+
[WARNING]
====
...
+
image:deploying-to-multiple-environments-3.png[image]
...
** `production.properties`
+
image:deploying-to-multiple-environments-4.png[image]

. Double-click each of these new files to open them in new tabs in Studio. By editing hem, you can configure the properties of the environment that correspond to the filename. For example, in `production.properties`, you may wish to add the properties as per the image below.  Keep in mind that you can use these properties anywhere in your application.
+
image:deploying-to-multiple-environments-5.png[image]
+
[TIP]
====
Learn how to encrypt a properties file using the link:/mule-user-guide/mule-credentials-vault[Mule Credentials Vault].
...
.  Add an environment variable named *`mule.env`* with a value that matches the environment to which you wish to deploy by default (in the image below, the value is `qa`).
+
image:deploying-to-multiple-environments-6.png[image]
...
+
image:deploying-to-multiple-environments-7.png[image]
...
* Learn more about encrypting a properties file using the link:/mule-user-guide/mule-credentials-vault[Mule Credentials Vault].

* To learn more about deploying to test environments in CloudHub, access the CloudHub Sandbox Environments documentation.

* Learn more about link:/mule-user-guide/configuring-properties[Properties Placeholders] in Mule. 

* Learn how to proceed when deploying multiple applications with link:/mule-user-guide/shared-resources[Shared Resources] +
