...
+
[NOTE]
JDBC Inbound Endpoints can only be set to a one-way exchange pattern. JDBC Outbound Endpoints can be set to either a one-way or request-response exchange pattern. For details about exchange patterns, see link:/mule-fundamentals/anypoint-studio-essentials[Anypoint Studio Essentials].
...
image:jdbc-connector-1.png[image]
...
|*SQL* *Statement* |See link:#JDBCConnector-SQLStatement[SQL Statement] for details.
...
image:jdbc-connector-2.png[image]
...
+
image:jdbc-connector-3.png[image]

=== Advanced Tab

image:jdbc-connector-4.png[image]
...
image:jdbc-connector-5.png[image]
...
image:jdbc-connector-6.png[image]
...
For details on setting the properties for a JDBC endpoint using an XML editor, consult the link:/mule-user-guide/jdbc-transport-reference[JDBC Transport Reference].
