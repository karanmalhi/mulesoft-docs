...
. Install link:http://www.oracle.com/technetwork/java/javase/downloads/java-archive-downloads-javase7-521261.html[JDK 7] for DevKit 3.6 or 3.7. DevKit 3.7 also supports link:http://www.oracle.com/technetwork/java/javase/downloads/index.html[JDK 8].
...
. Install the link:http://www.mulesoft.org/documentation/display/current/Setting+Up+Your+Dev+Environment#SettingUpYourDevEnvironment-DevKitPlugin[Anypoint DevKit Plugin].
...
. Create an instance of link:http://www.mulesoft.org/documentation/display/current/Functional+Testing#FunctionalTesting-FunctionalTestCase[FunctionalTestCase] to run the test. For DevKit testing, run Mule inside a JUnit test case. Each test case you build extends the class link:http://www.mulesoft.org/documentation/display/current/Functional+Testing#FunctionalTesting-FunctionalTestCase[FunctionalTestCase] (a subclass of JUnit). The test case manages the lifecycle of a Mule runtime instance and calls flows to carry out the test. 
