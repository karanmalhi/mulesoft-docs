...
For OAuth-based authentication DevKit provides functionality using OAuth-related annotations. See link:http://www.mulesoft.org/documentation/display/current/OAuth+V1[OAuth V1] and link:http://www.mulesoft.org/documentation/display/current/OAuth+V2[OAuth V2] for details.
...
In the Example above, the username parameter in the `@Connect` method is annotated with `@ConnectionKey`. If pools are enabled (see link:http://www.mulesoft.org/documentation/display/current/Basic+Auth#BasicAuth-ConnectionPooling[Connection Pooling] below), Mule keeps a pool of simultaneous connections which are used as needed to make calls.
