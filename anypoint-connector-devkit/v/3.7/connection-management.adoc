...
In the Example above, the username parameter in the `@Connect` method is annotated with `@ConnectionKey`. If pools are enabled (see link:/anypoint-connector-devkit/v/3.7/basicauth[Connection Pooling] below), Mule keeps a pool of simultaneous connections which are used as needed to make calls.
