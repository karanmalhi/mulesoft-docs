...
For OAuth-based authentication DevKit provides functionality using OAuth-related annotations. See link:/anyoint-connector-devkit/v/3.7/oauth-v1[OAuth V1] and link:/anyoint-connector-devkit/v/3.7/oauth-v2[OAuth V2] for details.
...
In the Example above, the username parameter in the `@Connect` method is annotated with `@ConnectionKey`. If pools are enabled (see link:/anyoint-connector-devkit/v/3.7/basicauth[Connection Pooling] below), Mule keeps a pool of simultaneous connections which are used as needed to make calls.
