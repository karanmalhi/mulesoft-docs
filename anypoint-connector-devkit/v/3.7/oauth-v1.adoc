...
NOTE:
For more information on configuring an *HTTPS connector*, see the link:/mule-user-guide/v/3.7/https-transport-reference[HTTPS Transport Reference] and link:http://www.mulesoft.org/documentation/display/current/TLS+Configuration#TLSConfiguration-Examples(forHTTPS)[Examples for HTTPS].
