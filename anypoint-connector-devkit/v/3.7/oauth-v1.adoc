...
You must also arrange access to the API.  The process is specific to each API provider and the typical steps for this process are described in link:/documentation/display/current/Setting+Up+API+Access[Setting Up API Access]. 
...
[NOTE]
For more information on configuring an *HTTPS connector,* see the link:/documentation/display/current/HTTPS+Transport+Reference[HTTPS Transport Reference] and http://www.mulesoft.org/documentation/display/current/TLS+Configuration#TLSConfiguration-Examples(forHTTPS)[Examples for HTTPS].
