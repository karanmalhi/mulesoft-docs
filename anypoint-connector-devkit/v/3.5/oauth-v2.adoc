...
image:/docs/download/attachments/122751291/5-package.png?version=1&modificationDate=1421451000498[image]
...
[TIP]
For details on configuring an *HTTPS connector*, consult the link:/docs/display/35X/HTTP+Transport+Reference[HTTPS Transport Reference].

== See Also

* *NEXT:* Continue to link:/docs/display/35X/Defining+Connector+Attributes[Defining Attributes, Operations and Data Model] for your connector.
* See the link:/docs/display/35X/Creating+a+Connector+for+a+RESTful+API+using+@RESTCall+Annotations[Creating a Connector for a RESTful API using @RESTCall Annotations] example to see the OAuth V2 Authentication annotations in a fully working example.
