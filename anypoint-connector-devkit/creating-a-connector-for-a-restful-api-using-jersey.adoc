...
* This sample implements OAuth1 authentication. Find a more detailed discussion of implementing OAuth1 support (with reference to the same sample code) in link:/anypoint-connector-devkit/oauth-v1[OAuth V1]. Implementing OAuth2 support is similar; see link:/anypoint-connector-devkit/oauth-v2[OAuth V2] for more details.
...
image:creating-a-connector-for-a-restful-api-using-jersey-1.png[image]
...
* The use of `@Optional` and `@Default` with `@Configurable`. For more on these annotations, see link:/anypoint-connector-devkit/defining-connector-attributes[Defining Connector Attributes] and link:/anypoint-connector-devkit/annotation-reference[Annotation Reference].
...
For a detailed discussion on the code changes required for OAuth support, seelink:/anypoint-connector-devkit/oauth-v1[Implementing OAuth 1.0 Authentication], which references the same code sample used in this discussion.
...
The process is described in link:/anypoint-connector-devkit/installing-and-testing-your-connector-in-studio[Installing and Testing Your Connector]. 
...
image:creating-a-connector-for-a-restful-api-using-jersey-2.png[image]
...
* Add tests to your test suite as described in link:/anypoint-connector-devkit/developing-devkit-connector-tests[Developing DevKit Connector Tests]. 
* You can also return to the link:/anypoint-connector-devkit/anypoint-connector-development[Anypoint Connector Development].
