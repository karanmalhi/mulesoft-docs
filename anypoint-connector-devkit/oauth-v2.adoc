...
This document assumes you are familiar with the  link:/mule-fundamentals/anypoint-studio-essentials[Anypoint Connector Essentials]  and are ready to implement authentication on your connector. It assumes you are familiar with the various  link:/anypoint-connector-devkit/authentication-methods[authentication methods]  and may be using OAuth V1 to authenticate with your API.

If you are not familiar with OAuth V1 authentication, what it is or how it works, we recommend visiting http://tools.ietf.org/html/rfc5849[OAuth 1.0 protocol/RFC-5849]. Also see link:/anypoint-connector-devkit/oauth-v1[OAuth V1].
...
* link:/mule-user-guide/http-transport-reference[HTTPS Transport Reference]
