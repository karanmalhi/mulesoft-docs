...
This document summarizes typical preparation steps you might need to perform, then presents examples for two cases: link:#SettingUpAPIAccess-SalesforceExample[Salesforce] and link:#SettingUpAPIAccess-TwitterExample[Twitter].
...
+
image:setting-up-api-access-1.png[image]
+
...
** Consumer key
** Consumer secret
. For detailed instructions on creating an application on the Twitter platform, see link:/cloudhub/configure-twitter-for-api-access[Configure Twitter for API Access] .
...
*NEXT:* Continue to link:/anypoint-connector-devkit/creating-an-anypoint-connector-project[Creating an Anypoint Connector Project] to begin developing your connector.
