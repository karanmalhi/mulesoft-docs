...
For OAuth-based authentication DevKit provides functionality using OAuth-related annotations. See link:/anypoint-connector-devkit/oauth-v1[OAuth V1] and link:/anypoint-connector-devkit/oauth-v2[OAuth V2] for details.

== Prerequisites

This document assumes you are familiar with the link:/anypoint-connector-devkit/[Anypoint Connector DevKit], have already link:/anypoint-connector-devkit/creating-an-anypoint-connector-project[created a connector project], and are ready to implement authentication on your connector. Further, it assumes you are familiar with the various link:/anypoint-connector-devkit/authentication-methods[authentication methods], have compared the different link:/anypoint-connector-devkit/authentication[authentication support options in Devkit], and chosen Basic Authentication to connect to your API.
...
The specific implementation of these methods depends on the target. For more information on annotations, see the link:/anypoint-connector-devkit/annotation-reference[Annotation Reference].
