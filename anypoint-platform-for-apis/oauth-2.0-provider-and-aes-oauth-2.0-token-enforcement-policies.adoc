...
image:oauth-2.0-provider-and-aes-oauth-2.0-token-enforcement-policies-1.png[image]
...
This document also assumes that you are familiar with OAuth 2.0. If you need a refresher, read this link:/mule-user-guide/mule-secure-token-service[overview].
...
|*Grant Type(s)** a|
Select one or more of the following link:/mule-user-guide/authorization-grant-types[grant types] to enable them:
