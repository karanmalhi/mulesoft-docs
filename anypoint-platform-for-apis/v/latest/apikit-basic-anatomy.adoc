...
[TIP]
====
When creating a new Mule project in Anypoint Studio, you can import a RAML file from your local disk, via a direct URL to an address that contains a RAML file, or from an API in your Anypoint Platform account.

When importing from an API, Studio also imports any dependencies that your RAML file may have. You can then keep both versions in sync by pushing or pulling changes to the API definition in your account.
====
