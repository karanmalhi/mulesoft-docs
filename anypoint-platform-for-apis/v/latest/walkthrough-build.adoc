...
+
Click the 'Preview' button to display a  now matches the desired output:
////The step to generate the sample XML input data is not in here and doesnt work as expected in Mule 3.7.3////
+
...
////This is not the case in the new DataWeave GUI, Drag and Drop flowvars.orderId on orderId and http.query.params.email on email////
...
////This is not the case in the new DataWeave GUI, Drag and Drop Payload: Json on XML<OrderTshirt> choose 'auto assign'fields////
+
[source,dataweave,linenums]
----
%dw 1.0
