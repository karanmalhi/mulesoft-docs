...
.. <<Send and Receive a Message for a Queue>> - Send the demo
queue a message and then get the message.
.. <<Send a Message to an Exchange>> - Send the demo
exchange a message and then get the message from the demo queue.
...
=== Send and Receive a Message for a Queue
...
This section builds on the last. To get a message from a queue,
