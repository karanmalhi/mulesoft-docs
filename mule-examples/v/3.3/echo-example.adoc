...
image:/docs/download/attachments/87687916/responses.png?version=1&modificationDate=1348167913915[image]
...
The *Echo* example application consists of one, simple link:#[flow] which receives end user HTTP requests, logs messages’ payloads, and returns the payloads to end users as HTTP responses.
...
In Mule Studio, double-click an icon representing a link:#[building block] to open its *Properties Panel* and examine the configuration details. Alternatively, you can click the *Configuration XML* tab to access and examine the application’s XML config file.

image:/docs/download/attachments/87687916/config_tab.png?version=1&modificationDate=1348167937623[image]
====

=== EchoFlow

image:/docs/download/attachments/87687916/echoFlow.png?version=1&modificationDate=1348167965096[image]
...
image:/docs/download/attachments/87687916/http_endpoints.png?version=1&modificationDate=1348167984625[image]
====
...
Next, the flow uses a **link:/docs/display/33X/Logger+Component+Reference[Logger Component]** to log the message payload in the application’s log files. The logger component uses an** link:/docs/display/33X/Mule+Expression+Language+MEL[Expression]** to determine what information in, or about, the message that it should log. In this case, because it needs to log the message payload, the instructions to log read `About to echo #[message:payload]`. +
 +
 image:/docs/download/attachments/87687916/logger_properties.png?version=1&modificationDate=1348168006666[image]
