...
The application contains two link:/docs/display/33X/Mule+Application+Architecture[flows] , which work together to process orders for t-shirts. The sections below offer flow-by-flow descriptions of the Excel to JSON application’s actions as it processes t-shirt orders.
