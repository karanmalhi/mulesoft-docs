...
This example should help you to make informed decisions about using the four above-listed elements in your Mule applications. To understand more about Mule ESB’s ability to integrate services and systems, access the Mule link:/docs/display/33X/Mule+Examples[examples] and see other applications in action.
...
. Complete the procedure in link:/docs/display/33X/Mule+Examples[Examples and Exercises] to create, then run the Loan Broker template in Mule Studio, or the Loan Broker example in Mule Standalone (Mule ESB without Studio).
...
+
image:/docs/download/attachments/87687920/submitted_m.png?version=1&modificationDate=1346340590275[image]
...
+
image:/docs/download/attachments/87687920/submitted2_m.png?version=1&modificationDate=1346340608571[image]

== How it Works

The Loan Broker application consists of several link:/docs/display/33X/Mule+Application+Architecture[flows and subflows]. These flows integrate Web services to process end user requests for interest rate quotes from banks.
...
image:/docs/download/attachments/87687920/config_tab.png?version=1&modificationDate=1346340630834[image]
====
...
image:/docs/download/attachments/87687920/loan_sync_all.png?version=1&modificationDate=1346340650854[image]

// View the XML

The request-response **link:/docs/display/33X/HTTP+Endpoint+Reference[HTTP Inbound Endpoint] **in this flow receives an end user request. Because it has a two-way message exchange pattern, this HTTP endpoint is responsible for both receiving and returning messages.
...
image:/docs/download/attachments/87687920/http_endpoints.png?version=1&modificationDate=1346340671356[image]
====
...
Then, Mule employs a content-based router to direct the message for further processing. The **link:/docs/display/33X/Choice+Flow+Control+Reference[Choice Flow Control] **routes each message to one of two processing pathways according to its payload contents.
...
+
image:/docs/download/attachments/87687920/choice_first_route2.png?version=1&modificationDate=1346340691249[image]
...
With a new `CustomerQuoteRequest` object in its payload, the message encounters its first **link:/docs/display/33X/Message+Enricher[Message Enricher]**. Throughout this flow, Mule _enriches_ messages with data rather than changing the payload contents. By enriching a message, Mule preserves the payload content so that other elements in the application can access the original data.

The `Enrich with creditProfile` enricher contains only a **link:/docs/display/33X/Flow+Ref+Component+Reference[Flow Reference Component]**. This type of component invokes other flows, or subflows, in the application to acquire, then add data to the message. In this case, the `lookupCustomerCreditProfile` component demands that the LookupCustomerCreditProfile subflow access an external Web service to acquire the borrower’s credit score. Mule enriches the message with the credit score, then passes the message to the next enricher in the flow.
...
Mule then uses a **link:/docs/display/33X/Variable+Transformer+Reference[Variable Transformer]** to create an empty list variable. Mule will fill this empty `quotes` list variable with the quotes it fetches from banks. With an empty list to fill, the message next encounters a *link:/docs/display/33X/Foreach[Foreach]* scope. One by one, this iterative processor fetches data to populate each item on the list.

To fetch these data, the flow reference component first invokes the LookupLoanQuote subflow to acquire a quote from a bank. Then, the message enricher adds the quote to the list variable. Foreach continues to invoke, then enrich, until it has acquired a quote from each bank on the list of banks. Foreach then passes the message to the next **link:/docs/display/33X/Mule+Studio+Essentials[message processor]** in the flow.
...
Notice that the Loan-broker-sync flow also contains a **link:/docs/display/33X/Catch+Exception+Strategy[Catch Exception Strategy]**. Rather than use Mule’s link:/docs/display/33X/Error+Handling[default exception strategy] this flow uses a customized exception strategy to handle errors. If an error occurs in the flow, the exception strategy’s *Set Payload Transformer* sets an error message on the payload. The application sends this error message, which reads, `Error processing loan request`, as a response to the end user.
...
image:/docs/download/attachments/87687920/credit_profile.png?version=1&modificationDate=1346340713002[image]
...
Mule leverages http://cxf.apache.org[Apache’s CXF framework] to build Web services. The Processor Chain that wraps the **link:/docs/display/33X/SOAP+Component+Reference[SOAP Component]** and HTTP outbound endpoint is a CXF requirement. It ensures that Mule completes all processing activities prior to logging the processing result.

Last in this flow, the **link:/docs/display/33X/Logger+Component+Reference[Logger Component]** logs the payload of the Web service’s response on the message payload as the `Credit Profile`.
...
image:/docs/download/attachments/87687920/look_up_banks.png?version=1&modificationDate=1346340731723[image]
...
image:/docs/download/attachments/87687920/choice_banks2.png?version=1&modificationDate=1346340748803[image]
...
image:/docs/download/attachments/87687920/lookupLoanRequest.png?version=1&modificationDate=1346340769222[image]
...
image:/docs/download/attachments/87687920/find_lowestLoanQuote.png?version=1&modificationDate=1346340785207[image]
...
image:/docs/download/attachments/87687920/expression_bestRate.png?version=1&modificationDate=1346340802192[image]
...
* a **link:/docs/display/33X/Java+Component+Reference[Java Component]** which produces random data to mimic Web service processing.
...
* For more information on setting variables on messages, see link:/docs/display/33X/Variable+Transformer+Reference[Variable Transformer Reference].
