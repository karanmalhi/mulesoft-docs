...
If there is availability for the flight segment, the Groovy transformer sends the payload to the next building block, `Acquire Seats Info`. This is an link:/docs/display/33X/VM+Transport+Reference[In Memory (VM) Outbound Endpoint] that invokes the <<AcquireSeatsInfo Flow>> (visible near the bottom of the Studio canvas). The *AcquireSeatsInfo* flow returns a seat number or a message that reads, `No seat info available`.
