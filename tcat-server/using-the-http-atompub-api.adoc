...
* link:#UsingtheHTTPAtomPubAPI-ImportantFeedURLs[Important Feed URLs]
* link:#UsingtheHTTPAtomPubAPI-ArtifactsasAtomEntries[Artifacts as Atom Entries]
* link:#UsingtheHTTPAtomPubAPI-ArtifactHistoryFeeds[Artifact History Feeds]
* link:#UsingtheHTTPAtomPubAPI-UsingtheLibraries[Using the Libraries]
* link:#UsingtheHTTPAtomPubAPI-ArtifactOperations[Artifact Operations]
* link:#UsingtheHTTPAtomPubAPI-BrowsingtheRepository[Browsing the Repository]
* link:#UsingtheHTTPAtomPubAPI-AddinganArtifact[Adding an Artifact]
* link:#UsingtheHTTPAtomPubAPI-AddingaWorkspace[Adding a Workspace]
* link:#UsingtheHTTPAtomPubAPI-EditinganArtifact'sMetadata[Editing an Artifact's Metadata]
...
Each feed listed above will contain a series of Atom entries that represent artifacts inside the repository. An artifact might look like this:
...
For a more in-depth example using Abdera, see the link:#[AtomPub API Example].
...
For a reference on which characters must be URL-encoded, see http://www.blooberry.com/indexdot/html/topics/urlencoding.htm[this page]. Simply replace any of these characters in your query with the appropriate encoded character. For instance, if you were encoding "select artifact", you would replace the space with `%20`, as that is the URL-encoded representation of the space character.
...
<entry xmlns="http://www.w3.org/2005/Atom">
  ...
  <updated>2008-02-27T01:12:38.758Z</updated>
  <author>
    <name>Ignored</name>
  </author>
...
This will create a new workspace called "MyNewWorkspace" in the parent workspace "Default Workspace".

To add a top-level workspace, POST to `/api/registry`. This workspace will then be listed in `/api/registry;workspaces`.
...
<entry xmlns="http://www.w3.org/2005/Atom">
  ...
  <updated>2008-02-27T00:57:07.116Z</updated>
  <author>
    <name>Galaxy</name>
  </author>
  <summary type="xhtml">
    This will become your new artifact description.
  </summary>
...
Note that you must PUT it to the Atom representation, which means you must append `;atom` to the URL.

To edit an artifact's metadata, you include a `<metadata>` element inside your Atom entry. Any `<property>` elements inside this element will be modified. In the following example, the property `favoriteSauce` is added to the artifact with a value of `ketchup`.
...
----
Any metadata properties that are not present will NOT be deleted. To delete a metadata property, remove all value attributes and child elements:
