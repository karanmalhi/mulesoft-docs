...
image:/documentation-3.2/download/attachments/29425934/thread_pooling.png?version=1&modificationDate=1380316345516[image]
...
The _threading profile_ specifies how the thread pools behave in Mule. You can specify a separate threading profile for each receiver thread pool, flow thread pool, and dispatcher thread pool. The most important setting of each is `maxThreadsActive`, which specifies how many threads are in the thread pool. The link:#TuningPerformance-calculating[MULE3USER:Calculating Threads] section below describes methodologies for determining what this setting should be, whereas the link:#TuningPerformance-threadConfig[MULE3USER:Threading Profile Configuration Reference] section describes the options you can set for each threading profile.
...
For more information on configuring the pooling profile, see link:#TuningPerformance-pooling[MULE3USER:Pooling Profile Configuration Reference] below.
...
====
xslt: Read http://svn.codehaus.org/mule/branches/mule-3.1.x/tools/schemadocs/src/main/resources/xslt//single-element-wiki.xsl error because of: java.io.IOException: Server returned HTTP response code: 401 for URL: http://svn.codehaus.org/mule/branches/mule-3.1.x/tools/schemadocs/src/main/resources/xslt//single-element-wiki.xsl
====
