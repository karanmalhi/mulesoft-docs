= Anypoint Studio 5.3.0 K2 Release
...
*DataWeave* +
Version 1.2 +
See xref:dataweavern[DataWeave Release Notes]
...
* None. See xref:dataweavern[DataWeave Release Notes].
...
|MDF-85|Attributes are lost after using *using* operation
|MDF-86|DW Writer should scape keys when required
...
|MDF-99|Java Writer does not supporting "java.sql.Timestamp" and "java.sql.Time"
|MDF-100|DataWeave does not convert array to set for JavaWriter
|MDF-101|Weave is throwing can not coerce :null to :string when it should work
...
|MDF-105|Null Safe node is not working with enclosed expr
|MDF-106|Data Weave can not handle MessageCollection
...
|MDF-114|Invalid i18n message error
