...
MuleSoft is pleased to announce the early access M1 release of the Mule Runtime 3.8.0.
This release marks an important landmark: the unification of the Mule ESB runtime with the link:/anypoint-platform-for-apis/api-gateway-101[API Gateway] Runtime. From here onwards, the capabilities of both are united into a single runtime.
New features also include TLS security improvements, an improved FlowStack to help troubleshooting and link:/* MULE-user-guide/v/3.8-beta/dataweave[DataWeave] improvements for dealing with flat file formats.
