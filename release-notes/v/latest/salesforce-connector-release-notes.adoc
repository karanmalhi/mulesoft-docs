...
The Version 7.0.1 Salesforce connector is compatible with:
...
* Fixed the way query results are returned. Exceptions related to "No converter found!" are no longer thrown for inner selects (for child relationships).
...
* Batch Operation Results are reported correctly now if the Batch Commit component is used. Before, the connector had problems reporting how many records were successful, and how many failed (all the records were reported as successful).
* Invocation of Apex Rest methods that are void no longer fails.

=== Version 7.0.1 Known Issues

* Batch Operation Results report all records as successful when the Batch Commit component is not used (that is, when only the Batch Step component is used).
...
where
*** apexClassName is the value chosen by the user for the "Apex Class" parameter under the General Category;
*** apexMethodName is the value chosen by the user for the "Method Name" parameter under the General Category;
