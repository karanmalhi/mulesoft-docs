...
The Version 7.0.0 Salesforce connector is compatible with:
...
It uses DevKit 3.7 and its new features (like multi-level Datasense for invokeApexSoapMethod and invokeApexRestMethod) and dates are no longer passed as String, but as Dates and Calendars.

=== Migrating From Older Versions of the Connector to Version 7.0.0
...
. Click the Salesforce Connector version 7.0.0 checkbox, click *Next* and follow the instructions provided by the user interface.
. Restart Studio.
...
If updating the connector causes issues then simply uninstall the old version and install the new one:

. Anypoint Studio: *Help* > *Installation Details* > *Installed Software*:
.. Find the Salesforce connector, select it and then click the *Uninstall* button
. Anypoint Studio: *Help* > *Install New Software*:
.. Select "Anypoint Connectors Update Site" from the dropdown .. Type "Salesforce" in the bar above the dropdown
.. Under "Select" you will find version 7.0.0 of the connector .. Select its checkbox and then click *Next* to begin the installation process.
...
* Improved overall performance for Apex REST invocation by avoiding the download and parsing of the Apex REST class at every invocation; instead we are relying on the information obtained during Datasense and settings in the metadata key id.
...
** the way some metadata key ids are created was changed when the multi-level Datasense was introduced (invokeApexSoapMethod and invokeApexRestMethod are affected by this).
...
* Connector v7.0.0 does not work with versions v28.0, v29.0, v30.0 and v31.0 of the Salesforce API.
* Connector v7.0.0 does not support for Apex Rest the following data types:
