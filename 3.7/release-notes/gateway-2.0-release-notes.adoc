...
* External AES OAuth2 validation policy. See  link:/docs/display/current/External+OAuth+2.0+Token+Validation+Policy[External OAuth 2.0 Token Validation Policy].
* New HTTP Listener Connector. See  link:/docs/display/current/HTTP+Listener+Connector[HTTP Listener Connector].
...
* With the HTTP listener support, the old endpoint based version of the HTTP connector is no longer supported in 2.0. Use the link:/docs/display/current/Migration+Guide+to+API+Gateway+2.0.0+or+later[migration tool] for existing proxies.
* Use register-as when defining autodiscovery for endpoints removed from the schema.

== Migration Guide

. Modify and migrate all deployed applications and proxies to use the new link:/docs/display/current/HTTP+Listener+Connector[HTTP listener] used in the 2.0 runtime. For automatically generated (but not modified) proxies of versions 1.3.X or older, there is a link:/docs/display/current/Migration+Guide+to+API+Gateway+2.0.0+or+later[migration tool.] However, custom apps or post-generation modified proxies will need to be manually modified and re-deployed. 
. Custom policies continue to work. However as-is if policy violations occur, these are not reported to analytics as such events need to include their ID. To ensure that violations are reporting, the policy needs to be modified to include an ID. See link:/docs/display/current/Applying+Custom+Policies[Applying Custom Policies] for examples of both runtimes.
...
* Learn more about the link:/docs/display/current/API+Gateway+101[API Gateway Runtime]
