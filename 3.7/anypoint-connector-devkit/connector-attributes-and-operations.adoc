...
This document provides an overview of the various approaches to selecting a data model and introduces the main styles of API access. You can learn to build connectors for each of them in the pages that follow. 
...
More details can be found in link:/docs/display/current/Defining+Connector+Attributes[Defining Connector Attributes]. 

== Static vs. Dynamic Data Models 

The underlying data model for the API you connect to often drives design decisions about the best way to represent data going to and from your connector. The goal in general is to choose a representation that fits the underlying data, while still supporting the necessary metadata to work with link:/docs/display/current/DataSense[DataSense].
...
More details can be found in Static Data Model & Dynamic Data Model Examples in this document about link:/docs/display/current/Adding+DataSense[Adding DataSense].
...
* *NEXT:* Continue to link:/docs/display/current/Defining+Connector+Attributes[Defining Connector Attributes].
* You can also jump into your specific use case, checking the link:/docs/display/current/Anypoint+Connector+Examples[examples].
