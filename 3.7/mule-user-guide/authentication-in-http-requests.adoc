...
The link:#[HTTP Request Connector] supports four types of authentication:

* link:#AuthenticationinHTTPRequests-basic[Basic authentication]
* link:#AuthenticationinHTTPRequests-nt[NTLM authentication]
* link:#AuthenticationinHTTPRequests-digest[Digest authentication]
* *OAuth* authorization with 
* link:#AuthenticationinHTTPRequests-code[Authorization Code] grant type
* *OAuth* authorization with 
* link:#AuthenticationinHTTPRequests-clientcredentials[Client credentials] grant type
...
image:/docs/plugins/servlet/confluence/placeholder/unknown-attachment?locale=en_GB&version=2[image,title="preemptive basic auth.png"]
....
...
[WARNING]
====
Preemptive Basic Authentication is only supported on Mule 3.7 and later.
====
|===
...
[WARNING]
====
NTLM Authentication is only supported on Mule 3.7 and later.
====
|===

In this example, a GET request is sent to www.example.com/test, adding an "Authorization" header with the provided username and password.
...
image:/docs/plugins/servlet/confluence/placeholder/unknown-attachment?locale=en_GB&version=2[image,title="oauth danceposta.png"]
...
When this is the case, the parameters will be automatically extracted and you can easily use link:/docs/display/current/Mule+Expression+Language+MEL[MEL expressions] later in the flow to reference these values in the Mule Message that was generated from the response to the request to the token URL.
...
It’s possible to customize when Mule will perform one of these requests to obtain a new access token. This is configured through a link:/docs/display/current/Mule+Expression+Language+MEL[MEL Expression] that is evaluated against the Mule Message that is generated from the response of the HTTP Request Connector call.
...
image:/docs/plugins/servlet/confluence/placeholder/unknown-attachment?locale=en_GB&version=2[image,title="oauth dance post a simple.png"]
...
By default, the token manager uses an in-memory object store to store the credentials. You can customize the token manager object store by using the objectStore-ref attribute. link:/docs/display/current/Mule+Object+Stores[See how to configure a custom object store].

== See Also

* link:#[HTTP Request Connector]
* link:#[HTTP Listener Connector]
...
* link:/docs/display/current/Migrating+to+the+New+HTTP+Connector[Migrating to the New HTTP Connector]
