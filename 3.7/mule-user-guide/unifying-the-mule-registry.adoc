...
Mule 3.7 and newer versions provides upgrades to the lifecycle and link:/docs/display/current/Dependency+Injection[dependency injection mechanisms], and unifies the Mule Registry. These changes maintain backwards compatibility.
...
*Important:* Before starting, http://blogs.mulesoft.org/sneak-peek-mule-3-7-deepest-internals/[read the Mule 3.7 internals blog]. 
...
Mule 3.7 and newer converges on SpringRegistry, which is a single Spring registry. The previous Mule TransientRegistry is deprecated. For backward compatibility, TransientRegistry is provided in the Mule software distribution, but not used.
...
When Mule starts, it searches the classpath for `registry-bootstrap.properties` files, which contain keys and class names for objects to automatically add to the registry. For example, the new link:/docs/display/current/HTTP+Connector[HTTP Connector] uses a properties file to register a connection manager. The Batch module uses it to register the batch engine, etc.
