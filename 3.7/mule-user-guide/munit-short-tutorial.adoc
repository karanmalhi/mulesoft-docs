...
If you’ve been reading this tutorial from the beginning, you already know that in MUnit you do not mock `flow-ref` message processors, you mock the flows that would be called by them (see link:#MUnitShortTutorial-flow-ref[above]). That’s what we’re doing here, mocking `exampleFlow2` which was called from `exampleFlow`.
