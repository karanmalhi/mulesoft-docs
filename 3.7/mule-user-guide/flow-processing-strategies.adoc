...
* Should the message be link:#FlowProcessingStrategies-nonblock[non-blocking]? (Available in Mule 3.7 and newer)
...
This page assumes that you are familiar with link:/docs/display/current/Mule+Concepts[Mule flows] and basic link:/docs/display/current/Mule+Application+Architecture[application architecture].

== Implicit Processing Strategies

Even if you do not configure a processing strategy for your flow, the flow follows a processing strategy automatically applied by Mule. The processing strategy can ge:
...
In addition, some inbound endpoints can be configured to operate link:/docs/display/current/Transaction+Management[transactionally], regardless of their exchange pattern. If a flow is transactional, Mule applies a synchronous processing strategy.
...
* *Change to Synchronous* - In cases where Mule has selected queued-asynchronous processing for a flow, you can link:#FlowProcessingStrategies-SpecifyingaSpecificProcessingStrategy[specify a synchronous flow instead]. You might want to specify a synchronous flow to achieve reliability. Remember that the synchronous strategy is ideally suited for flows where the flow's inbound endpoint must be notified of all errors that occur during the processing of the message. The link:/docs/display/current/Reliability+Patterns[Reliability Patterns] document discusses this situation further. Although this is not applicable to most use cases, if Mule has selected queued-asynchronous processing for a flow, you can specify another kind of asynchronous processing strategy instead. *Note*: You cannot force a queued-asynchronous processing strategy on a synchronous flow. 

* *Fine Tune*: You can accept Mule's choice of the queued-asynchronous flow processing strategy, but then link:#FlowProcessingStrategies-Fine-TuningaProcessingStrategy[fine-tune the processing strategy]. *Note*: You cannot fine-tune a synchronous flow.

* *Wrap a Flow:* You can wrap one or more message processors within your synchronous flow in an link:/docs/display/current/Async+Scope+Reference[asynchronous scope] to define a processing block that Mule processes asynchronously with the main flow. You can also create an asynchronous flow by wrapping a flow-ref in an asynchronous scope. 

* *Subflow*: You can call out to a subflow (which always processes synchronously) or a synchronous flow from any flow, regardless of what processing strategy is applied to the triggering flow. The triggering flow waits until this synchronous processing block returns a message before proceeding with processing. See link:/docs/display/current/Flows+and+Subflows[Flows and Subflows] for more information about subflows and synchronous flows.

* *Non-Blocking*: You can use the new link:#FlowProcessingStrategies-nonblock[Non-Blocking processing] strategy available in Mule 3.7 and newer to take advantage of NIO with the link:/docs/display/current/HTTP+Connector[HTTP Connector] and avoid having threads blocked while waiting for HTTP responses, which also requires less tuning. *Note*: Not all components are currently supported, unsupported components cause a flow to fall back to synchronous processing. 

* *Custom*: You can create a link:#FlowProcessingStrategies-CreatingaProcessingStrategy[custom flow processing strategy] to fit your exact needs. For instance, you might prefer a queued-asynchronous flow that uses an increased number of threads to handle high peak loads. 
...
|*Synchronous* |*No.* You cannot force a flow with a request-response exchange pattern and/or transactionality to be asynchronous. |*No*. You cannot fine-tune the synchronous processing strategy. You can, however, customize the inbound endpoint connector receiver link:/docs/display/current/Tuning+Performance[threading profile]. | *Yes.* You can use an Async Scope or an asynchronous flow to cause Mule to process a selected block of message processors asynchronously. link:#FlowProcessingStrategies-CreatinganAsynchronousProcessingBlock[See below]. |*Yes.* link:#FlowProcessingStrategies-CreatingaCustomProcessingStrategy[See below].
|*Queued-Asynchronous* |*Yes.* You can override an implicitly selected queued-asynchronous processing strategy by explicitly declaring a synchronous processing strategy (or, in rare use cases, a different kind of asynchronous processing strategy) instead. link:#FlowProcessingStrategies-SpecifyingaProcessingStrategy[See below]. |*Yes.* You can fine-tune the queued-asynchronous processing strategy to meet your needs. link:#FlowProcessingStrategies-Fine-TuningaQueued-AsynchronousProcessingStrategy[See below]. |*Yes.* A synchronous link:/docs/display/current/Flows+and+Subflows[flow or subflow] processes a selected block of message processors synchronously, regardless of the processing strategy of the main flow. |*Yes.* link:#FlowProcessingStrategies-CreatingaCustomProcessingStrategy[See below].
|=====
...
[NOTE]
Note that in addition to fine-tuning the attributes of the queued-asynchronous processing strategy, you can also link:/docs/display/current/Tuning+Performance[tune performance] of your Mule application by analyzing and configuring thread profiles.
...
. Wrap one or more processors in an link:/docs/display/current/Async+Scope+Reference[async scope]
. Create an link:/docs/display/current/Flows+and+Subflows[asynchronous flow] by wrapping a flow-ref element in an async scope so that the contents of the flow will be processed asynchronously with the triggering flow
...
For more information about configuring the asynchronous scope element, refer to the link:/docs/display/current/Async+Scope+Reference[Async Scope reference].
...
* The flow's inbound endpoint must be notified of all errors that occur during the processing of the message. This situation is discussed further in link:/docs/display/current/Reliability+Patterns[Reliability Patterns].
...
image:/docs/download/attachments/131957124/queued+asynchronous.png?version=1&modificationDate=1435683779928[image]
...
Mule has traditionally always used a threaded processing model for handling incoming requests from a Message Sources using the request-response Message Exchange Pattern. This means that the whole Mule Flow, both request and response phases, are executed using the same thread. This same thread is also used to send the response back to the client. This threaded request-response processing is used whenever the link:#FlowProcessingStrategies-SyncProc[Synchronous Processing Strategy] is used, either explicitly or by Mule  implicitly selecting this strategy when one isn’t specified.  This processing model can more easily be shown with this diagram:

image:/docs/download/attachments/131957124/SyncProcStrat.png?version=1&modificationDate=1436407277534[image]
...
image:/docs/download/attachments/131957124/NonBlockProcStrat.png?version=1&modificationDate=1436407774553[image]

In this example, the HTTP components support Non-Blocking IO (NIO). 
...
The main advantage of this processing-strategy is that in a flow that uses this processing strategy, as long as the flow doesn’t block for IO or use `Thread.sleep()` anywhere, no tuning is required to achieve optimum performance. With the synchronous processing strategy, by comparison, it is necessary to tune the listener thread pool size based on the expected usage of the flow.
...
Before using the non-blocking strategy, ensure you are also familiar with what is currently supported and its limitations as follows:

* The only Message Source that supports non-blocking is the link:/docs/display/current/HTTP+Listener+Connector[HTTP Connector Listener]
* The only Message Processor that supports non-blocking is the link:/docs/display/current/HTTP+Request+Connector[HTTP Connector Request]
* The only Message Exchange Pattern currently supported is link:/docs/display/current/HTTP+Request-Response+with+Logger+Example[Request-Response]
* Only a limited set of components are fully supported for use for use in non-blocking flows
...
image:/docs/download/attachments/131957124/NonBlockWithSyncFB.png?version=1&modificationDate=1436412431771[image]
...
* Refer to the link:/docs/display/current/Async+Scope+Reference[Async Scope Reference] for information on how to embed an asynchronous processing block in an otherwise synchronous flow.
* Review the different kinds of link:/docs/display/current/Flows+and+Subflows[Flows and Subflows] in Mule.
* Read more about link:/docs/display/current/Reliability+Patterns[Reliability Patterns].
* Find out how to link:/docs/display/current/Tuning+Performance[tune performance] by analyzing and configuring threading profiles. 
