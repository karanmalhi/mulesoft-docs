...
For more about Mule transformers, see link:/docs/display/current/Using+Transformers[Using Transformers].
...
For a current reference to using expressions in Mule, see link:/docs/display/current/Mule+Expression+Language+MEL[Mule Expression Language MEL]. 
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see link:/docs/display/current/Logger+Component+Reference[Logger Component Reference]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see link:/docs/display/current/Routers[Routing Message Processors].
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see link:/docs/display/current/Entry+Point+Resolver+Configuration+Reference[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

❾ Calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about connectors, see link:/docs/display/current/Anypoint+Connectors[Anypoint Connectors].
...
❶❺ Writes the message that caused the exception to a JMS queue, where it can be examined.  For more about exception strategies, see link:/docs/display/current/Error+Handling[Error Handling].
...
For more about configuration patterns, see link:/docs/display/current/Using+Mule+Configuration+Patterns[Using Mule Configuration Patterns].
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see link:/docs/display/current/Transaction+Management[Transaction Management].
...
For more about Mule security, see link:/docs/display/current/Configuring+Security[Configuring Security].
