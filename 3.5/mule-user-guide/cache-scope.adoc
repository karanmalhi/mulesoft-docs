...
*link:#CacheScope-CompleteCodeExample[Skip to the Code]*
...
[NOTE]
The default caching strategy uses an InMemoryObject store and should only be used for testing; on actual implementations you should link:#CacheScope-CreatingaCachingStrategy[provide a custom caching strategy].
...
For details on configuring caching strategies, see link:#CacheScope-cachingstrat[Creating a Caching Strategy].
...
+
image:/docs/download/attachments/122752243/cache.png?version=1&modificationDate=1418679466556[image]
+
...
+
image:/docs/download/attachments/122752243/cache_scope.png?version=1&modificationDate=1399486996629[image] +
+
...
.2+|*Caching strategy reference* |Use Default caching strategy |(Default) Select if you want the cache scope to follow Mule’s link:#CacheScope-DefaultCachingStrategy[default caching strategy]. |n/a
|Reference to a strategy |Select to configure the cache scope to follow a link:#CacheScope-CreatingaGlobalCachingStrategy[global caching strategy] that you have created; select the global caching strategy from the drop-down menu or create one by clicking the image:/docs/s/en_GB/3391/c989735defd8798a9d5e69c058c254be2e5a762b.76/_/images/icons/emoticons/add.png[(plus)]. |`cachingStrategy-ref="Caching_Strategy"`
...
|*cachingStrategy-ref* |  |(Optional) Specify the name of the global caching strategy that you have defined as a global element. If no `cachingStrategy-ref` is defined, Mule will use the link:#CacheScope-DefaultCachingStrategy[default caching strategy].
|===
...
+
image:/docs/download/attachments/122752243/caching_strategy_general.png?version=1&modificationDate=1399486996655[image] +
+
...
|*Object Store* | |(Optional) Configure an object store in which Mule will store all of the scope’s cached responses. Refer to the link:#CacheScope-ConfiguringanObjectStoreforCache[Configuring an Object Store for Cache] section below for configuration specifics. Unless otherwise configured, Mule stores all cached responses in an InMemoryObjectStore by default. |`<custom-object-store>` +
 +
`<in-memory-store>` +
 +
`<managed-store>` +
...
+
image:/docs/download/attachments/122752243/caching_strategy_advanced.png?version=1&modificationDate=1399486996646[image]
...
|*custom-object-store* .3+|See link:#CacheScope-ConfiguringAnObjectStoreforCache[Configuring An Object Store for Cache], below
|*in-memory-store*
|*managed-store*
|*simple-text-file-store*
|===
...
By default, Mule stores all cached responses in an InMemoryObjectStore. link:#CacheScope-CreatingaCachingStrategy[Create a caching strategy] and define a new link:/docs/display/current/Mule+Object+Stores[object store] if you want to customize the way Mule stores cached responses.
...
image:/docs/download/attachments/122752243/cache+flow+2.png?version=1&modificationDate=1418679714405[image]

 View the XML

Note: This project requires  link:/docs/download/attachments/122752243/FibonacciResponseGenerator.java?version=1&modificationDate=1403121760104[FibonacciResponseGenerator.java]
...
image:/docs/download/attachments/122752243/reqA.png?version=1&modificationDate=1399486996686[image]

image:/docs/download/attachments/122752243/reqB.png?version=1&modificationDate=1399486996694[image]

image:/docs/download/attachments/122752243/reqC.png?version=1&modificationDate=1399486996703[image]

image:/docs/download/attachments/122752243/reqD.png?version=1&modificationDate=1399486996712[image]
...
* Read more about link:/docs/display/current/Scopes[Scopes] in Mule.
