...
Developing in Maven is greatly simplified by the link:/docs/display/current/Maven+Tools+for+Mule+ESB[Maven Tools for Mule ESB] plugin, which provides Maven archetypes for building Mule applications.

== Assumptions

This document assumes that you are working with Maven outside of Anypoint Studio, and thus will be using archetypes to build Mule applications. See link:/docs/display/current/Using+Maven+with+Mule[Using Maven with Mule] for an overview of using Maven to manage your Mule projects.

[NOTE]

Note that you generally do not need to modify your settings.xml file if you are link:/docs/display/current/Using+Maven+with+Mule[working with Maven through Studio], as Studio keeps track of the dependencies you'll need and attempts to manage them for you in the application's associated POM file. If dependency errors occur, you may need to manually adjust your POM file, and, in some cases, in your settings.xml. Refer to the Maven documentation for the http://maven.apache.org/pom.html[pom.xml] and http://maven.apache.org/settings.html[settings.xml] for specific instructions.
...
* Learn about link:/docs/display/current/Maven+Tools+for+Mule+ESB[Maven tools for Mule ESB], a Mule plugin to develop Mule applications in Maven.
* Control instances of Mule ESB using the link:/docs/display/current/Mule+ESB+Plugin+For+Maven[Mule ESB plugin for Maven].
