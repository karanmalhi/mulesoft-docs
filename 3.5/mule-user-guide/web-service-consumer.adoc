...
link:#WebServiceConsumer-WebServiceConsumerandDataSenseExplorer[Web Service Consumer and DataSense Explorer]

* link:#WebServiceConsumer-MessageStateBeforeWebServiceConsumer[Message State Before Web Service Consumer]
* link:#WebServiceConsumer-MessageStateAfterWebServiceConsumer[Message State After Web Service Consumer]

link:#WebServiceConsumer-SecurelyConnectingtoaSOAPWebService[Securely Connecting to a SOAP Web Service]

link:#WebServiceConsumer-UsingBasicAuthenticationandSSL[Using Basic Authentication and SSL]

* link:#WebServiceConsumer-ReferencingtheDeprecatedHTTPTransport[Referencing the Deprecated HTTP Transport]

link:#WebServiceConsumer-CompleteCodeExample[Complete Code Example]

link:#WebServiceConsumer-SeeAlso[See Also]
...
image:/docs/download/attachments/122752084/con+globitos+new.png?version=1&modificationDate=1418415292153[image]

image:/docs/download/attachments/122752084/con+globitos+new+2.png?version=1&modificationDate=1418415316884[image]

[cols=",",]
|===
|image:/docs/images/icons/emoticons/warning.png[image] |The component formerly known as the *SOAP Connector* is now known as the *link:/docs/display/current/CXF+Component+Reference[CXF Component]* . It should not be confused with the Web Service Consumer.
...
To prepare an application to use a Web Service as in one of the examples above, you must first add the Web service's WSDL to your Mule project, then configure, at minimum, the required parameters for the Web Service Consumer element and a Global Web Service Consumer element. The diagram and section below describe how to address these requirements.
 ...
 image:/docs/download/attachments/122752084/wsc_workflow2.png?version=1&modificationDate=1410984989527[image] +
...
* If DataMapper *precedes* the fully-configured Web Service Consumer, DataMapper pre-populates its *Output values* (see below).  +
 image:/docs/download/attachments/122752084/wsc_dm_output.png?version=1&modificationDate=1410984989467[image] To take advantage of the Web Service Consumer-DataSense-DataMapper magic, be sure to configure these elements in the following order:
...
[cols=",",]
|===
|image:/docs/images/icons/emoticons/warning.png[image] |Outbound properties that begin with a *`soap.`* prefix will be treated as SOAP headers and ignored by the transport. All properties that aren't named with a `soap.` prefix will be treated as transport headers (by default, the WSC uses the HTTP transport).
